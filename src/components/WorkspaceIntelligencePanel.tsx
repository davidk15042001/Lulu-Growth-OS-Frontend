...
export function WorkspaceIntelligencePanel({
  ...
  if (loading && !bundle) return <section className="lulu-workspace-intelligence-panel mb-7 rounded-xl border border-border bg-card p-5"><div className="flex items-center gap-3 text-sm text-muted-foreground"><RefreshCw className="animate-spin" size={16} /> {pageId ? 'Loading page intelligence…' : 'Loading workspace intelligence…'}</div></section>;
  if (error) return <section className="lulu-workspace-intelligence-panel mb-7 rounded-xl border border-chart-5/30 bg-chart-5/5 p-5"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 shrink-0 text-chart-5" size={18} /><div className="min-w-0"><h2 className="text-sm font-semibold text-foreground">Intelligence data unavailable</h2><p className="mt-1 text-sm text-muted-foreground">{error}</p><button type="button" onClick={() => void load()} className="mt-3 inline-flex items-center gap-2 rounded-md border border-border px-3 py-2 text-xs text-foreground hover:bg-secondary"><RefreshCw size={13} /> Retry</button></div></div></section>;
  if (!bundle?.snapshot) return <section className="lulu-workspace-intelligence-panel mb-7 rounded-xl border border-border bg-card p-5"><div className="flex items-start gap-3"><Clock3 className="mt-0.5 shrink-0 text-muted-foreground" size={18} /><div><h2 className="text-sm font-semibold text-foreground">{pendingTitle}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{pendingDescription}</p></div></div></section>;
