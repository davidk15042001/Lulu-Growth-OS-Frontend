...
function ProviderConnectionPanel({ provider, platform, connected, busy, oauthHelpVisible, onConnect, onDisconnect, onRestartWordPress, t }: {
  provider: Provider;
  platform: Platform | undefined;
  connected: boolean;
  busy: boolean;
  oauthHelpVisible: boolean;
  onConnect: () => void;
  onDisconnect: () => void;
  onRestartWordPress: () => void;
  t: (value: string) => string;
}) {
  const providerLabel = provider === "wordpress" ? "WordPress / Jetpack" : "Webflow";
  const pending = Boolean(platform && ["pending", "error"].includes(platform.connectionStatus.toLowerCase()));
  const accountName = platform?.settings?.accountName ? String(platform.settings.accountName) : "";
...
export default function App() {
  ...
  const [generationJob, setGenerationJob] = useState<TrackedGenerationJob | null>(() => readStoredGenerationJob());
  ...
  useEffect(() => {
    if (!workspaceId || !selectedSite?.id || generationJob) return;
    let cancelled = false;
    websitesApi.getActiveGenerationJob(workspaceId, selectedSite.id)
      ...
      .catch(() => undefined);
    return () => { cancelled = true; };
  }, [workspaceId, selectedSite?.id, selectedSite?.provider, generationJob]);
  ...
  const cancelAutomaticGeneration = async () => {
    if (!workspaceId || !generationJob || !isRunningGenerationJob(generationJob.job) || generationCancelling) return;
    setGenerationCancelling(true);
    setError("");
    try {
      const response = await websitesApi.cancelGenerationJob(workspaceId, generationJob.siteId, generationJob.job.id);
      setGenerationJob((current) => current ? { ...current, job: response.data } : current);
      setGenerationPanelOpen(true);
      void refresh();
    } catch (requestError) {
      setError(getFriendlyErrorMessage(requestError, t("Website generation could not be cancelled.")));
    } finally {
    ...
    const poll = async () => {
      try {
        ...
        if ([401, 404].includes(errorStatus(requestError) ?? -1)) {
          ...
          setGenerationJob(null);
          setGenerationStarting(null);
          setError("Der Generation-Job ist auf dem Server nicht mehr vorhanden. Die alte Planung wurde aus dieser Sitzung entfernt.");
          return;
        }
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setError(getFriendlyErrorMessage(requestError, "Die Websites und Verbindungen konnten nicht geladen werden."));
    }
  };
  ...
  const generationPercent = generationProgress?.percent ?? (generationStatus === "published"
    ? 100
    : generationStatus === "failed" || generationStatus === "cancelled" ? (Number(generationJob?.job.attemptCount ?? 0) > 0 ? 18 : 0)
    ...
    <div className="min-h-screen bg-[var(--background)] text-foreground">
      ...
      <main data-lulu-scroll-container className="min-h-screen min-w-0 overflow-x-hidden">
        <div className="mx-auto max-w-[1400px] px-5 py-6 sm:px-8 sm:py-8">
          ...
          {provider === "webflow" && sectionParam === "webflow-9014" && <div className="space-y-5"><ProviderConnectionPanel provider="webflow" platform={connectedPlatform} connected={providerConnected} busy={connectionBusy === "webflow"} oauthHelpVisible={false} onConnect={() => void connectProvider("webflow")} onDisconnect={() => void disconnectProvider("webflow")} onRestartWordPress={() => undefined} t={t} />{providerConnected && <WebflowDashboard content={providerData} loading={wordpressContentLoading} />}</div>}
          {sectionParam === "domains-9018" && <DomainOwnershipPanel key={selectedSite?.id ?? 'none'} site={selectedSite ?? null} />}
