...
export function LuluIntegrations() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [platformResponse, whatsappResponse] = await Promise.all([
  ...
  const showManagedWhatsApp = Boolean(whatsappConnection?.adminFallback.configured) && !whatsappPlatform;
  const configuredCount = platforms.length + (showManagedWhatsApp ? 1 : 0);
