...
const activity = ['New order #10847 from Connected customer — $142.00', 'Order #10839 fulfilled and shipped', 'Payment received for order #10845', 'Refund issued for order #10821 — $67.00', 'New customer Maria Garcia registered', 'Cart abandoned — $234.00', 'Inventory updated: 50 units added to Cotton Tee', 'Store sync completed — Lulu Store'];
...
export function LuluEcommerceOverview() {
  ...
  const liveSnapshot: Array<[string, string, string, Tone]> = [['Today’s Revenue', orderRecords.length ? (orderRecords.reduce((sum, record) => sum + (Number(String(record.valueAmount ?? '').replace(/[^0-9.-]/g, '')) || 0), 0)).toLocaleString() : '—', 'Live order records', 'teal'], ['Orders', String(orderRecords.length), 'Live records', 'violet'], ['Customers', `${customerRecords.length}`, 'Live records', 'blue'], ['Average Order Value', orderRecords.length ? (orderRecords.reduce((sum, record) => sum + (Number(String(record.valueAmount ?? '').replace(/[^0-9.-]/g, '')) || 0), 0) / orderRecords.length).toLocaleString() : '—', 'Calculated', 'teal'], ['Conversion', '—', 'Analytics contract required', 'violet'], ['Pending Orders', '—', 'Status data required', 'amber'], ['Unfulfilled Orders', '—', 'Fulfillment data required', 'amber'], ['Returns', '—', 'Returns contract required', 'teal']];
  ...
  return <div className="min-h-screen bg-[var(--background)] font-sans text-foreground">
    ...
    <main className="mx-auto max-w-[1480px] space-y-7 p-5 lg:p-8">{ecommerceError && <div role="alert" className="rounded-lg border border-chart-5/30 bg-chart-5/10 px-4 py-3 text-sm text-chart-5">Ecommerce overview data could not be loaded. Check connected stores, orders and customers, then try again.</div>}{!ecommerceLoading && !ecommerceError && orderRecords.length === 0 && customerRecords.length === 0 && storeRecords.length === 0 && <div className="rounded-lg border border-dashed border-border bg-card px-4 py-3 text-sm text-muted-foreground">No ecommerce records are available yet.</div>}<p className="-mt-3 max-w-2xl text-sm text-muted-foreground">Monitor your ecommerce business, stores, orders, customers, products and operations from one place.</p>
