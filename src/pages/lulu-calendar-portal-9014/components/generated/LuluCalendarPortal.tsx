...
export default function CalendarPortal() {
  ...
  return <main className="calendar-page">
    ...
    <section className="calendar-timeline calendar-timeline--native">{!events.length && <div className="calendar-empty-state"><CalendarDays size={30} /><h2>{t('No appointments yet')}</h2><p>{t('Create your first appointment to start a customer meeting.')}</p>{permissions.canEdit && <button type="button" className="calendar-button" onClick={() => setShowCreate(true)}><Plus size={16} />{t('Create appointment')}</button>}</div>}{groupedEvents.map((group) => <article key={group.key} className="calendar-day"><header><h2>{group.label}</h2><span>{group.items.length} {t('appointments')}</span></header><div className="calendar-event-list">{group.items.map((event) => <div key={event.id} className={`calendar-event calendar-event--native status-${event.status}`}><div className="calendar-event__time"><strong>{dateLabel(event.startAt, language, { weekday: undefined })}</strong><span>{dateLabel(event.endAt, language, { weekday: undefined, day: undefined, month: undefined, year: undefined })}</span></div><div className="calendar-event__content"><div><h3>{event.title}</h3>{event.description && <p>{event.description}</p>}</div><div className="calendar-event__meta"><span><Video size={14} />{t('Agora meeting')}</span>{event.location && <span><MapPin size={14} />{event.location}</span>}<span>{event.timezone}</span></div><div className="calendar-event__actions">{event.guestJoinPath && <button type="button" onClick={() => { void navigator.clipboard?.writeText(event.guestJoinPath!); setNotice(t('Link copied.')); }}><Link2 size={14} />{t('Copy guest link')}</button>}<button type="button" disabled={actionBusy} className="danger" onClick={() => void removeEvent(event)}><Trash2 size={14} />{t('Delete')}</button></div></div></div>)}</div></article>)}</section>
    {showCreate && <CreateEventDialog workspaceId={workspaceId} busy={actionBusy} onClose={() => setShowCreate(false)} onError={setError} onCreated={(event, link) => { setEvents((current) => [...current, event].sort((a, b) => a.startAt.localeCompare(b.startAt))); setCreatedLink(link); setShowCreate(false); setNotice(t('Appointment created.')); }} />}
  </main>;
}

function CreateEventDialog({ workspaceId, busy, onClose, onError, onCreated }: { workspaceId: string; busy: boolean; onClose: () => void; onError: (value: string) => void; onCreated: (event: NativeCalendarEvent, link: string) => void }) {
  const t = useTranslation();
  const [title, setTitle] = useState(''); const [description, setDescription] = useState(''); const [startAt, setStartAt] = useState(''); const [endAt, setEndAt] = useState(''); const [location, setLocation] = useState(''); const [saving, setSaving] = useState(false);
  async function submit() {
    const start = localDateTimeToIso(startAt); const end = localDateTimeToIso(endAt);
    if (!title.trim() || !start || !end) { onError(t('Enter a title, start time and end time.')); return; }
    setSaving(true);
    try { const result = await calendarApi.createNativeEvent(workspaceId, { title: title.trim(), ...(description.trim() ? { description: description.trim() } : {}), startAt: start, endAt: end, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone, ...(location.trim() ? { location: location.trim() } : {}) }); onCreated(result.data, result.data.guestJoinPath); }
    catch (cause) { onError(getFriendlyErrorMessage(cause, t('The appointment could not be created.'))); }
    finally { setSaving(false); }
  }
  return <div className="calendar-modal-backdrop"><section className="calendar-dialog calendar-dialog--modern" role="dialog" aria-modal="true" aria-labelledby="calendar-create-title"><header><div><p className="calendar-eyebrow">Lulu / Agora RTC</p><h2 id="calendar-create-title">{t('Create appointment')}</h2><p>{t('Customers join through a secure link without an account.')}</p></div><button type="button" onClick={onClose} aria-label={t('Close')}><X /></button></header><div className="calendar-form-grid"><label className="wide">{t('Title')}<input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder={t('Product consultation')} /></label><label>{t('Starts')}<input type="datetime-local" value={startAt} onChange={(event) => setStartAt(event.target.value)} /></label><label>{t('Ends')}<input type="datetime-local" value={endAt} onChange={(event) => setEndAt(event.target.value)} /></label><label className="wide">{t('Location (optional)')}<input value={location} onChange={(event) => setLocation(event.target.value)} placeholder={t('Agora video meeting')} /></label><label className="wide">{t('Description (optional)')}<textarea rows={3} value={description} onChange={(event) => setDescription(event.target.value)} /></label></div><footer><button type="button" className="calendar-button calendar-button--secondary" onClick={onClose}>{t('Cancel')}</button><button type="button" className="calendar-button" disabled={busy || saving} onClick={() => void submit()}>{saving ? <LoaderCircle className="spin" size={16} /> : <Plus size={16} />}{t('Create appointment')}</button></footer></section></div>;
}
