...
import { ArrowLeft, Download, FileCheck2, FileText, Plus, RefreshCw, Send, ShieldAlert, Trash2 } from 'lucide-react';
...
type LineDraft = { productId: string; productName: string; sku: string; quantity: string; unitPrice: string; discount: string; tax: string; quantityUnit: string };
const emptyLine = (): LineDraft => ({ productId: '', productName: '', sku: '', quantity: '1', unitPrice: '0', discount: '0', tax: '0', quantityUnit: 'pcs' });
...
export default function CommercialDocumentsPage({ kind, create = false }: { kind: Kind; create?: boolean }) {
  ...
  const previewTotals = useMemo(() => {
    const subtotal = lines.reduce((sum, line) => sum + numeric(line.quantity) * numeric(line.unitPrice), 0);
    const discount = lines.reduce((sum, line) => sum + numeric(line.discount), 0); const tax = lines.reduce((sum, line) => sum + numeric(line.tax), 0);
    return { subtotal, discount, tax, total: subtotal - discount + tax + numeric(form.shippingTotal) };
  ...
  const downloadPdf = async () => {
    if (!selectedWorkspace || kind !== 'invoices' || !selected) return;
    setDownloadingPdf(true); setError('');
    try {
      const blob = await commercialDocumentsApi.downloadInvoicePdf(selectedWorkspace.id, selected.id);
      const url = URL.createObjectURL(blob);
      const anchor = document.createElement('a');
      anchor.href = url;
      anchor.download = `${(selected as Invoice).invoiceNumber || 'invoice'}.pdf`;
      anchor.click();
  ...
  if (create) return <WorkspaceSurfaceShell activeSlug={activeSlug}><main className="page-frame grid min-h-[70vh] place-items-center bg-[var(--background)] p-8"><div className="max-w-lg text-center"><ShieldAlert className="mx-auto opacity-50"/><h1 className="mt-4 text-xl font-semibold">{t('Documents are created automatically')}</h1><p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">{t('Lulu agents create invoices and quotes from your company profile, products, customers and approved policies. Manual creation is not available in the workspace.')}</p><button type="button" onClick={() => navigateApp(kind === 'quotes' ? routes.app.quotes : routes.app.invoices)} className="mt-5 inline-flex items-center rounded-xl border border-[var(--border)] px-4 py-2.5 text-sm font-semibold hover:bg-[var(--secondary)]">{t('Back')}</button></div></main></WorkspaceSurfaceShell>;
  ...
  return <WorkspaceSurfaceShell activeSlug={activeSlug}><main className="page-frame min-h-screen bg-[var(--background)] p-4 sm:p-8"><div className="mx-auto max-w-7xl space-y-6"><header className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Lulu Commerce</p><h1 className="text-3xl font-semibold">{title}</h1><p className="mt-2 text-sm text-[var(--muted-foreground)]">{kind === 'quotes' ? t('Offers and quotes created by Lulu agents.') : t('Invoices created by Lulu agents.')}</p></div><div className="flex gap-2"><button type="button" onClick={() => void load()} className="rounded-xl border border-[var(--border)] p-2.5" aria-label={t('Refresh')}><RefreshCw size={16} className={loading ? 'animate-spin' : ''}/></button></div></header>{error ? <div role="alert" className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">{error}</div> : null}<section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(340px,.8fr)]"><div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]"><div className="border-b border-[var(--border)] px-5 py-4 text-sm text-[var(--muted-foreground)]">{loadState === 'loading' || loadState === 'error' ? '—' : items.length} {t('documents')}</div>{(loadState === 'loading' || loadState === 'refreshing') && items.length === 0 ? <div className="grid min-h-56 place-items-center text-sm text-[var(--muted-foreground)]">{t('Loading…')}</div> : (loadState === 'error' || loadState === 'stale') && items.length === 0 ? <div className="grid min-h-56 place-items-center p-8 text-center text-sm text-[var(--muted-foreground)]"><ShieldAlert size={30} className="mb-3 opacity-40"/><p>{t('Documents unavailable.')}</p><p className="mt-1">{t('Refresh to verify the current workspace state.')}</p></div> : loadState === 'ready' && items.length === 0 ? <div className="grid min-h-56 place-items-center p-8 text-center text-sm text-[var(--muted-foreground)]"><FileText size={30} className="mb-3 opacity-40"/><p>{kind === 'quotes' ? t('No quotes yet.') : t('No invoices yet.')}</p><p className="mt-1">{t('Lulu will create the first document automatically when it is needed.')}</p></div> : <div className="divide-y divide-[var(--border)]">{items.map((item) => <button key={item.id} type="button" onClick={() => void open(item)} className={`flex w-full items-center justify-between gap-4 p-4 text-left hover:bg-[var(--secondary)] ${selected?.id === item.id ? 'bg-[var(--secondary)]' : ''}`}><div className="min-w-0"><p className="truncate font-medium">{kind === 'quotes' ? (item as Quote).quoteNumber : (item as Invoice).invoiceNumber}</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">{item.creationMode} · {item.status} · {new Date(item.createdAt).toLocaleDateString()}</p></div><strong className="shrink-0">{kind === 'quotes' ? `${(item as Quote).grandTotal ?? '0'} ${(item as Quote).currency}` : `${(item as Invoice).grandTotal ?? '0'} ${(item as Invoice).currency}`}</strong></button>)}</div>}</div><div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5">{selected && detail ? <Detail kind={kind} item={selected} detail={detail} onSend={() => void send()} onDownloadPdf={() => void downloadPdf()} downloadingPdf={downloadingPdf} canWrite={canSendSelected}/> : <div className="grid min-h-56 place-items-center text-center text-sm text-[var(--muted-foreground)]"><ShieldAlert size={30} className="mb-3 opacity-40"/><p>{t('Select a document to inspect it.')}</p></div>}</div></section></div></main></WorkspaceSurfaceShell>;
}
...
function DocumentPreview({ kind, form, lines, totals, customers, sellerProfile }: { kind: Kind; form: { customerRecordId: string; currency: string; invoiceType: string; dueDate: string; validUntil: string; shippingTotal: string }; lines: LineDraft[]; totals: { subtotal: number; discount: number; tax: number; total: number }; customers: WorkspaceRecord[]; sellerProfile: DocumentSellerProfile | null }) {
  const t = useTranslation();
  const customer = customers.find((item) => item.id === form.customerRecordId);
  const money = (value: number) => new Intl.NumberFormat(undefined, { style: 'currency', currency: form.currency || 'CNY' }).format(value);

  return <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white text-slate-900 shadow-[0_20px_60px_-35px_rgba(15,23,42,.45)]">
    <div className="h-1.5 bg-gradient-to-r from-indigo-500 via-violet-500 to-cyan-400" />
    <div className="p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-5 border-b border-slate-200 pb-7"><div className="flex items-start gap-4">{sellerProfile?.logoUrl ? <img src={sellerProfile.logoUrl} alt={sellerProfile.companyName || t('Company logo')} className="h-14 w-14 rounded-xl bg-white object-contain p-1 shadow-sm ring-1 ring-slate-200"/> : null}<div><p className="text-xs font-semibold uppercase tracking-[.2em] text-indigo-600">{sellerProfile?.companyName || 'Lulu Commerce'}</p><h2 className="mt-2 text-3xl font-semibold tracking-tight">{kind === 'quotes' ? t('Quote preview') : t('Invoice preview')}</h2><p className="mt-1 text-sm text-slate-500">{kind === 'quotes' ? t('Create, version and send commercial offers.') : t('Issue, send and track customer invoices.')}</p></div></div><span className="rounded-full border border-indigo-100 bg-indigo-50 px-3.5 py-1.5 text-xs font-semibold uppercase tracking-wide text-indigo-700">{kind === 'quotes' ? 'DRAFT QUOTE' : form.invoiceType}</span></div>
      <div className="mt-7 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{t('Customer')}</p><p className="mt-2 font-semibold text-slate-900">{customer?.name || t('Select customer')}</p><p className="mt-1 text-xs text-slate-500">{customer ? t('Customer record') : t('Select customer')}</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{kind === 'quotes' ? t('Valid until') : t('Due date')}</p><p className="mt-2 font-semibold text-slate-900">{kind === 'quotes' ? form.validUntil || '—' : form.dueDate || '—'}</p><p className="mt-1 text-xs text-slate-500">{t('Currency')}: {form.currency}</p></div></div>
      <div className="mt-7 overflow-hidden rounded-2xl border border-slate-200"><div className="grid grid-cols-[minmax(0,1fr)_auto_auto] gap-4 bg-slate-900 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-white"><span>{t('Item')}</span><span>{t('Quantity')}</span><span className="text-right">{t('Total')}</span></div>{lines.filter((line) => line.productName.trim()).map((line, index) => <div key={index} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-4 border-t border-slate-200 px-4 py-4 text-sm"><span className="min-w-0"><strong className="block truncate font-semibold">{line.productName}</strong><small className="mt-1 block text-xs text-slate-500">{line.sku || '—'}</small></span><span className="whitespace-nowrap text-slate-600">{line.quantity} {line.quantityUnit}</span><span className="whitespace-nowrap text-right font-medium">{money(numeric(line.quantity) * numeric(line.unitPrice) - numeric(line.discount) + numeric(line.tax))}</span></div>)}{!lines.some((line) => line.productName.trim()) && <p className="px-4 py-10 text-center text-sm text-slate-500">{t('Add a line item to preview it.')}</p>}</div>
      <div className="mt-7 flex justify-end"><dl className="w-full max-w-sm space-y-2.5 rounded-2xl bg-slate-50 p-5 text-sm"><Info label={t('Subtotal')} value={money(totals.subtotal)}/><Info label={t('Discount')} value={`− ${money(totals.discount)}`}/><Info label={t('Tax')} value={money(totals.tax)}/><Info label={t('Shipping')} value={money(numeric(form.shippingTotal))}/><div className="mt-3 flex items-baseline justify-between border-t border-slate-200 pt-3"><dt className="font-semibold text-slate-700">{t('Total')}</dt><dd className="text-2xl font-bold tracking-tight text-indigo-700">{money(totals.total)}</dd></div></dl></div>
    </div>
    <SellerFooter kind={kind} sellerProfile={sellerProfile} t={t}/>
  </section>;
}

function SellerFooter({ kind, sellerProfile, t }: { kind: Kind; sellerProfile: DocumentSellerProfile | Partial<DocumentSellerProfile> | null | undefined; t: ReturnType<typeof useTranslation> }) {
  if (!sellerProfile) return null;
  return <footer className="border-t border-slate-200 bg-slate-50/80 px-6 py-5 text-sm sm:px-8"><div className="flex items-start gap-3">{sellerProfile.logoUrl ? <img src={sellerProfile.logoUrl} alt={sellerProfile.companyName || t('Company logo')} className="h-12 w-12 rounded-lg bg-white object-contain p-1 ring-1 ring-slate-200"/> : null}<div><h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">{t('Seller')}</h3><p className="mt-1 font-semibold">{sellerProfile.companyName || '—'}</p><p className="text-slate-600">{[sellerProfile.address, sellerProfile.countryRegion].filter(Boolean).join(', ') || '—'}</p></div></div><div className="mt-3 grid gap-x-4 gap-y-1 text-xs text-slate-600 sm:grid-cols-2"><span>{t('Tax ID')}: {sellerProfile.taxId || '—'}</span><span>{t('Legal form')}: {sellerProfile.legalForm || '—'}</span><span>{t('Legal representative')}: {sellerProfile.legalRepresentative || '—'}</span><span>{t('Phone number')}: {sellerProfile.phoneNumber || '—'}</span></div>{kind === 'invoices' && <div className="mt-3 border-t border-slate-200 pt-3 text-xs text-slate-600"><p className="font-semibold text-slate-700">{t('Bank details')}</p><p>{sellerProfile.bankOpeningBank || '—'} · {sellerProfile.bankBranch || '—'}</p><p>{t('Account')}: {sellerProfile.bankAccountNumber || '—'} · {t('Bank code')}: {sellerProfile.bankCode || '—'}</p></div>}</footer>;
}
function Info({ label, value }: { label: string; value: string }) { return <div className="flex justify-between gap-4"><dt className="text-slate-500">{label}</dt><dd>{value}</dd></div>; }
function Detail({ kind, item, detail, onSend, onDownloadPdf, downloadingPdf, canWrite }: { kind: Kind; item: Quote | Invoice; detail: Record<string, unknown>; onSend: () => void; onDownloadPdf: () => void; downloadingPdf: boolean; canWrite: boolean }) {
  const t = useTranslation();
  const document = (detail.quote ?? detail.invoice) as Record<string, unknown>;
  const seller = detail.sellerProfile as Partial<DocumentSellerProfile> | null | undefined;
  const lines = (detail.lines as Array<Record<string, unknown>> | undefined) ?? [];
  const deliveries = (detail.deliveries as Array<Record<string, unknown>> | undefined) ?? [];
  const status = String(item.status);
  return <div><div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[.16em] text-[var(--muted-foreground)]">{item.creationMode}</p><h2 className="mt-1 text-2xl font-semibold">{kind === 'quotes' ? (item as Quote).quoteNumber : (item as Invoice).invoiceNumber}</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">{status} · {item.currency}</p></div><div className="flex flex-wrap justify-end gap-2"><button type="button" disabled={kind !== 'invoices' || downloadingPdf} onClick={onDownloadPdf} className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] px-3 py-2 text-sm disabled:opacity-40"><Download size={15}/>{downloadingPdf ? t('Preparing PDF…') : t('Download PDF')}</button><button type="button" disabled={!canWrite || (kind === 'quotes' ? !['DRAFT','READY','AWAITING_APPROVAL'].includes(status) : !['DRAFT','ISSUED','SENT'].includes(status))} onClick={onSend} className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] px-3 py-2 text-sm disabled:opacity-40"><Send size={15}/>{t('Send')}</button></div></div><dl className="mt-6 grid grid-cols-2 gap-4 text-sm"><Info label={t('Customer record')} value={String(document.customerRecordId ?? '—')}/><Info label={t('Total')} value={`${String(document.grandTotal ?? '0')} ${item.currency}`}/>{kind === 'invoices' ? <Info label={t('Amount due')} value={`${String(document.amountDue ?? '0')} ${item.currency}`}/> : <Info label={t('Version')} value={String(document.currentVersion ?? '1')}/>}<Info label={t('Delivery attempts')} value={String(deliveries.length)}/></dl><div className="mt-6 border-t border-[var(--border)] pt-5"><h3 className="text-sm font-medium">{t('Line items')}</h3><div className="mt-3 space-y-2">{lines.length === 0 ? <p className="text-sm text-[var(--muted-foreground)]">{t('No line items.')}</p> : lines.map((line, index) => <div key={String(line.id ?? index)} className="flex justify-between gap-3 text-sm"><span>{String(line.productName ?? '—')} × {String(line.quantity ?? '0')}</span><strong>{String(line.lineTotal ?? '0')}</strong></div>)}</div></div><SellerFooter kind={kind} sellerProfile={seller} t={t}/></div>;
}
