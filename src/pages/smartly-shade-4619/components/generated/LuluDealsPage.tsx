...
function Sidebar() {
  return <aside className="hidden w-[220px] shrink-0 flex-col border-r border-border bg-[var(--sidebar)] px-3 py-5 lg:flex">
    ...
    <nav className="space-y-1 text-sm" aria-label="Primary navigation">
      {[['Dashboard', Gauge], ['AI Insights', Sparkles], ['AI Assistant', Bot], ['AI Agents', Zap]].map(([label, Icon]) => <button key={label as string} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-foreground transition hover:bg-secondary hover:text-foreground"><Icon size={17} /><span>{label as string}</span></button>)}
      <div className="pt-4"><div className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">CRM</div>
        {[['Overview', LayoutGrid], ['Contacts', Users], ['Companies', BriefcaseBusiness], ['Leads', Target], ['Deals', FolderKanban], ['Pipeline', BarChart3], ['Activities', Activity], ['Tasks', CheckCircle2]].map(([label, Icon]) => <button key={label as string} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[13px] transition ${label === 'Deals' ? 'bg-secondary/15 font-medium text-foreground' : 'text-foreground hover:bg-secondary hover:text-foreground'}`}><Icon size={16} /><span>{label as string}</span>{label === 'Deals' && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary text-primary-foreground" />}</button>)}
      </div>
      <div className="pt-4"><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-foreground hover:bg-secondary hover:text-foreground"><Settings size={17} /><span>Settings</span></button></div>
    </nav>
