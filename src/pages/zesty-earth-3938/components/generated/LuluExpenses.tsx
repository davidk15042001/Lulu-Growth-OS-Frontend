...
export const LuluExpenses = () => {
  ...
  const detailRecord = detail ? liveRecords.find((record) => record.id === detail) : undefined;
  return <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-sans">{liveError && <div className="mx-6 mt-4 rounded-lg border border-chart-5/30 bg-chart-5/5 px-4 py-3 text-sm text-chart-5 lg:ml-[238px]">{liveError}</div>}
    ...
    <aside className={`fixed inset-y-0 left-0 z-30 flex w-[238px] flex-col bg-[var(--sidebar)] text-foreground transition-transform lg:translate-x-0 ${mobileNav ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
    ...
    <div className="min-w-0 lg:ml-[238px]">


      <header className="flex min-h-[76px] flex-wrap items-center justify-between gap-3 border-b border-border bg-card px-4 sm:px-6 lg:px-9"><button className="rounded-md border border-border p-2 text-foreground lg:hidden" onClick={() => setMobileNav(true)} aria-label="Open navigation"><Menu size={17} /></button><div><div className="mb-1 flex items-center gap-2 text-[11px] text-muted-foreground"><span>Finance</span><ChevronRight size={12} /><span className="text-foreground">Expenses</span></div><h1 className="text-[22px] font-bold tracking-[-.03em]">Expenses</h1></div><div className="flex w-full flex-wrap items-center gap-2 sm:w-auto"><button className="flex h-9 items-center gap-2 rounded-md bg-primary px-4 text-xs font-semibold text-primary-foreground shadow-sm hover:bg-primary"><Plus size={15} />Record Expense</button><button className="flex h-9 items-center gap-2 rounded-md border border-border px-3 text-xs font-medium hover:bg-card"><Upload size={14} />Import</button><button className="flex h-9 items-center gap-2 rounded-md border border-border px-3 text-xs font-medium hover:bg-card"><Download size={14} />Export</button><button className="grid h-9 w-9 place-items-center rounded-md border border-border hover:bg-card"><MoreHorizontal size={17} /></button></div></header>
      <div className="px-4 py-6 sm:px-6 lg:px-9"><div className="mb-5 flex flex-col items-start justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-[13px] text-muted-foreground">Track, categorize and understand your business expenses.</p><p className="mt-1 text-[11px] text-muted-foreground">Live sync status is shown when connected account data is available.</p></div><div className="flex items-center gap-2"><button className="flex h-9 items-center gap-3 rounded-md border border-border bg-card px-3 text-xs font-semibold shadow-sm"><CalendarDays size={14} className="text-muted-foreground" /><span>{period}</span><ChevronDown size={13} /></button><button className="flex h-9 items-center gap-2 rounded-md border border-border bg-card px-3 text-xs font-medium shadow-sm">vs Previous Period<ChevronDown size={13} /></button></div></div>
      <section className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-7">{[['Total Expenses', '—', 'Live expense records', ''], ['Recurring Expenses', '—', 'Live expense records', ''], ['One-Time Expenses', '—', 'Live expense records', ''], ['Outstanding', '—', 'Live expense records', ''], ['Reimbursable', '—', 'Live expense records', ''], ['Average Expense', '—', 'Live expense records', ''], ['Expense Growth', '—', 'Live comparison data', '']].map(card => <article key={card[0]} className="rounded-lg border border-border bg-card p-4 shadow-[0_1px_2px_rgba(0,0,0,.03)]"><p className="text-[10px] font-semibold text-muted-foreground">{card[0]}</p><strong className="mt-2 block text-[19px] tracking-[-.03em]">{card[1]}</strong><p className="mt-1 text-[10px] text-muted-foreground">{card[2]}</p><span className={`mt-3 inline-flex items-center gap-1 text-[10px] font-semibold ${card[3].startsWith('↓') ? 'text-chart-4' : 'text-chart-5'}`}>{card[3] === 'trend' ? <span className="flex items-end gap-0.5"><i className="h-2 w-1 bg-primary text-primary-foreground" /><i className="h-3 w-1 bg-primary text-primary-foreground" /><i className="h-4 w-1 bg-primary text-primary-foreground" /><i className="h-5 w-1 bg-primary text-primary-foreground" /></span> : card[3]} {card[3] !== 'trend' && <span className="font-normal text-muted-foreground">vs prev.</span>}</span></article>)}</section>
      <section className="mb-5 rounded-lg border border-border bg-card p-4"><div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center"><div className="relative flex-1"><Search className="absolute left-3 top-2.5 text-muted-foreground" size={15} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search expenses..." className="h-9 w-full rounded-md border border-border pl-9 pr-9 text-xs outline-none focus:border-border" />{query && <button onClick={() => setQuery('')} className="absolute right-3 top-2.5 text-foreground"><X size={14} /></button>}</div><button className="flex h-9 items-center gap-2 rounded-md border border-border px-3 text-xs font-semibold"><Tune size={14} />Filters <span className="grid h-4 w-4 place-items-center rounded-full bg-secondary text-[10px] text-foreground">Live</span></button><button className="h-9 rounded-md border border-border px-3 text-xs font-medium">Save Filter</button></div><p className="mt-2 pl-1 text-[10px] text-muted-foreground">Search by Expense ID, Description, Vendor, Category, Amount, Reference</p><div className="flex items-center gap-2 overflow-x-auto pb-1">{filters.map(filter => <button key={filter} className="flex shrink-0 items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-[10px] font-medium text-foreground hover:border-border hover:text-foreground">{filter}<ChevronDown size={11} /></button>)}<button className="shrink-0 text-[11px] font-semibold text-foreground hover:text-foreground">Clear filters</button></div></section>
      {selected.length > 0 && <div className="mb-2 flex flex-wrap items-center gap-3 rounded-lg border border-border bg-secondary px-4 py-2 text-xs"><strong>{selected.length} selected</strong><button>Categorize</button><button>Assign Account</button><button>Mark as Paid</button><button>Mark reimbursable</button><button>Export</button><button className="text-chart-5">Delete</button></div>}
      <section className="overflow-hidden rounded-lg border border-border bg-card shadow-[0_1px_2px_rgba(0,0,0,.03)]"><div className="flex items-center justify-between border-b border-border px-5 py-3"><div><h2 className="text-sm font-bold">All expenses</h2><p className="text-[10px] text-muted-foreground">{liveExpenses.length} live records</p></div><button className="flex items-center gap-2 text-xs font-medium text-foreground"><SlidersHorizontal size={14} />Columns</button></div><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-[11px]"><thead className="bg-secondary text-[10px] uppercase tracking-wider text-muted-foreground"><tr><th className="w-10 px-4 py-3"><input type="checkbox" aria-label="Select all expenses" onChange={e => setSelected(e.target.checked ? expenses.map(x => x[0]) : [])} /></th>{['Expense ID', 'Date', 'Vendor', 'Category', 'Amount', 'Currency', 'Status', 'Account', 'Reimbursement', 'Source', 'Created', 'Actions'].map(h => <th key={h} className={`px-3 py-3 font-semibold ${h === 'Amount' ? 'text-right' : ''}`}>{h}<ChevronDown size={10} className="ml-1 inline" /></th>)}</tr></thead><tbody className="divide-y divide-border">{(liveLoading ? [] : liveExpenses).map(row => {
                ...
                })}</tbody></table></div><div className="flex items-center justify-between border-t border-border px-5 py-3 text-[11px] text-muted-foreground"><span>Showing {liveExpenses.length} live expenses</span><div className="text-[11px] text-muted-foreground">Additional pages appear when live records are available.</div></div></section>
