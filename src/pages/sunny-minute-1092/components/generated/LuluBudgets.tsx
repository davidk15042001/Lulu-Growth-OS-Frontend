...
export function LuluBudgets() {
  ...
  const { items: budgetRecords, loading: budgetsLoading, error: budgetsError, refresh: refreshBudgets } = useLiveRecords('ad_budgets');
  const getBudgetField = (record: typeof budgetRecords[number], key: string) => String((record as unknown as Record<string, unknown>)[key] ?? record.data?.[key] ?? '');
  ...
  if (budgetsLoading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live budgets…</main>;
  if (budgetsError) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-destructive">{budgetsError}</main>;
  if (budgetRecords.length === 0) return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-3xl rounded-2xl border border-dashed border-border bg-card p-8 text-center"><DollarSign className="mx-auto mb-4 text-muted-foreground" size={28} /><h1 className="text-2xl font-semibold">Budgets</h1><p className="mt-3 text-sm text-muted-foreground">No live advertising budget data is available yet. Connect a verified platform before managing budgets.</p><button type="button" onClick={() => void refreshBudgets()} className="mt-5 inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm hover:bg-secondary"><RefreshCw size={15} /> Refresh</button></div></main>;
  return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-6xl"><header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Advertising / Budgets</p><h1 className="mt-2 text-3xl font-bold">Budgets</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Verified budget records from the connected workspace. Spend, utilization and forecasts appear only when supported by backend data.</p></div><button type="button" onClick={() => void refreshBudgets()} className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm hover:bg-secondary"><RefreshCw size={15} /> Refresh</button></header><section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="border-b border-border p-4"><label className="flex items-center gap-2 rounded-lg border border-border bg-secondary px-3 py-2"><Search size={15} className="text-muted-foreground" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search live budget records" className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" /></label></div><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Budget</th><th className="px-4 py-3">Platform</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Budget</th><th className="px-4 py-3">Spend</th><th className="px-4 py-3">Utilization</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{budgetRecords.filter(record => !query || `${record.name} ${record.description ?? ''} ${record.status} ${getBudgetField(record, 'platform')}`.toLowerCase().includes(query.toLowerCase())).map(record => <tr key={record.id}><td className="px-4 py-3 font-medium">{record.name}</td><td className="px-4 py-3 text-muted-foreground">{getBudgetField(record, 'platform') || '—'}</td><td className="px-4 py-3">{record.status}</td><td className="px-4 py-3">{getBudgetField(record, 'budget') || record.valueAmount || '—'} {record.currency ?? ''}</td><td className="px-4 py-3">{getBudgetField(record, 'spend') || '—'}</td><td className="px-4 py-3">{getBudgetField(record, 'utilization') || '—'}</td><td className="px-4 py-3 text-muted-foreground">{new Date(record.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section></div></main>;
