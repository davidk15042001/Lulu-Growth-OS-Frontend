...
export function FinanceOverview() {
  ...
  const formatMoney = (amount: number) => new Intl.NumberFormat(undefined, { style: 'currency', currency: liveAccounts[0]?.currency || liveInvoices[0]?.currency || 'EUR', maximumFractionDigits: 0 }).format(amount);
  const liveRevenue = liveInvoices.reduce((sum, record) => sum + Number(record.valueAmount || record.data?.amount || 0), 0);
  const liveExpensesTotal = liveExpenses.reduce((sum, record) => sum + Number(record.valueAmount || record.data?.amount || 0), 0);
  const liveNetIncome = liveRevenue - liveExpensesTotal;
  const liveKpis: string[][] = hasLiveFinanceData ? [['Revenue', formatMoney(liveRevenue), 'Live records', 'Current period', 'Observed', 'From invoices', 'green'], ['Expenses', formatMoney(liveExpensesTotal), 'Live records', 'Current period', 'Observed', 'From expenses', 'amber'], ['Net Income', formatMoney(liveNetIncome), 'Calculated', 'Revenue minus expenses', 'Calculated', 'Derived from live records', liveNetIncome >= 0 ? 'green' : 'red'], ['Cash Flow', formatMoney(liveAccounts.reduce((sum, record) => sum + Number(record.valueAmount || 0), 0)), 'Live records', 'Connected accounts', 'Observed', 'Account balances', 'blue']] : [];
  const metrics = ['Revenue', 'Expenses', 'Net Income', 'Cash Flow'];
  if (financeLoading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live finance data…</main>;
  if (financeError) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-destructive">{financeError}</main>;
  if (!hasLiveFinanceData) return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-3xl rounded-2xl border border-dashed border-border bg-card p-8 text-center"><CircleDollarSign className="mx-auto mb-4 text-muted-foreground" size={28} /><h1 className="text-2xl font-semibold">Finance Overview</h1><p className="mt-3 text-sm text-muted-foreground">No live finance data is available yet. Connect a finance platform or add verified records before reviewing financial metrics.</p></div></main>;
  const financeRows = [...liveInvoices, ...liveExpenses, ...liveAccounts].filter(record => !query || `${record.name} ${record.description ?? ''} ${record.status}`.toLowerCase().includes(query.toLowerCase()));
  return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-6xl"><header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Finance</p><h1 className="mt-2 text-3xl font-bold">Finance Overview</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Verified financial records from the connected workspace. Totals are calculated only from records returned by the backend.</p></div><div className="flex gap-2"><button type="button" onClick={() => window.location.reload()} className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm hover:bg-secondary"><RefreshCw size={15} /> Refresh</button></div></header><section className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{liveKpis.map(([name, value, source]) => <article key={name} className="rounded-xl border border-border bg-card p-4"><p className="text-xs text-muted-foreground">{name}</p><p className="mt-2 text-2xl font-semibold">{value}</p><p className="mt-1 text-xs text-muted-foreground">{source}</p></article>)}</section><section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="border-b border-border p-4"><label className="flex items-center gap-2 rounded-lg border border-border bg-secondary px-3 py-2"><Search size={15} className="text-muted-foreground" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search live finance records" className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" /></label></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Record</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{financeRows.map(record => <tr key={record.id}><td className="px-4 py-3 font-medium">{record.name}</td><td className="px-4 py-3 text-muted-foreground">{record.resourceType}</td><td className="px-4 py-3">{record.status}</td><td className="px-4 py-3">{record.valueAmount ?? '—'} {record.currency ?? ''}</td><td className="px-4 py-3 text-muted-foreground">{new Date(record.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section></div></main>;
