...
export function ProductsIntelligence() {
  const { items, loading, error } = useLiveRecords('ecommerce_products');
  return <div className="min-h-screen bg-[var(--background)] text-foreground"><div className="flex min-h-screen"><Sidebar /><main className="min-w-0 flex-1"><header className="border-b border-border bg-card px-5 py-6 sm:px-8 lg:px-10"><div className="mx-auto max-w-[1440px]"><div className="mb-5 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span>Intelligence</span><span>/</span><strong className="text-foreground">Products</strong></div><div className="flex flex-wrap items-end justify-between gap-5"><div><h1 className="text-3xl font-black tracking-[-.04em] sm:text-4xl">Products</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Live product intelligence from connected workspace sources.</p></div><button className="btn-primary"><Bot size={15} />Ask Lulu AI</button></div></div></header><div className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-10"><section className="mt-10 flex min-h-[520px] items-center justify-center rounded-xl border border-dashed border-border bg-card p-10 text-center"><div><PackageSearch className="mx-auto text-muted-foreground" size={34} />{loading ? <h2 className="mt-4 text-xl font-semibold">Loading live product data…</h2> : error ? <h2 className="mt-4 text-xl font-semibold">Product data could not be loaded</h2> : <h2 className="mt-4 text-xl font-semibold">No product data available yet</h2>}<p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">{items.length ? `${items.length} live product records are available.` : 'Connect an ecommerce platform to populate products, categories, demand and profitability. No example metrics are displayed.'}</p></div></section></div></main></div></div>;
}
