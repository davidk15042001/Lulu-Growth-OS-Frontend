...
function Sidebar() {
  return <aside className="sidebar"><div className="brand"><span className="brand-mark">L</span><strong>LULU <em>AI</em></strong></div><div className="workspace"><span className="workspace-mark">N</span><span><strong>Northstar Commerce</strong><small>Enterprise workspace</small></span><ChevronDown size={14} /></div><nav aria-label="Primary navigation"><p className="nav-label">INTELLIGENCE</p><a><BarChart3 size={16} /> Intelligence Overview</a><a><Activity size={16} /> Executive Overview</a><a><FileBarChart size={16} /> Business Intelligence <ChevronRight className="nav-caret" size={14} /></a><a className="nav-open"><TrendingUp size={16} /> Analytics <ChevronDown className="nav-caret" size={14} /></a><div className="subnav">{navItems.map(item => <a key={item} className={item === 'Reports' ? 'active' : ''}><span>{item}</span>{item === 'Reports' && <i />}</a>)}</div><a><Sparkles size={16} /> AI Intelligence <ChevronRight className="nav-caret" size={14} /></a></nav><div className="sidebar-bottom"><a><Database size={16} /> Settings</a><div className="user"><span className="avatar">AS</span><span><strong>Alex Smith</strong><small>Admin</small></span><MoreHorizontal size={16} /></div></div></aside>;
}
function Status({
  status
}: {
