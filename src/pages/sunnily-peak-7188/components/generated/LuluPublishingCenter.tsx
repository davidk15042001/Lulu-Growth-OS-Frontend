...
import { Activity, AlertTriangle, ArrowRight, Bell, Bot, Check, ChevronDown, ChevronRight, Clock3, FileCheck2, Filter, HelpCircle, History, LayoutGrid, Link2, Megaphone, MoreHorizontal, Pause, Play, Plus, Search, Send, Settings2, ShieldCheck, Sparkles, Tag, TrendingUp, Upload, Users, X, Zap } from 'lucide-react';
type Risk = 'High' | 'Medium' | 'Low';
type Platform = 'Google' | 'Meta' | 'LinkedIn' | 'TikTok';
const approvals: Array<Record<string, any>> = [];
const operations = [['Brand Awareness — DE', 'Google', 'Create Campaign', 'Publishing', 'Lulu AI', 'Autonomous', '2 min ago'], ['Retargeting — DACH', 'Meta', 'Budget Update', 'Published', 'Lulu AI', 'Wallet policy', '18 min ago'], ['B2B Enterprise', 'LinkedIn', 'Create Campaign', 'Failed', 'Lulu AI', 'Autonomous', '1 hr ago'], ['Summer Collection', 'TikTok', 'Create Campaign', 'Scheduled', 'Lulu AI', 'Wallet policy', 'Not available, 09:00'], ['Q4 Performance Max', 'Google', 'Activate Campaign', 'Published', 'Lulu AI', 'Autonomous', 'Yesterday']];
const history: any[][] = [];
...
export function LuluPublishingCenter() {
  ...
  return <div className="lulu-app">
    ...
    <main className="main">
      <header className="page-header"><div><div className="crumb">Advertising <ChevronRight size={13} /> <strong>Autonomous Publishing</strong></div><h1>Autonomous Publishing <Upload size={20} /></h1><p>Lulu validates, schedules, publishes and optimizes campaigns across connected platforms without human approval. Prepaid budget is the only authorization boundary.</p></div><div className="header-actions"><span className="inline-flex items-center rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground">Always-on execution</span><button className="text-link">View Activity <ArrowRight size={14} /></button></div></header>
      <section className="kpis">{[['Human Approvals', '0', 'Fully autonomous', 'green'], ['Ready to Publish', '2', '2 campaigns validated', 'green'], ['Scheduled', '3', 'Next execution queued', 'blue'], ['Publishing', '1', 'Google Ads · In progress', 'violet'], ['Published Today', '7', 'Across 3 platforms', 'green'], ['Failed', '1', 'Auto-recovery active', 'red']].map(([label, num, sub, color]) => <article className={`kpi ${color}`} key={label}><strong>{num}</strong><div><h3>{label}</h3><p>{sub}</p></div></article>)}</section>
      <div className="workspace"><div className="left-col">
        <section><SectionTitle title="Autonomous Execution Queue" count="0 blocked" /><div className="filters"><button>All Platforms <ChevronDown size={13} /></button><button>All Types <ChevronDown size={13} /></button><button>Risk <ChevronDown size={13} /></button><button className="amber-outline" onClick={() => notify('Execution policy is active')}><Zap size={14} /> Autopilot Active</button></div>
        ...
        <section><SectionTitle title="Publishing Operations" /><div className="table-tools"><button><Filter size={14} /> Filter</button><label><Search size={14} /><input placeholder="Search operations..." value={query} onChange={e => setQuery(e.target.value)} /></label></div><div className="data-table"><div className="table-head"><span>Campaign</span><span>Platform</span><span>Operation</span><span>Status</span><span>Initiated By</span><span>Execution Policy</span><span>Updated</span><span /></div>{operations.filter(row => row.join(' ').toLowerCase().includes(query.toLowerCase())).map(row => <div className="table-row" key={row[0]}>{row.map((cell, i) => i === 1 ? <span key={cell}><PlatformBadge platform={cell === 'Google' ? 'Google' : cell as Platform} /></span> : i === 3 ? <Status key={cell} value={cell} /> : <span key={`${cell}-${i}`}>{cell}</span>)}<button className="view">View <ArrowRight size={12} /></button></div>)}</div></section>
        ...
        <article className="empty"><div><h3>Nothing Is Blocked</h3><p>Lulu is executing every eligible advertising operation automatically within the prepaid wallet.</p></div><button className="primary"><Plus size={15} /> Create Campaign with AI</button></article>
      </div><aside className="right-col">
        
        <article className="side-card"><div className="side-head"><div><h2>Advertising Automation Policy</h2><p>Current Policy <span className="policy">Fully Autonomous</span></p></div></div><div className="policy-level"><p><span className="radio on" />Analyze, decide, publish and optimize continuously</p></div><h3 className="subhead">Financial Safeguards</h3><ul className="safeguards"><li>Spend ceiling: <strong>prepaid wallet balance</strong></li><li>New money: <strong>customer-authorized top-up only</strong></li><li>Connected platforms are selected automatically</li><li>Campaign, audience and allocation changes: autonomous</li><li>Every execution remains auditable</li></ul><small className="muted-note">Lulu cannot spend beyond funded ad spend. No downstream campaign action requires another approval.</small></article>
        <article className="side-card"><div className="side-head"><h2>AI Alerts <span className="alert-count">2</span></h2><Bell size={16} /></div><div className="alert amber"><h3><AlertTriangle size={14} /> DACH Retargeting — CPA Increased</h3><p>CPA increased — over the last 72 hours since budget change was published.</p><small>Observed · Meta Ads · Updated 4 min ago</small><div><button>Review</button></div></div><div className="alert blue"><h3><HelpCircle size={14} /> Brand Awareness — Learning Phase</h3><p>Campaign entered learning phase on Google Ads. Expect performance fluctuation for 7–10 days. No action required.</p><small>Observed · Google Ads · Updated 2 min ago</small><div><button>Dismiss</button></div></div></article>
    ...
    </main><div className="sticky"><span><strong>Autopilot active</strong> across connected advertising platforms</span><div><span>Execution is limited only by the prepaid wallet balance.</span></div><span>Human approvals: 0</span></div>{toast && <div className="toast"><Check size={15} /> {toast}</div>}
  </div>;
}
const styles = `
