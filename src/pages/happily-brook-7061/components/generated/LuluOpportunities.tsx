...
const navSections: Array<Record<string, any>> = [];
...
export const LuluOpportunities = () => {
  ...
  const { items: opportunityRecords, loading, error, refresh: refreshOpportunities } = useLiveRecords('growth_opportunities');
  const [refreshing, setRefreshing] = useState(false);
  const refresh = () => { void refreshOpportunities(); setRefreshing(true); window.setTimeout(() => setRefreshing(false), 700); };
  if (loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live opportunities…</main>;
  ...
  if (!loading && !error) return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-6xl"><header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Intelligence / Opportunities</p><h1 className="mt-2 text-3xl font-bold">Opportunities</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Verified growth opportunity records from connected workspace data. Potential, confidence and recommendations appear only when returned by the backend.</p></div><span className="inline-flex items-center rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground">Use Update in the navigation bar</span></header>{opportunityRecords.length === 0 ? <section className="rounded-2xl border border-dashed border-border bg-card p-10 text-center"><Lightbulb className="mx-auto mb-4 text-muted-foreground" size={30} /><h2 className="text-xl font-semibold">No verified opportunities yet</h2><p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">Connect and analyze verified workspace data before reviewing opportunity potential or AI recommendations.</p></section> : <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Opportunity</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Stage</th><th className="px-4 py-3">Description</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{opportunityRecords.map(record => <tr key={record.id}><td className="px-4 py-3 font-medium">{record.name}</td><td className="px-4 py-3">{record.status}</td><td className="px-4 py-3 text-muted-foreground">{record.stage ?? '—'}</td><td className="max-w-md px-4 py-3 text-muted-foreground">{record.description ?? '—'}</td><td className="px-4 py-3 text-muted-foreground">{new Date(record.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>}</div></main>;
  return <div className="min-h-screen bg-[var(--background)] text-foreground font-sans selection:bg-secondary/30">
    ...
    <main className="lg:pl-64"><header className="sticky top-0 z-20 border-b border-border bg-[var(--card)]/90 px-5 py-4 backdrop-blur-xl md:px-8"><div className="flex items-center gap-3"><button className="text-foreground lg:hidden" onClick={() => setMobileNav(true)} aria-label="Open navigation"><Menu size={20} /></button><div className="hidden items-center gap-2 text-xs text-muted-foreground md:flex"><span>Intelligence</span><ChevronRight size={13} /><span>AI Intelligence</span><ChevronRight size={13} /><span className="text-foreground">Opportunities</span></div><div className="ml-auto flex items-center gap-3"><button className="hidden rounded-lg border border-border px-3 py-2 text-xs text-foreground hover:bg-secondary md:flex"><Bell size={14} className="mr-2" />Notifications</button><button className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-primary to-primary text-xs font-semibold text-primary-foreground">AR</button></div></div></header>
      <div className="mx-auto max-w-[1600px] px-5 py-8 md:px-8"><section className="mb-8 flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><p className="mb-3 flex items-center gap-2 text-xs font-medium text-foreground"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-primary text-primary-foreground" /> AI INTELLIGENCE</p><h1 className="text-4xl font-bold tracking-tight text-foreground md:text-5xl">Opportunities</h1><p className="mt-3 max-w-xl text-sm text-muted-foreground">Discover the highest-value opportunities Lulu AI has identified across your business.</p></div><div className="flex flex-wrap gap-2"><span className="inline-flex items-center rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground">Use Update in the navigation bar</span><button className="hidden items-center gap-2 rounded-lg border border-border px-3 py-2.5 text-xs text-foreground hover:bg-secondary md:flex"><FileText size={14} /> Create Report</button></div></section>
