...
export function FinanceCustomers() {
  ...
  const visibleCustomers = useMemo(() => customers.filter(customer => customer.name.toLowerCase().includes(query.toLowerCase())), [query]);
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground">
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-[244px] flex-col bg-[var(--sidebar)] text-foreground lg:flex"><div className="flex h-16 items-center gap-3 border-b border-border px-6"><div className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-primary-foreground"><Sparkles size={17} /></div><strong className="text-lg tracking-tight text-foreground">LULU <span className="font-normal text-foreground">AI</span></strong></div><nav className="flex-1 overflow-y-auto px-4 py-5"><p className="mb-3 px-2 text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground">Workspace</p>{['Dashboard', 'AI', 'CRM', 'Marketing'].map(item => <button key={item} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-[12px] text-foreground hover:bg-secondary hover:text-foreground"><span className="h-1.5 w-1.5 rounded-full bg-muted" />{item}</button>)}<div className="mt-3"><div className="flex items-center gap-3 rounded-lg bg-secondary/20 px-3 py-2 text-[12px] font-semibold text-foreground"><CircleDollarSign size={15} className="text-foreground" />Finance <ChevronDown size={13} className="ml-auto" /></div><div className="ml-5 mt-1 space-y-0.5 border-l border-border pl-3">{['Overview', 'Invoices', 'Offers & Quotes', 'Income', 'Transactions', 'Payments', 'Expenses', 'Customers'].map(item => <button key={item} className={`flex w-full items-center rounded-md px-2 py-1.5 text-left text-[11px] ${item === 'Customers' ? 'bg-primary font-bold text-primary-foreground' : 'text-primary-foreground hover:text-primary-foreground'}`}>{item}</button>)}</div></div>{['HR', 'Projects', 'Settings'].map(item => <button key={item} className="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-[12px] text-foreground hover:bg-secondary hover:text-foreground"><span className="h-1.5 w-1.5 rounded-full bg-muted" />{item}</button>)}</nav><div className="border-t border-border p-4"><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-xs text-foreground hover:bg-secondary"><Settings2 size={15} />Workspace settings</button><div className="mt-3 flex items-center gap-3 rounded-lg bg-secondary p-3"><div className="grid h-7 w-7 place-items-center rounded-full bg-secondary text-xs font-bold text-foreground">AM</div><div><p className="text-xs font-semibold text-foreground">Alex Morgan</p><p className="text-[10px] text-muted-foreground">Admin</p></div><MoreHorizontal size={15} className="ml-auto" /></div></div></aside>
