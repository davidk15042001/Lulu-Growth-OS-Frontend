...
export const LuluExecutiveDashboard = () => {
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground" style={{
    ...
    <aside className={`${mobileNav ? 'flex' : 'hidden'} fixed inset-y-0 left-0 z-20 w-60 flex-col border-r border-border bg-[var(--sidebar)] p-5 lg:flex`}>
      ...
      <div className="space-y-3 border-t border-border pt-4"><div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-card text-xs font-semibold text-foreground">{accountInitial}</div><div><p className="text-sm font-medium text-foreground">{accountName}</p><p className="text-xs text-muted-foreground">Workspace: {selectedWorkspace?.companyName ?? "Workspace account"}</p></div><MoreHorizontal className="ml-auto text-muted-foreground" size={16} /></div><div className="flex items-center gap-2 rounded-lg border border-chart-4/20 bg-chart-4/5 px-3 py-2 text-xs text-chart-4"><span className="h-2 w-2 rounded-full bg-chart-4" /> AI Active <span className="ml-auto text-muted-foreground">Live</span></div></div>
    </aside>
