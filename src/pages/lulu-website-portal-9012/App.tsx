    ...
    <div className="min-h-screen bg-[var(--background)] text-foreground">
      ...
      <main data-lulu-scroll-container className="min-h-screen min-w-0 overflow-x-hidden">
        <div className="mx-auto max-w-[1400px] px-5 py-6 sm:px-8 sm:py-8">
          ...
          {(() => { const section = sectionContent[sectionParam]; return <section><div className="mb-3 flex items-end justify-between"><div><p className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">{section?.eyebrow ?? "Website"}</p><h2 className="mt-1 text-lg font-semibold tracking-tight text-foreground">{section?.title ?? `${activeSection} · ${content.shortLabel}`}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#4b5563]">{section?.description ?? "Wähle einen Website-Unterbereich aus der globalen Navigation."}</p></div><button type="button" className="rounded-lg p-2 text-muted-foreground hover:bg-secondary" aria-label="Website-Einstellungen"><Settings size={17} /></button></div><div className="grid gap-3 md:grid-cols-3">{(section?.cards ?? content.actions.map((action) => ({ title: action.title, text: action.description }))).map((card) => <article key={card.title} className="group rounded-xl border border-border bg-card p-4 text-left transition hover:-translate-y-px hover:border-primary/40"><div className="mb-5 flex items-center justify-between"><span className="grid h-9 w-9 place-items-center rounded-lg bg-secondary text-muted-foreground group-hover:text-foreground"><ArrowRight size={17} /></span><ArrowRight size={16} className="text-muted-foreground transition group-hover:translate-x-0.5 group-hover:text-foreground" /></div><h3 className="text-sm font-semibold text-foreground">{card.title}</h3><p className="mt-2 text-xs leading-5 text-muted-foreground">{card.text}</p></article>)}</div></section>; })()}
        </div>
      </main>
    </div>
  );
