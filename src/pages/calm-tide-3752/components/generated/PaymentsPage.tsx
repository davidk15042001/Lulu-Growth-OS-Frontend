...
const customers = [['Acme GmbH', '38', 'EUR 42,850', '1', '2', 'EUR 1,127', '10 Aug 2026'], ['Apex Solutions', '24', 'EUR 38,200', '0', '0', 'EUR 1,592', '08 Aug 2026'], ['Global Retail AG', '21', 'EUR 31,600', '3', '1', 'EUR 1,505', '09 Aug 2026'], ['Meridian Corp', '18', 'EUR 26,400', '0', '2', 'EUR 1,466', '09 Aug 2026'], ['TechNova Ltd', '16', 'EUR 22,100', '1', '0', 'EUR 1,381', '10 Aug 2026']];
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground" style={{
    ...
    <div className="flex min-h-screen">
      <aside className="hidden w-[220px] shrink-0 flex-col bg-[var(--sidebar)] text-foreground lg:flex">
        ...
        <nav className="flex-1 px-3 py-5"><p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground">Workspace</p><button className="mb-5 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-foreground hover:bg-secondary"><Bot size={17} /><span>AI Assistant</span></button><p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground">Modules</p>{['CRM', 'Marketing', 'Advertising'].map(item => <button key={item} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-foreground hover:bg-secondary"><Layers3 size={16} /><span>{item}</span></button>)}<div className="mt-5"><button className="flex w-full items-center justify-between rounded-lg bg-secondary px-3 py-2.5 text-sm font-semibold text-foreground"><span className="flex items-center gap-3"><CircleDollarSign size={17} className="text-foreground" /><span>Finance</span></span><ChevronDown size={15} /></button><div className="mt-1 border-l border-border pl-4">{navFinance.map(item => <button key={item} onClick={() => setActiveNav(item)} className={`flex w-full items-center rounded-md px-3 py-2 text-[12px] transition ${activeNav === item ? 'bg-primary font-semibold text-primary-foreground shadow-lg shadow-black/30' : 'text-primary-foreground hover:bg-secondary hover:text-primary-foreground'}`}><span>{item}</span></button>)}</div></div></nav>
        <div className="border-t border-border p-3"><button className="flex w-full items-center gap-3 rounded-lg p-2 text-left hover:bg-secondary"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-xs font-bold text-foreground">JD</span><span className="min-w-0 flex-1"><strong className="block truncate text-xs text-foreground">Jordan Davis</strong><span className="block text-[10px] text-muted-foreground">Administrator</span></span><Settings size={16} className="text-muted-foreground" /></button></div>
      </aside>
      <main className="min-w-0 flex-1 overflow-hidden"><div className="mx-auto max-w-[1700px] px-4 py-5 sm:px-6 lg:px-8">
