...
export function LuluCashFlow() {
 ...
 <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Cash Flow Risks" subtitle="No live risks detected yet · AI-analyzed"><div className="space-y-3">{([] as string[][]).map(r => <div key={r[0]} className="rounded-lg border border-border p-3"><div className="flex items-start gap-2"><AlertTriangle size={16} className="mt-0.5 text-foreground" /><div className="flex-1"><p className="text-xs font-bold">{r[0]}</p><p className="mt-1 text-[11px] text-muted-foreground">{r[1]} · Evidence: {r[3]} · Affected: {r[4]}</p></div><Badge tone={r[5] as Tone}>{r[2]}</Badge></div><div className="mt-3 flex gap-2"><button className="btn-secondary">Investigate</button><button className="btn-secondary">Ask Lulu AI</button></div></div>)}</div><p className="mt-4 text-[11px] italic text-muted-foreground">AI-detected · Does not constitute regulated financial advice.</p></Card><Card title="Cash Flow Anomalies" subtitle="No live anomalies detected yet"><div className="space-y-3">{([] as string[][]).map(r => <div key={r[0]} className="rounded-lg border border-border p-3"><p className="text-xs font-bold">{r[0]} <span className="ml-2 font-normal text-muted-foreground">{r[1]}</span></p><p className="mt-1 text-[11px] text-muted-foreground">{r[2]}</p><div className="mt-3 flex flex-wrap gap-2"><button className="btn-secondary">Investigate</button><button className="btn-secondary">Open Transactions</button><button className="btn-secondary">Ask Lulu AI</button></div></div>)}</div></Card></div>
