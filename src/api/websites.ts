...
export const websitesApi = {
  ...
  startAutomaticGeneration: (workspaceId: string, provider: 'wordpress' | 'webflow', targetMode: WebsiteGenerationTargetMode, language?: string, siteId?: string) => requestApi<{ site: WebsiteSite; job: WebsiteGenerationJob; reused: boolean }>({ path: `/workspaces/${workspaceId}/websites/automatic-generation`, method: 'POST', body: { provider, targetMode, ...(siteId ? { siteId } : {}), ...(language ? { language } : {}) }, timeoutMs: 120_000 }),
