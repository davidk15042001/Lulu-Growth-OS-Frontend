...
export function LuluWelcomeScreen() {
  ...
  const [tokenTest, setTokenTest] = useState<{ status: 'idle' | 'loading' | 'success' | 'error'; number: number | null; message: string }>({ status: 'idle', number: null, message: '' });
  async function runTokenTest() {
    if (!selectedWorkspace?.id) return;
    setTokenTest({ status: 'loading', number: null, message: '' });
    try {
      const result = await onboardingApi.aiTokenTest(selectedWorkspace.id);
  ...
  useEffect(() => {
    if (selectedWorkspace?.id) void runTokenTest();
      ...
      <main className="mx-auto flex w-full max-w-[1180px] flex-1 items-center px-5 py-4 sm:px-8 lg:px-10">
        <section aria-labelledby="welcome-title" className="grid w-full items-center gap-7 lg:grid-cols-[1.03fr_.97fr] lg:gap-10">
          <div className="text-center lg:text-left">
            ...
            <section aria-labelledby="token-test-title" className="welcome-enter welcome-delay-4 mx-auto mt-4 max-w-[620px] rounded-lg border border-[var(--border)] bg-[var(--card)] p-3 text-left lg:mx-0">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <h2 id="token-test-title" className="text-[12px] font-semibold text-[var(--foreground)]">ChatGPT token test</h2>
                  <p className="mt-0.5 text-[11px] leading-4 text-[var(--muted-foreground)]">The number below is generated by the configured AI token.</p>
                </div>
                <button type="button" onClick={() => void runTokenTest()} disabled={tokenTest.status === 'loading' || !selectedWorkspace?.id} className="shrink-0 rounded-md border border-[var(--border)] px-2.5 py-1.5 text-[11px] font-medium text-[var(--foreground)] transition hover:bg-[var(--secondary)] disabled:cursor-not-allowed disabled:opacity-50">{tokenTest.status === 'loading' ? 'Testing…' : 'Test again'}</button>
              </div>
              <div className="mt-3 flex items-center gap-3">
                <span aria-live="polite" className="grid h-12 w-12 shrink-0 place-items-center rounded-lg bg-[var(--primary)] text-xl font-bold text-[var(--primary-foreground)]">{tokenTest.status === 'loading' ? '…' : tokenTest.number ?? '—'}</span>
                <p className={`text-[11px] leading-4 ${tokenTest.status === 'error' ? 'text-[var(--destructive)]' : 'text-[var(--muted-foreground)]'}`}>{tokenTest.status === 'error' ? tokenTest.message : tokenTest.status === 'success' ? `Token response received. ${tokenTest.message}` : 'Waiting for the token test…'}</p>
              </div>
            </section>
