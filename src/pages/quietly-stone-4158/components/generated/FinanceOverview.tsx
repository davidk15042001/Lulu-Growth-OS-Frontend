...
const kpis = [['Revenue', '€2,847,320', '+12.4%', 'vs previous year', 'Observed', 'Updated 2m ago', 'green'], ['Expenses', '€1,923,180', '+8.1%', 'vs previous year', 'Observed', 'Updated 2m ago', 'amber'], ['Net Income', '€924,140', '+19.7%', 'vs previous year', 'Calculated', 'Based on recorded income', 'green'], ['Cash Balance', '€1,204,560', '4 accounts', 'consolidated balance', 'Observed', 'Synced 2m ago', 'violet'], ['Accounts Receivable', '€438,200', '€62,400', 'overdue', 'Observed', 'Updated 2m ago', 'amber'], ['Accounts Payable', '€187,340', '€23,100', 'overdue', 'Observed', 'Updated 2m ago', 'blue'], ['Overdue Invoices', '14 invoices', '€62,400', 'outstanding', 'Observed', 'Updated 2m ago', 'red'], ['Recurring Revenue', '€94,200/mo', '+6.3%', 'MRR', 'Calculated', 'Subscription data', 'violet']];
const accounts = [['Main Business Account', 'IBAN Bank', 'EUR', '€842,340', '2m ago'], ['Operations Account', 'IBAN Bank', 'EUR', '€213,780', '2m ago'], ['Stripe', 'Payment Provider', 'EUR', '€124,640', '5m ago'], ['USD Account', 'IBAN Bank', 'USD', '$29,840 ≈ €27,800', '10m ago']];
const aging = [['Current', '€287,400', '65%'], ['1–30 days', '€88,400', '20%'], ['31–60 days', '€38,200', '9%'], ['61–90 days', '€15,800', '4%'], ['90+ days', '€8,400', '2%']];
const activities = [['Invoice #INV-2847 paid', '€4,200 · Stripe', '2 min ago', 'green'], ['Expense recorded', 'Software · €1,840 · Auto-import', '14 min ago', 'amber'], ['AI Insight generated', 'Cash flow analysis', '1 hour ago', 'violet'], ['Invoice #INV-2846 sent', '€8,400', '2 hours ago', 'blue'], ['Account synchronized', 'Main Business Account', '2 hours ago', 'green'], ['Reconciliation completed', '184 transactions', 'Yesterday', 'slate']];
const transactions = [['Today', 'Client payment · INV-2847', 'Stripe', 'Services', '+€4,200', 'Settled'], ['Today', 'Adobe Creative Cloud', 'Main Account', 'Software', '−€249', 'Reconciled'], ['Yesterday', 'Payroll · March', 'Operations', 'Payroll', '−€42,800', 'Reconciled'], ['Mar 18', 'Client payment · INV-2842', 'Main Account', 'Services', '+€8,400', 'Reconciled'], ['Mar 17', 'Google Ads', 'Main Account', 'Marketing', '−€1,240', 'Pending'], ['Mar 16', 'Stripe payout', 'Stripe', 'Income', '+€12,480', 'Reconciled'], ['Mar 15', 'Office supplies', 'Operations', 'Operations', '−€480', 'Reconciled'], ['Mar 14', 'Client payment · INV-2834', 'Main Account', 'Products', '+€3,800', 'Reconciled']];
function Badge({
  children,
  tone = 'slate'
}: {
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground">
    ...
    <main className="lg:ml-[244px]"><div className="flex h-16 items-center justify-between border-b border-border bg-card px-5 lg:px-8"><div className="flex items-center gap-3"><PanelLeft size={18} className="text-muted-foreground lg:hidden" /><span className="text-xs text-muted-foreground">Business /</span><span className="text-xs font-semibold text-foreground">Finance</span></div><div className="flex items-center gap-3"><div className="hidden items-center gap-2 rounded-lg border border-border px-3 py-2 md:flex"><Search size={14} className="text-muted-foreground" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search finance..." className="w-40 bg-transparent text-xs outline-none placeholder:text-muted-foreground" /></div><button className="rounded-lg p-2 text-foreground hover:bg-secondary" aria-label="Notifications"><Activity size={17} /></button><div className="h-7 w-7 rounded-full bg-secondary text-center text-[10px] font-bold leading-7 text-muted-foreground">AM</div></div></div>
      <div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-8"><div className="mb-6 flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><p className="mb-3 text-xs font-medium text-muted-foreground">Finance <span className="mx-1">/</span> Finance Overview</p><h1 className="text-3xl font-bold tracking-[-0.04em] text-foreground">Finance Overview</h1><p className="mt-2 text-sm text-muted-foreground">Understand your financial performance, cash flow and financial position in one place.</p></div><div className="flex flex-wrap gap-2"><button className="inline-flex items-center gap-2 rounded-lg bg-primary px-3.5 py-2.5 text-xs font-bold text-primary-foreground shadow-sm hover:bg-primary"><Bot size={15} />Ask Lulu AI</button>{['Create Invoice', 'Create Offer', 'Record Expense', 'Record Income', 'Export'].map(item => <button key={item} className="rounded-lg border border-border bg-card px-3 py-2.5 text-xs font-semibold text-foreground hover:border-border hover:text-foreground">{item}</button>)}<button className="rounded-lg border border-border bg-card p-2.5 text-foreground hover:bg-card" aria-label="More actions"><MoreHorizontal size={16} /></button></div></div>
        ...
        <Card title="Financial Performance" subtitle="Monthly view · EUR · current period compared with previous year" action="View report"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div className="flex gap-1 rounded-lg bg-secondary p-1">{metrics.map(metric => <button key={metric} onClick={() => setActiveMetric(metric)} className={`rounded-md px-3 py-1.5 text-xs font-bold transition ${activeMetric === metric ? 'bg-card text-foreground shadow-sm' : 'text-foreground'}`}>{metric}</button>)}</div><div className="flex items-center gap-4 text-[11px] text-muted-foreground"><span className="flex items-center gap-2"><i className="h-0.5 w-5 bg-primary text-primary-foreground" />This year</span><span className="flex items-center gap-2"><i className="w-5 border-t border-dashed border-border" />Previous year</span></div></div><div className="rounded-lg bg-card/60 p-3"><svg viewBox="0 0 900 250" className="h-[260px] w-full" role="img" aria-label={`${activeMetric} performance line chart`}><g stroke="var(--border)" strokeDasharray="3 5"><path d="M30 35H880M30 90H880M30 145H880M30 200H880" /></g><path d="M30 194 C85 175 102 180 145 156 S205 167 250 128 S306 140 350 118 S405 133 450 99 S506 120 550 74 S605 96 650 67 S710 80 755 45 S823 57 880 27" fill="none" stroke="var(--chart-2)" strokeWidth="3" /><path d="M30 205 C85 190 110 198 145 177 S210 186 250 163 S305 174 350 151 S405 158 450 136 S505 145 550 116 S605 130 650 104 S710 119 755 89 S823 98 880 72" fill="none" stroke="var(--muted-foreground)" strokeWidth="2" strokeDasharray="7 6" /><path d="M30 194 C85 175 102 180 145 156 S205 167 250 128 S306 140 350 118 S405 133 450 99 S506 120 550 74 S605 96 650 67 S710 80 755 45 S823 57 880 27 L880 220 L30 220Z" fill="var(--chart-2)" opacity=".08" /><g fill="var(--border)" fontSize="10"><text x="30" y="240">Jan</text><text x="180" y="240">Mar</text><text x="330" y="240">May</text><text x="480" y="240">Jul</text><text x="630" y="240">Sep</text><text x="780" y="240">Nov</text></g></svg></div></Card>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3"><Card title="Income" subtitle="Income over time · YTD"><div className="flex items-end justify-between"><div><p className="text-2xl font-bold">€2,847,320</p><p className="mt-1 text-xs text-chart-4">↑ 12.4% growth</p></div><Sparkline bars /></div><div className="mt-5 grid grid-cols-3 gap-2 border-t border-border pt-4 text-xs"><div><p className="text-muted-foreground">Paid</p><strong>€2,409,120</strong></div><div><p className="text-muted-foreground">Outstanding</p><strong>€376,800</strong></div><div><p className="text-muted-foreground">Overdue</p><strong className="text-chart-5">€62,400</strong></div></div><p className="mt-4 text-[11px] font-semibold text-muted-foreground">Income by source</p><div className="mt-2 flex h-2 overflow-hidden rounded-full"><span className="w-[52%] bg-primary text-primary-foreground" /><span className="w-[25%] bg-primary text-primary-foreground" /><span className="w-[18%] bg-primary text-primary-foreground" /><span className="w-[5%] bg-muted" /></div><p className="mt-2 text-[11px] text-muted-foreground">Services 52% · Products 25% · Subscriptions 18% · Other 5%</p></Card><Card title="Expenses" subtitle="Expenses over time · YTD"><div className="flex items-end justify-between"><div><p className="text-2xl font-bold">€1,923,180</p><p className="mt-1 text-xs text-chart-1">↑ 8.1% growth</p></div><Sparkline color="var(--chart-1)" bars /></div><div className="mt-5 grid grid-cols-2 gap-3 border-t border-border pt-4 text-xs"><div><p className="text-muted-foreground">Recurring</p><strong>€812,400</strong></div><div><p className="text-muted-foreground">One-time</p><strong>€1,110,780</strong></div></div><p className="mt-4 text-[11px] font-semibold text-muted-foreground">Top categories</p><p className="mt-2 text-xs text-muted-foreground">Software <b>34%</b> · Payroll <b>28%</b> · Marketing <b>19%</b> · Operations <b>19%</b></p></Card><Card title="Net Income" subtitle="Profitability at a glance"><div className="rounded-lg bg-secondary p-3 text-xs text-foreground">€2,847,320 <span className="text-foreground">−</span> €1,923,180 <strong className="float-right">= €924,140</strong></div><div className="mt-5 flex items-end justify-between"><div><p className="text-2xl font-bold">€924,140</p><p className="mt-1 text-xs font-bold text-chart-4">↑ 19.7% vs previous year</p></div><Sparkline color="var(--chart-4)" /></div><p className="mt-5 text-[11px] text-muted-foreground"><Badge tone="violet">Calculated</Badge> Based on recorded income and expenses</p></Card></div>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Cash Flow" subtitle="Movement across all connected accounts"><div className="grid grid-cols-2 gap-y-3 text-xs sm:grid-cols-5"><div><p className="text-muted-foreground">Opening</p><strong>€980,200</strong></div><div><p className="text-muted-foreground">Inflows</p><strong className="text-chart-4">+€2,409,120</strong></div><div><p className="text-muted-foreground">Outflows</p><strong>−€2,184,760</strong></div><div><p className="text-muted-foreground">Net flow</p><strong className="text-chart-2">+€224,360</strong></div><div><p className="text-muted-foreground">Closing</p><strong>€1,204,560</strong></div></div><div className="mt-5"><Sparkline color="var(--chart-4)" bars /></div><p className="text-[11px] text-muted-foreground">Operating 82% · Investing 11% · Financing 7%</p></Card><Card title="Cash Position" subtitle="4 connected accounts · Values converted to EUR"><div className="divide-y divide-border">{accounts.map(([name, institution, currency, balance, sync]) => <div key={name} className="flex items-center gap-3 py-3 first:pt-0"><div className="grid h-8 w-8 place-items-center rounded-lg bg-secondary text-muted-foreground"><Landmark size={15} /></div><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{name}</p><p className="text-[11px] text-muted-foreground">{institution} · {currency} · Synced {sync}</p></div><strong className="text-xs">{balance}</strong><Badge tone="green">Active</Badge></div>)}</div></Card></div>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Accounts Receivable" subtitle="Total outstanding · €438,200"><div className="flex items-center justify-between"><p className="text-2xl font-bold">€438,200</p><Badge tone="amber">€62,400 overdue</Badge></div><div className="mt-5 h-3 overflow-hidden rounded-full bg-secondary"><span className="block h-full w-[65%] bg-primary text-primary-foreground" /></div><div className="mt-4 space-y-2">{aging.map(([label, value, percent]) => <div key={label} className="flex justify-between text-xs"><span className="text-muted-foreground">{label}</span><strong>{value} <span className="ml-2 font-normal text-muted-foreground">{percent}</span></strong></div>)}</div><div className="mt-5 flex flex-wrap gap-2"><button className="btn-secondary">View Invoices</button><button className="btn-secondary">View Overdue</button><button className="btn-primary">Ask Lulu AI</button></div></Card><Card title="Accounts Payable" subtitle="Total payable · €187,340"><p className="text-2xl font-bold">€187,340</p><div className="mt-5 grid grid-cols-3 gap-3 text-xs"><div><p className="text-muted-foreground">Due soon</p><strong>€94,200</strong></div><div><p className="text-muted-foreground">Overdue</p><strong className="text-chart-5">€23,100</strong></div><div><p className="text-muted-foreground">Paid period</p><strong>€1,735,840</strong></div></div><p className="mt-5 text-[11px] font-bold text-muted-foreground">Top vendors</p><div className="mt-2 space-y-2 text-xs"><div className="flex justify-between"><span>Vendor A</span><strong>€42,300</strong></div><div className="flex justify-between"><span>Vendor B</span><strong>€28,100</strong></div><div className="flex justify-between"><span>Vendor C</span><strong>€21,400</strong></div></div><div className="mt-5 flex gap-2"><button className="btn-secondary">View Expenses</button><button className="btn-secondary">View Vendors</button><button className="btn-primary">Ask Lulu AI</button></div></Card></div>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3"><Card title="Invoices" subtitle="284 total invoices"><div className="flex flex-wrap gap-1.5">{['Draft 12', 'Sent 48', 'Paid 196', 'Partially Paid 14', 'Overdue 14'].map(x => <Badge key={x} tone={x.includes('Overdue') ? 'red' : x.includes('Paid') ? 'green' : 'slate'}>{x}</Badge>)}</div><div className="mt-5 space-y-2 text-xs"><div className="flex justify-between"><span>Invoice value</span><strong>€2,847,320</strong></div><div className="flex justify-between"><span>Paid</span><strong>€2,409,120</strong></div><div className="flex justify-between"><span>Outstanding</span><strong>€376,800</strong></div><div className="flex justify-between"><span>Overdue</span><strong className="text-chart-5">€62,400</strong></div></div><p className="mt-4 text-[11px] text-muted-foreground">Invoice Status Distribution · Paid 69% · Sent 17% · Other 14%</p></Card><Card title="Payments" subtitle="892 total payments"><div className="grid grid-cols-2 gap-3 text-xs"><div><p className="text-muted-foreground">Successful</p><strong>864</strong></div><div><p className="text-muted-foreground">Pending</p><strong>18</strong></div><div><p className="text-muted-foreground">Failed</p><strong>6</strong></div><div><p className="text-muted-foreground">Refunded</p><strong>4</strong></div></div><div className="mt-5 border-t border-border pt-4 text-xs"><div className="flex justify-between"><span>Processing fees</span><strong>€12,840</strong></div><div className="mt-2 flex justify-between"><span>Net payout</span><strong>€2,396,280</strong></div></div><p className="mt-4 text-[11px] text-muted-foreground">Card 72% · Bank Transfer 21% · Other 7%</p></Card><Card title="Offers & Quotes" subtitle="Conversion pipeline"><div className="flex flex-wrap gap-1.5">{['Draft 8', 'Sent 24', 'Accepted 31', 'Rejected 6', 'Expired 3', 'Converted 28'].map(x => <Badge key={x} tone={x.includes('Accepted') ? 'green' : 'slate'}>{x}</Badge>)}</div><div className="mt-5 space-y-2 text-xs"><div className="flex justify-between"><span>Total offer value</span><strong>€1,240,800</strong></div><div className="flex justify-between"><span>Accepted</span><strong>€892,400</strong></div><div className="flex justify-between"><span>Conversion rate</span><strong className="text-foreground">62.4%</strong></div></div><div className="mt-5 flex gap-2"><button className="btn-primary">Create Offer</button><button className="btn-secondary">View Offers</button></div></Card></div>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3"><Card title="Recurring Revenue" subtitle="Calculated · Subscription data"><div className="flex justify-between"><div><p className="text-xs text-muted-foreground">MRR</p><p className="text-2xl font-bold">€94,200</p></div><div className="text-right"><p className="text-xs text-muted-foreground">ARR</p><p className="text-lg font-bold">€1,130,400</p></div></div><div className="mt-5 grid grid-cols-2 gap-2 text-xs"><span>New MRR <b className="text-foreground">+€8,400</b></span><span>Churned <b className="text-chart-5">−€2,100</b></span><span>Expansion <b className="text-chart-4">+€3,200</b></span><span>Contraction <b>−€800</b></span></div><p className="mt-5 rounded-lg bg-chart-4/10 p-3 text-xs font-bold text-chart-4">Net MRR growth +€8,700 · +10.2%</p></Card><Card title="Budget Performance" action="View Budgets"><div className="space-y-3 text-xs"><div className="grid grid-cols-4 gap-2 border-b pb-2 font-semibold text-muted-foreground"><span>Category</span><span>Budget</span><span>Actual</span><span>Variance</span></div>{[['Marketing', '€20,000', '€17,800', '−€2,200 ✓'], ['Software', '€18,000', '€19,400', '+€1,400 ⚠'], ['Operations', '€35,000', '€32,100', '−€2,900 ✓'], ['Sales', '€25,000', '€24,200', '−€800 ✓']].map(([a, b, c, d]) => <div key={a} className="grid grid-cols-4 gap-2"><span className="font-semibold">{a}</span><span>{b}</span><span>{c}</span><span className={d.includes('⚠') ? 'font-bold text-chart-5' : 'font-bold text-foreground'}>{d}</span></div>)}</div></Card><Card title="Financial Planning" subtitle="Next 6 months"><Badge tone="violet">AI Forecast</Badge><div className="mt-4 space-y-2 text-xs"><div className="flex justify-between"><span>Forecast Revenue</span><strong>€3,180,000</strong></div><div className="flex justify-between"><span>Forecast Expenses</span><strong>€2,040,000</strong></div><div className="flex justify-between"><span>Forecast Net Income</span><strong>€1,140,000</strong></div><div className="flex justify-between"><span>Forecast Cash Balance</span><strong>€1,420,000</strong></div></div><div className="mt-4 flex items-center justify-between rounded-lg bg-secondary p-3 text-xs"><span>Confidence</span><strong className="text-foreground">78%</strong></div><p className="mt-3 text-[11px] italic text-muted-foreground">AI Forecast — Never guaranteed outcomes</p></Card></div>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Financial Anomalies" subtitle="Detected across connected data"><div className="space-y-3">{[['Software Expense Increase', '+34% vs previous period · Detected today', 'High severity', 'red'], ['Unusual Refund Activity', '3 large refunds detected · €8,400 total', 'Medium severity', 'amber'], ['Cash Balance Decline', '−12% week over week', 'Low severity', 'blue']].map(([title, desc, severity, tone]) => <div key={title} className="flex gap-3 rounded-lg border border-border p-3"><div className="mt-0.5 text-chart-1"><AlertTriangle size={17} /></div><div className="flex-1"><p className="text-xs font-bold">{title}</p><p className="mt-1 text-[11px] text-muted-foreground">{desc}</p></div><div className="text-right"><Badge tone={tone as Tone}>{severity}</Badge><button className="mt-2 block text-[11px] font-bold text-foreground">Investigate</button></div></div>)}</div></Card><Card title="Attention Required" subtitle="4 items need your review"><div className="space-y-3 text-xs">{[['🔴', '14 Overdue Invoices', '€62,400 outstanding past due date', 'View Invoices'], ['🟡', 'Budget Exceeded', 'Software budget exceeded by €1,400', 'View Budgets'], ['🟡', 'Failed Payments', '6 payment failures this period', 'View Payments'], ['🔵', 'Account Sync Pending', 'USD Account last synced 10 min ago', 'View Accounts']].map(([icon, title, desc, action]) => <div key={title} className="flex items-center gap-3 border-b border-border pb-3 last:border-0 last:pb-0"><span className="text-sm">{icon}</span><div className="flex-1"><strong>{title}</strong><p className="mt-1 text-[11px] text-muted-foreground">{desc}</p></div><button className="font-bold text-foreground">{action}</button></div>)}</div></Card></div>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3"><Card title="Financial Accounts" subtitle="Manage and reconcile accounts"><div className="space-y-3">{accounts.slice(0, 3).map(([name, institution, currency, balance, sync]) => <div key={name} className="rounded-lg border border-border p-3"><div className="flex justify-between"><strong className="text-xs">{name}</strong><Badge tone="green">Active</Badge></div><p className="mt-1 text-[11px] text-muted-foreground">{institution} · {currency} · {balance}</p><p className="mt-2 text-[11px] text-muted-foreground">Last sync {sync}</p><div className="mt-3 flex gap-3 text-[11px] font-bold text-foreground"><button>Open Account</button><button>Reconcile</button><button>Transactions</button></div></div>)}</div></Card><Card title="Recent Transactions" subtitle="Last 8 transactions" action="View all"><div className="overflow-x-auto"><table className="w-full text-left text-[11px]"><thead className="text-muted-foreground"><tr><th className="pb-2">Date</th><th className="pb-2">Description</th><th className="pb-2">Category</th><th className="pb-2 text-right">Amount</th></tr></thead><tbody className="divide-y divide-border">{transactions.map(([date, desc, account, category, amount, status]) => <tr key={`${date}-${desc}`}><td className="py-2 text-muted-foreground">{date}</td><td className="max-w-[150px] truncate py-2 font-semibold">{desc}<span className="block font-normal text-muted-foreground">{account} · {status}</span></td><td className="py-2 text-muted-foreground">{category}</td><td className={`py-2 text-right font-bold ${amount.startsWith('+') ? 'text-foreground' : ''}`}>{amount}</td></tr>)}</tbody></table></div></Card><Card title="Reconciliation" subtitle="Reconciliation health"><div className="grid grid-cols-2 gap-3 text-xs"><div className="rounded-lg bg-secondary p-3"><p className="text-foreground">Reconciled</p><strong className="text-lg text-foreground">1,847</strong></div><div className="rounded-lg bg-chart-1/10 p-3"><p className="text-chart-1">Unreconciled</p><strong className="text-lg text-chart-1">24</strong></div><div><p className="text-muted-foreground">Exceptions</p><strong>3</strong></div><div><p className="text-muted-foreground">Needs attention</p><strong>1 account</strong></div></div><p className="mt-5 text-xs text-muted-foreground">Last reconciliation: <strong>Today 09:14</strong></p><div className="mt-5 flex gap-2"><button className="btn-primary">Open Reconciliation</button><button className="btn-secondary">Review Exceptions</button></div></Card></div>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Taxes" subtitle="YTD 2024"><div className="grid grid-cols-3 gap-3 text-xs"><div><p className="text-muted-foreground">Tax collected</p><strong>€284,732</strong></div><div><p className="text-muted-foreground">Tax paid</p><strong>€142,840</strong></div><div><p className="text-muted-foreground">Liabilities</p><strong>€141,892</strong></div></div><div className="mt-5 flex items-center justify-between"><span className="text-xs text-muted-foreground">Filing status</span><Badge tone="amber">In Progress</Badge></div><p className="mt-4 text-[11px] text-muted-foreground">Tax information based on connected financial data. Verify with an appropriate tax professional.</p><button className="btn-primary mt-4">Open Taxes</button></Card><Card title="Payouts" subtitle="Provider · Stripe"><div className="space-y-3 text-xs"><div className="flex justify-between"><span>Pending · 3 payouts</span><strong>€24,640</strong></div><div className="flex justify-between"><span>Completed · 48 payouts</span><strong className="text-chart-4">€1,840,200</strong></div><div className="flex justify-between"><span>Failed · 1 payout</span><strong className="text-chart-5">€3,400</strong></div></div><div className="mt-5 flex gap-2"><button className="btn-primary">View Payouts</button><button className="btn-secondary">Open Payment Provider</button></div></Card></div>
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="AI Insights" subtitle="AI-generated · Evidence-backed"><div className="space-y-3">{['Expenses increased 14% compared with the previous period, primarily driven by higher software and supplier expenses.', 'Revenue growth of 12.4% is driven by a 22% increase in subscription income.', 'Cash flow improved despite increased expenses due to faster invoice collection.'].map(text => <div key={text} className="rounded-lg border border-border bg-secondary/50 p-3"><p className="text-xs font-semibold leading-5 text-foreground">{text}</p><p className="mt-2 text-[11px] text-foreground">Evidence · Software +34% · Suppliers +12%</p></div>)}</div></Card><Card title="AI Recommendations" subtitle="AI Recommendation"><div className="space-y-3">{['Review 14 overdue invoices · €62,400 at risk', 'Review software expense growth · 34% increase detected', 'Investigate payment failures · 6 failed payments', 'Review cash flow forecast · minimum balance €840,000 in 45 days'].map((text, i) => <button key={text} className="flex w-full items-center gap-3 rounded-lg border border-border p-3 text-left hover:border-border"><span className="grid h-6 w-6 place-items-center rounded-full bg-secondary text-xs font-bold text-foreground">{i + 1}</span><span className="text-xs font-semibold">{text}</span><ChevronDown size={14} className="ml-auto -rotate-90 text-muted-foreground" /></button>)}</div></Card></div>
        <Card title="Finance Activity" subtitle="Recent activity across your workspace"><div className="grid grid-cols-1 divide-y divide-border md:grid-cols-2 md:divide-x md:divide-y-0">{activities.map(([event, detail, time, tone]) => <div key={event} className="flex items-center gap-3 py-3 md:px-4 first:pt-0 md:first:pl-0"><div className={`grid h-8 w-8 place-items-center rounded-full ${tone === 'violet' ? 'bg-secondary text-foreground' : tone === 'green' ? 'bg-chart-4/10 text-chart-4' : 'bg-secondary text-muted-foreground'}`}><Activity size={14} /></div><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{event}</p><p className="truncate text-[11px] text-muted-foreground">{detail}</p></div><time className="text-[11px] text-muted-foreground">{time}</time></div>)}</div></Card>
