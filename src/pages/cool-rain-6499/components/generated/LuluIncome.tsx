...
export function LuluIncome() {
  const { items: liveRecords, loading: liveLoading, error: liveError } = useLiveRecords('finance_income');
  const liveIncomeRows: IncomeRow[] = liveRecords.map((record) => {
    const fields = record as unknown as Record<string, unknown>;
    return { id: record.id, date: record.updatedAt, customer: String(fields.customer ?? record.name), category: String(fields.category ?? '—'), amount: record.valueAmount ?? '—', currency: record.currency ?? '—', status: (record.status || 'Recorded') as Status, account: String(fields.account ?? '—'), channel: String(fields.channel ?? '—'), source: String(fields.source ?? 'Live record'), created: record.createdAt };
  });
  const [selected, setSelected] = useState<string[]>([]);
  const [query, setQuery] = useState('');
  const [activeRange, setActiveRange] = useState('Monthly');
  const [sent, setSent] = useState(false);
  const filtered = useMemo(() => liveIncomeRows.filter(r => Object.values(r).join(' ').toLowerCase().includes(query.toLowerCase())), [query]);
  const toggle = (id: string) => setSelected(s => s.includes(id) ? s.filter(x => x !== id) : [...s, id]);
  return <main className="min-h-screen bg-[var(--background)] text-foreground flex font-sans">
    {liveError && <div className="mx-5 mt-4 rounded-lg border border-chart-5/30 bg-chart-5/5 px-4 py-3 text-sm text-chart-5 lg:ml-[250px]">{liveError}</div>}
    {!liveLoading && liveIncomeRows.length === 0 && <div className="mx-5 mt-4 rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground lg:ml-[250px]">No income records are available yet. Connect a finance platform or add the first income record.</div>}
  <aside className="w-[238px] shrink-0 bg-[var(--background)] text-foreground flex flex-col min-h-screen p-4 fixed inset-y-0 left-0 z-10"><div className="flex items-center gap-3 px-3 py-3 mb-5"><div className="h-8 w-8 rounded-xl bg-primary flex items-center justify-center shadow-lg shadow-black/40 text-primary-foreground"><Sparkles size={17} /></div><div><strong className="text-[16px] tracking-tight">lulu<span className="text-foreground">.</span></strong><p className="text-[9px] text-muted-foreground tracking-[.18em] uppercase">AI workspace</p></div></div><LuluSectionNavigation activeId="cool-rain-6499" /><div className="mt-auto p-3 rounded-xl bg-secondary border border-border"><div className="flex items-center gap-2"><div className="h-7 w-7 rounded-full bg-primary text-primary-foreground text-[11px] font-bold flex items-center justify-center">—</div><div><p className="text-[11px] font-semibold">—</p><p className="text-[10px] text-muted-foreground">Administrator</p></div><MoreHorizontal size={15} className="ml-auto text-muted-foreground" /></div></div></aside>
  <section className="ml-[238px] flex-1 min-w-0"><div className="max-w-[1440px] mx-auto px-8 py-7"><div className="flex items-center justify-between mb-7"><div><div className="flex gap-2 text-[11px] text-muted-foreground mb-2"><span>Finance</span><span>/</span><span className="text-muted-foreground">Income</span></div><h1 className="text-[28px] font-bold tracking-[-.04em]">Income</h1><p className="text-[13px] text-muted-foreground mt-1">Track, understand and analyze where your business income comes from.</p></div><div className="flex items-center gap-2"><button className="bg-primary text-primary-foreground rounded-lg px-4 py-2.5 text-[12px] font-bold shadow-sm hover:bg-primary"><span>+ Record Income</span></button>{['Ask Lulu AI', 'Import', 'Export'].map(x => <button key={x} className="bg-card border border-border rounded-lg px-3 py-2.5 text-[12px] font-semibold text-foreground hover:border-border">{x}</button>)}<button className="p-2.5 rounded-lg border border-border bg-card"><MoreHorizontal size={16} /></button></div></div>
   <div className="grid grid-cols-7 gap-3 mb-6">{([] as string[][]).map(kpi => <article key={kpi[0]} className="bg-card rounded-xl border border-border/80 px-4 py-4 shadow-[0_1px_2px_rgba(0,0,0,.03)] min-w-0"><p className="text-[10px] uppercase tracking-[.06em] text-muted-foreground font-bold truncate">{kpi[0]}</p><p className="text-[20px] font-bold tracking-[-.04em] mt-2 truncate">{kpi[1]}</p><div className="flex flex-wrap gap-1.5 mt-2"><span className="text-[10px] text-muted-foreground">{kpi[2]}</span><span className="text-[9px] font-semibold text-foreground bg-secondary px-1.5 py-0.5 rounded">{kpi[3]}</span></div></article>)}</div>
   ...
   <div className="grid grid-cols-2 gap-5 mb-6"><section className="bg-card border border-border/80 rounded-xl p-5"><SectionTitle title="Income by Source" subtitle="How income enters your business" /><div className="flex items-center gap-7 mb-4"><div className="h-36 w-36 rounded-full" style={{
              ...
              }}><div className="h-20 w-20 bg-card rounded-full relative top-8 left-8 flex items-center justify-center"><span className="text-center text-[11px] font-bold">—<small className="block text-[9px] text-muted-foreground font-normal">total</small></span></div></div><div className="space-y-2 text-[11px]">{(liveLoading ? [] : sourceData).map((r, i) => <div key={r[0]} className="flex items-center gap-2"><span className={`w-2 h-2 rounded-full ${['bg-primary', 'bg-primary', 'bg-primary', 'bg-primary', 'bg-muted'][i]}`} /><span>{r[0]}</span><strong className="ml-auto">{r[2]}</strong></div>)}</div></div><SmallTable headers={['Source', 'Income', '% of Total', 'Growth', 'Records']} rows={sourceData} /></section><section className="bg-card border border-border/80 rounded-xl p-5"><SectionTitle title="Income Categories" subtitle="Performance by category" /><div className="space-y-3 mb-5">{(liveLoading ? [] : categoryData).map((r, i) => <div key={r[0]} className="grid grid-cols-[120px_1fr_42px] items-center gap-2 text-[11px]"><span className="truncate">{r[0]}</span><div className="h-2 rounded-full bg-secondary"><div className="h-2 rounded-full bg-primary text-primary-foreground" style={{
   ...
   <div className="grid grid-cols-2 gap-5 mb-6"><section className="bg-card border border-border/80 rounded-xl p-5"><SectionTitle title="Income by Channel" /><div className="flex items-end gap-4 h-28 px-4 mb-4 border-b border-border">{([] as (string | number)[][]).map(x => <div key={x[0]} className="flex-1 flex flex-col items-center gap-1"><span className="text-[10px] text-muted-foreground">{x[1]}k</span><div className="w-full bg-primary rounded-t text-primary-foreground" style={{
                  height: `${x[1]}px`
