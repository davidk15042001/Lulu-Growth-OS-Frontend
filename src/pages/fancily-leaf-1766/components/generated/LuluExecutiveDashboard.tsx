...
const tasks: Task[] = [];
const activities: Array<readonly [string, string, string, string, typeof Sparkles]> = [];
const integrations: Array<readonly [string, string, string, typeof ShoppingBag]> = [];
const Sparkline = ({
  color = 'var(--chart-2)',
  dashed = false
}: {
...
const AreaChart = () => <div className="flex h-64 items-center justify-center rounded-lg border border-dashed border-border px-4 text-center text-sm text-muted-foreground" role="status">No live time series has been recorded for this workspace.</div>;
...
export const LuluExecutiveDashboard = () => {
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground" style={{
    ...
    <aside className={`${mobileNav ? 'flex' : 'hidden'} fixed inset-y-0 left-0 z-20 flex h-dvh min-h-0 w-60 max-w-[calc(100vw-1rem)] flex-col overflow-hidden border-r border-border bg-[var(--sidebar)] p-5 lg:flex`}>
      ...
      <div className="shrink-0 space-y-3 border-t border-border pt-4"><div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-card text-xs font-semibold text-foreground">{accountInitial}</div><div><p className="text-sm font-medium text-foreground">{accountName}</p><p className="text-xs text-muted-foreground">{t("Workspace")}: {selectedWorkspace?.companyName ?? t("Workspace account")}</p></div><MoreHorizontal className="ml-auto text-muted-foreground" size={16} /></div><div className="flex items-center gap-2 rounded-lg border border-chart-4/20 bg-chart-4/5 px-3 py-2 text-xs text-chart-4"><span className="h-2 w-2 rounded-full bg-chart-4" /> AI Active <span className="ml-auto text-muted-foreground">Live</span></div></div>
    </aside>
    ...
    <main className="min-h-screen min-w-0 overflow-x-hidden lg:ml-60"><div className="mx-auto max-w-[1400px] px-5 py-6 sm:px-8 sm:py-8">
      ...
      <section className="mb-5 flex flex-col justify-between gap-3 rounded-xl border border-border bg-[var(--card)] px-4 py-3 text-sm sm:flex-row sm:items-center"><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-muted-foreground" /><strong className="font-medium text-foreground">Business Status: Awaiting live data</strong><span className="hidden text-muted-foreground sm:inline">— no verified live workspace metrics are available yet</span></div><div className="flex items-center gap-3 text-xs text-muted-foreground"><span>Updated from backend</span><button aria-label="Refresh status" onClick={refresh} className="rounded p-1 text-foreground hover:bg-secondary hover:text-foreground"><RefreshCw size={14} /></button></div></section>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2"><section className="rounded-xl border border-border bg-[var(--card)] p-5"><div className="flex items-start justify-between"><div><p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Relationship health</p><h2 className="mt-1 text-lg font-semibold text-foreground">Customer Overview</h2></div><Users className="text-foreground" size={20} /></div><div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3">{customerMetrics.map(item => {
              ...
              })}</div><div className="mt-5 rounded-lg border border-border bg-[var(--secondary)] p-3"><div className="mb-2 flex justify-between text-xs text-muted-foreground"><span>Customer movement</span><span className="text-foreground">New / Returning</span></div><div className="flex h-24 items-end gap-1">{([] as number[]).map((height, index) => <div key={height} className="flex flex-1 flex-col justify-end gap-0.5"><span className="rounded-t-sm bg-primary text-primary-foreground" style={{
                    height: `${height * .65}%`
                  }} /><span className="bg-secondary/60" style={{
                    height: `${height * .35}%`
                  }} /></div>)}</div></div><div className="mt-5 flex items-center justify-between"><div><p className="text-xs text-muted-foreground">Customer Health</p><p className="mt-1 text-2xl font-bold text-foreground">—<span className="text-sm text-muted-foreground"> / 100</span></p></div><div className="grid grid-cols-2 gap-x-5 gap-y-2 text-xs text-muted-foreground">{([] as string[]).map(item => <span key={item}>{item}</span>)}</div></div><button className="mt-5 text-sm font-medium text-foreground hover:text-foreground">View Customer Insights <ArrowRight size={14} className="inline" /></button></section>
