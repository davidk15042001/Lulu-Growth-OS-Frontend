...
export const LuluComparisons = () => {
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground" style={{
    ...
    <main className="min-h-screen lg:ml-60"><div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-7 sm:py-7">
      <header className="mb-7 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><button className="mb-4 lg:hidden" aria-label="Open navigation" onClick={() => setMobileNav(true)}><Menu /></button><p className="mb-3 text-xs text-muted-foreground">Intelligence <span className="mx-2">/</span> Analytics <span className="mx-2">/</span> <span className="text-foreground">Comparisons</span></p><h1 className="text-3xl font-bold tracking-[-.04em] text-foreground sm:text-4xl">Comparisons</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Compare business performance, identify meaningful differences and understand what is driving them.</p></div><div className="flex flex-wrap gap-2"><button className="flex h-9 items-center gap-2 rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground hover:bg-primary"><Sparkles size={14} />Ask Lulu AI</button><button className="flex h-9 items-center gap-2 rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground hover:bg-primary"><Plus size={14} />Create Comparison</button><button className="flex h-9 items-center gap-2 rounded-md border border-border bg-secondary px-3 text-xs text-foreground hover:bg-secondary"><Download size={14} />Export</button><button aria-label="Refresh comparison" onClick={refresh} className="flex h-9 w-9 items-center justify-center rounded-md border border-border bg-secondary text-foreground hover:bg-secondary"><RefreshCw size={15} className={refreshing ? 'animate-spin' : ''} /></button></div></header>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.5fr_1fr]"><Section title="Performance Difference" subtitle="Relative improvement by KPI"><div className="space-y-4">{[['Revenue', '+15.7%', 52, 'A'], ['ROAS', '+16.7%', 56, 'A'], ['CAC', '+12.5%', 43, 'A'], ['Conversion', '−5.9%', 22, 'B'], ['Customers', '+29.6%', 78, 'A']].map(([label, value, width, side]) => <div key={label as string}><div className="mb-1 flex justify-between text-xs"><span className="text-muted-foreground">{label}</span><strong className={side === 'A' ? 'text-chart-4' : 'text-foreground'}>{value} · {side} better</strong></div><div className="h-2 rounded-full bg-secondary"><div className={`h-2 rounded-full ${side === 'A' ? 'bg-primary' : 'bg-primary'}`} style={{
                    width: `${width}%`
                  }} /></div></div>)}</div></Section><Section title="Overall Winner"><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-secondary/10 text-foreground"><TrendingUp /></div><div><h3 className="font-semibold text-foreground">Google Ads</h3><p className="text-xs text-muted-foreground">Outperformed in 4 of 5 KPIs</p></div></div><div className="mt-5 space-y-3 text-sm text-foreground"><p><strong className="text-foreground">Largest advantage:</strong> Customers <span className="text-chart-4">(+29.6%)</span></p><p><strong className="text-foreground">Smallest advantage:</strong> CAC <span className="text-chart-4">(−12.5%)</span></p><p><strong className="text-foreground">Only B advantage:</strong> Conversion Rate <span className="text-foreground">(+5.9%)</span></p></div></Section></div>

      <Section title="Comparison Over Time" subtitle="Google Ads vs Meta Ads · Revenue · Last 30 Days" action={<div className="flex rounded-md border border-border p-1">{['Daily', 'Weekly', 'Monthly', 'Quarterly'].map(tab => <button key={tab} onClick={() => setActiveTrend(tab)} className={`rounded px-3 py-1.5 text-xs ${activeTrend === tab ? 'bg-secondary/20 text-foreground' : 'text-foreground'}`}>{tab}</button>)}</div>}><div className="mb-4 flex gap-4 text-xs"><span className="text-foreground">● Google Ads</span><span className="text-foreground">○ Meta Ads</span><span className="text-muted-foreground">Peak gap · Jan 24</span></div><div className="overflow-x-auto"><svg viewBox="0 0 900 260" className="min-w-[680px] w-full" role="img" aria-label="Dual line chart comparing Google Ads and Meta Ads revenue over thirty days"><g stroke="rgba(0,0,0,.08)">{[45, 90, 135, 180, 225].map(y => <line key={y} x1="60" x2="870" y1={y} y2={y} />)}</g><line x1="645" x2="645" y1="22" y2="225" stroke="var(--chart-2)" strokeDasharray="4 5" /><path d="M60 190 C130 175 150 182 205 156 S300 174 350 130 S440 142 485 108 S575 126 625 84 S710 96 760 60 S820 69 870 42" fill="none" stroke="var(--chart-2)" strokeWidth="3" /><path d="M60 205 C130 198 150 193 205 184 S300 190 350 163 S440 178 485 145 S575 164 625 132 S710 144 760 108 S820 126 870 96" fill="none" stroke="var(--chart-1)" strokeWidth="2.5" strokeDasharray="7 6" /><g fill="var(--muted-foreground)" fontSize="11"><text x="8" y="48">€90k</text><text x="15" y="140">€60k</text><text x="20" y="228">€30k</text><text x="58" y="248">Jan 1</text><text x="255" y="248">Jan 8</text><text x="450" y="248">Jan 15</text><text x="635" y="248">Jan 22</text><text x="820" y="248">Jan 30</text></g></svg></div><div className="mt-4 flex flex-wrap gap-2">{['Revenue', 'ROAS', 'CAC', 'Customers', 'Conversion Rate'].map((item, index) => <button key={item} className={`rounded-full border px-3 py-1.5 text-xs ${index === 0 ? 'border-border/30 bg-secondary/10 text-foreground' : 'border-border text-foreground'}`}>{item}</button>)}</div></Section>

      <Section title="KPI Comparison" subtitle="Selected KPI: Revenue"><div className="grid items-center gap-5 lg:grid-cols-[1fr_auto_1fr]"><div className="rounded-lg border border-border/15 bg-secondary/[.04] p-4"><p className="text-xs text-muted-foreground">Google Ads · A</p><p className="mt-2 text-3xl font-bold text-foreground">€82,400</p><p className="mt-1 text-xs text-chart-4">↑ 15.7% · Positive trend</p><Sparkline /></div><div className="text-center"><p className="text-2xl font-semibold text-chart-4">+€11,200</p><p className="text-xs text-chart-4">+15.7% difference</p></div><div className="rounded-lg border border-border/15 bg-secondary/[.04] p-4"><p className="text-xs text-muted-foreground">Meta Ads · B</p><p className="mt-2 text-3xl font-bold text-foreground">€71,200</p><p className="mt-1 text-xs text-muted-foreground">↑ 8.4% · Positive trend</p><Sparkline color="var(--chart-1)" /></div></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-xs"><thead className="border-b border-border text-muted-foreground"><tr><th className="pb-2">Period</th><th>Google Ads</th><th>Meta Ads</th><th>Difference</th></tr></thead><tbody>{[['Jan', '€82,400', '€71,200', '+€11,200'], ['Dec', '€78,200', '€68,400', '+€9,800'], ['Nov', '€74,800', '€65,100', '+€9,700'], ['Oct', '€70,400', '€62,800', '+€7,600'], ['Sep', '€68,900', '€60,400', '+€8,500'], ['Aug', '€64,200', '€57,900', '+€6,300']].map(row => <tr key={row[0]} className="border-b border-border/[.05]"><td className="py-2.5 text-muted-foreground">{row[0]}</td><td className="text-foreground">{row[1]}</td><td className="text-foreground">{row[2]}</td><td className="text-foreground">{row[3]}</td></tr>)}</tbody></table></div><button className="mt-4 text-xs text-foreground">Explore in KPI Explorer <ArrowRight className="inline" size={13} /></button></Section>
      ...
      <Section title="Why Are They Different?" subtitle="AI-identified factors explaining the revenue difference between Google Ads and Meta Ads." action={<AIBadge label="AI Inferred" />}><div className="mx-auto max-w-3xl space-y-2"><div className="rounded-lg border border-border/20 bg-secondary/[.05] p-4 text-center"><h3 className="font-semibold text-foreground">Revenue Difference <span className="text-foreground">+€11,200</span></h3><span className="text-[10px] text-foreground">Calculated</span></div><div className="text-center text-muted-foreground">↓</div><div className="rounded-lg border border-border/[.08] bg-secondary p-4 text-center"><h3 className="font-medium text-foreground">Customer Volume <span className="text-chart-4">+29.6% via Google Ads</span></h3><span className="text-[10px] text-chart-4">Observed</span></div><div className="text-center text-muted-foreground">↓</div><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-lg border border-border/15 bg-secondary/[.04] p-4"><h3 className="font-medium text-foreground">Conversion Rate</h3><p className="mt-1 text-xs text-muted-foreground">Meta slightly higher: 5.1% vs 4.8%</p><span className="text-[10px] text-foreground">Observed</span></div><div className="rounded-lg border border-border/[.08] bg-secondary p-4"><h3 className="font-medium text-foreground">Traffic Quality</h3><p className="mt-1 text-xs text-muted-foreground">Google Ads higher-intent search traffic</p><span className="text-[10px] text-foreground">AI Inferred</span></div></div><div className="text-center text-muted-foreground">↓</div><div className="rounded-lg border border-border/[.08] bg-secondary p-4 text-center"><h3 className="font-medium text-foreground">Average Order Value <span className="text-foreground">€44.8 vs €43.2</span></h3><p className="text-xs text-muted-foreground">Product Mix: similar distribution, no significant skew detected.</p><span className="text-[10px] text-muted-foreground">Correlated</span></div></div><p className="mt-5 text-center text-xs italic text-muted-foreground">Correlation identified. Causal relationship not confirmed. AI Inferred factors are based on pattern analysis.</p></Section>
