...
import { Activity, AlertTriangle, ArrowDownRight, ArrowUpRight, BarChart3, Bell, Brain, Check, ChevronDown, ChevronRight, CircleHelp, Database, Gauge, Layers3, Menu, MessageSquare, Search, Send, Settings2, ShieldCheck, Sparkles, Target, TrendingUp, Users, WalletCards, X } from 'lucide-react';
...
export const LuluExecutiveOverview = () => {
  ...
  <main className="min-w-0 lg:ml-[248px]"><header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-border bg-secondary px-4 backdrop-blur md:px-8"><button onClick={() => setMobileMenu(true)} className="mr-3 lg:hidden" aria-label="Open navigation"><Menu size={21} /></button><div className="flex items-center gap-2 text-sm text-muted-foreground"><span>Intelligence</span><ChevronRight size={14} /><strong className="text-foreground">Executive Overview</strong></div><div className="flex items-center gap-2"><Button>Last 30 Days <ChevronDown className="ml-1 inline" size={13} /></Button><Button>Compare <ChevronDown className="ml-1 inline" size={13} /></Button><span className="hidden rounded-lg border border-border px-3 py-2 text-xs font-semibold text-muted-foreground md:inline-flex">Use Update in the navigation bar</span><button className="rounded-lg p-2" aria-label="Notifications"><Bell size={17} /></button></div></header>
  ...
  <section className="mb-9"><Header title="Autonomous Decisions in Progress" /><div className="grid gap-4 lg:grid-cols-2">{[['Reallocate Advertising Budget', 'Meta Ads efficiency declined while Google Ads is outperforming current allocation.', 'Shift a portion of budget toward Google Ads.', 'ROAS improvement ~+0.4'], ['Increase Retention Investment', 'CAC increasing while retention recently stabilized.', 'Allocate budget to retention programs for high-LTV segments.', '2–3pp retention improvement, lower CAC']].map(item => <article key={item[0]} className="rounded-xl border border-border bg-card p-5 shadow-sm"><h3 className="text-base font-bold">{item[0]}</h3><p className="mt-4 text-xs font-bold uppercase tracking-wide text-muted-foreground">Current situation</p><p className="mt-1 text-sm text-muted-foreground">{item[1]}</p><p className="mt-3 text-xs font-bold uppercase tracking-wide text-muted-foreground">Lulu decision</p><p className="mt-1 text-sm text-foreground">{item[2]}</p><p className="mt-3 text-sm font-semibold text-chart-4">Expected impact: {item[3]} · Risk: Low</p><div className="mt-4 flex flex-wrap gap-2"><Button>View execution</Button><Button primary>Open activity</Button></div><p className="mt-3 text-[11px] text-muted-foreground">Lulu executes consequential actions autonomously inside deterministic safeguards. Paid-media spend is capped by the prepaid wallet.</p></article>)}</div></section>
