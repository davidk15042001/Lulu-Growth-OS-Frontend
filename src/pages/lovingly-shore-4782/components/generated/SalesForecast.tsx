...
export function SalesForecast() {
  ...
  const liveKpis: any[][] = [];
  const liveCategories = ['Closed Won', 'Commit', 'Best Case', 'Upside', 'At Risk'].map(category => { const records = liveForecasts.filter(record => `${record.stage || record.status || ''}`.toLowerCase().includes(category.toLowerCase().split(' ')[0])); const amount = records.reduce((sum, record) => sum + Number(record.valueAmount || record.data?.value || 0), 0); return [category, String(records.length), formatMoney(amount), totalForecast ? `${Math.round(amount / totalForecast * 100)}%` : '—', 'Live records', category === 'At Risk' ? 'bg-destructive' : 'bg-primary']; });
