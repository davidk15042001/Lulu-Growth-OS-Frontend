...
export function LuluContacts() {
  ...
  return <div className="flex min-h-screen bg-[var(--background)] font-sans text-muted-foreground"><Sidebar /><main className="min-w-0 flex-1 overflow-hidden">
    ...
    {preview && <aside className="fixed inset-y-0 right-0 z-40 w-full max-w-sm overflow-y-auto border-l border-border bg-[var(--sidebar)] p-6 shadow-2xl"><div className="flex items-center justify-between"><p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Contact preview</p><button onClick={() => setPreview(null)} aria-label="Close preview" className="rounded p-1.5 text-foreground hover:bg-secondary"><X size={18} /></button></div><div className="mt-8 flex items-center gap-4"><span className="grid size-[72px] place-items-center rounded-full text-xl font-semibold text-foreground" style={{
          ...
          }}>{preview.initials}</span><div><h2 className="text-xl font-semibold text-foreground">{preview.name}</h2><p className="mt-1 text-sm text-muted-foreground">{preview.title} · {preview.company}</p><div className="mt-2 flex gap-2"><span className="rounded-md bg-secondary/15 px-2 py-1 text-[10px] text-foreground">{preview.type}</span><span className="rounded-md bg-secondary/10 px-2 py-1 text-[10px] text-foreground">{preview.status}</span></div></div></div><div className="mt-8 space-y-6"><section><h3 className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Contact Info</h3><dl className="mt-3 space-y-3 text-sm"><div><dt className="text-xs text-muted-foreground">Email</dt><dd className="mt-1 text-foreground">{preview.email}</dd></div><div><dt className="text-xs text-muted-foreground">Phone</dt><dd className="mt-1 text-foreground">{preview.phone}</dd></div><div><dt className="text-xs text-muted-foreground">Location</dt><dd className="mt-1 text-foreground">{preview.location}</dd></div></dl></section><section><h3 className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">CRM</h3><dl className="mt-3 space-y-3 text-sm"><div className="flex justify-between"><dt className="text-muted-foreground">Owner</dt><dd className="text-foreground">{preview.owner}</dd></div><div className="flex justify-between"><dt className="text-muted-foreground">Created</dt><dd className="text-foreground">{preview.created}</dd></div><div className="flex justify-between"><dt className="text-muted-foreground">Last Activity</dt><dd className="text-foreground">{preview.activity}</dd></div></dl><div className="mt-3 flex flex-wrap gap-1.5">{preview.tags.map(tag => <span key={tag} className="rounded-md bg-secondary/10 px-2 py-1 text-[10px] text-foreground">{tag}</span>)}</div></section><section><div className="flex items-center justify-between"><h3 className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Open Deals</h3><span className="text-[10px] text-foreground">AI Insight · 94%</span></div><div className="mt-3 rounded-lg border border-dashed border-border p-3 text-xs text-muted-foreground">No live deals are available for this contact yet.</div></section><section><h3 className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Recent Activity</h3>{([] as string[]).map((item, i) => <div key={item} className="mt-3 flex gap-3 text-sm"><span className="mt-0.5 text-foreground"><Activity size={14} /></span><div><p className="text-foreground">{item}</p><p className="mt-1 text-xs text-muted-foreground">{['2 hours ago', 'Yesterday', '4 days ago'][i]}</p></div></div>)}</section></div><div className="mt-8 flex flex-wrap gap-2 border-t border-border pt-5"><button className="rounded-lg bg-primary px-3 py-2 text-xs font-medium text-primary-foreground">Open Contact</button><button className="rounded-lg border border-border px-3 py-2 text-xs text-foreground">Add Task</button><button className="rounded-lg border border-border px-3 py-2 text-xs text-foreground">Add Note</button><button onClick={() => setModal('edit')} className="rounded-lg border border-border p-2 text-foreground" aria-label="Edit contact"><Edit3 size={14} /></button></div></aside>}
