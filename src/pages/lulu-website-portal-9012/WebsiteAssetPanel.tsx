...
import { Image, Loader2, Sparkles, Trash2, Upload, X, XCircle } from "lucide-react";
...
export function WebsiteAssetPanel({ workspaceId, site }: { workspaceId: string; site: WebsiteSite }) {
  ...
  const choose = (next: File | null) => {
    ...
    if (!next) { setFile(null); setPreviewUrl(""); setCropPosition({ x: 0, y: 0 }); setZoom(1); setIsDragging(false); dragRef.current = null; return; }
    setFile(next);
  ...
  const endCropDrag = () => { dragRef.current = null; setIsDragging(false); };
  ...
  const deleteAsset = async (asset: ManagedWebsiteAsset) => {
    ...
    setError("");
    try {
      await websitesApi.deleteAsset(workspaceId, site.id, asset.id);
      await load();
    } catch (cause) {
      setError(getFriendlyErrorMessage(cause, t("The image could not be deleted.")));
    } finally {
