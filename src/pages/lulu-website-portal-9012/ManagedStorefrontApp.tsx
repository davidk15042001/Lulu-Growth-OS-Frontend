...
export default function ManagedStorefrontApp({ initialPanel }: { initialPanel?: ManagedWebsitePanel } = {}) {
  ...
  return <main className="min-h-screen bg-[var(--background)] px-5 py-7 text-foreground sm:px-8 sm:py-10"><div className="mx-auto max-w-[1400px] space-y-6">
    ...
    {panel === "preview" ? <section className="space-y-6"><div className="rounded-2xl border border-border bg-card p-5 sm:p-7"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.15em] text-primary">Lulu Storefront</p><h2 className="mt-2 text-2xl font-semibold">Website-Vorschau</h2><p className="mt-2 text-sm text-muted-foreground">So sehen Kunden deine veröffentlichte Website und deine Produkte.</p></div>{publicSlug && storefront ? <a href={`/api/v1/public/storefront/${encodeURIComponent(publicSlug)}/render`} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center gap-2 rounded-xl border border-border px-4 text-sm font-semibold hover:bg-secondary">Öffnen <ExternalLink size={15} /></a> : null}</div><EmptyWebsiteTemplate />{storefront?.products.length ? <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{storefront.products.map((product) => <ProductCard key={product.id} product={product} />)}</div> : null}</div></section> : null}
