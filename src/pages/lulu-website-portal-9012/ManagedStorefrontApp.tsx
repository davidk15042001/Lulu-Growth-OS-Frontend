...
import {
  ...
  type WebsiteGenerationJob,
  type WebsiteSite,
  type WebsiteTemplateChoice,
} from "../../api/websites";
...
function TemplateChoicePicker({ choice, hasProducts, onChange, labels }: { choice: WebsiteTemplateChoice; hasProducts: boolean; onChange: (value: WebsiteTemplateChoice) => void; labels: { title: string; description: string; auto: string; autoDescription: string; standard: string; standardDescription: string; oneProduct: string; oneProductDescription: string; addProduct: string } }) {
  const options = [["auto", labels.auto, labels.autoDescription, Sparkles], ["standard", labels.standard, labels.standardDescription, Globe2], ["one-product", labels.oneProduct, labels.oneProductDescription, ShoppingBag]] as const;
  return <fieldset className="mt-5 rounded-xl border border-border bg-background p-4"><legend className="px-1 text-sm font-semibold">{labels.title}</legend><p className="mt-1 text-xs leading-5 text-muted-foreground">{labels.description}</p><div role="radiogroup" aria-label={labels.title} className="mt-4 grid gap-3 sm:grid-cols-3">{options.map(([value, label, description, Icon]) => { const selected = choice === value; const unavailable = value === "one-product" && !hasProducts; return <button key={value} type="button" role="radio" aria-checked={selected} disabled={unavailable} onClick={() => onChange(value)} className={`flex min-h-32 flex-col items-start gap-2 rounded-lg border p-4 text-left transition ${selected ? "border-primary bg-primary/10 ring-2 ring-primary/20" : "border-border bg-card hover:border-primary/50"} ${unavailable ? "cursor-not-allowed opacity-50" : ""}`}><Icon size={18} className={selected ? "text-primary" : "text-muted-foreground"} /><span className="text-sm font-semibold">{label}</span><span className="text-xs leading-5 text-muted-foreground">{unavailable ? labels.addProduct : description}</span></button>; })}</div></fieldset>;
}
...
function storefrontProductsToTemplateItems(products: StorefrontProduct[]): TemplateCatalogItem[] {
...
function storefrontServicesToTemplateItems(plan: Record<string, unknown>): TemplateCatalogItem[] {
  const profile = plan.contentProfile && typeof plan.contentProfile === "object" ? plan.contentProfile as Record<string, unknown> : {};
  const services = profile.services && typeof profile.services === "object" ? profile.services as Record<string, unknown> : {};
...
function PublishedWebsiteTemplate({ storefront, branding, palette }: { storefront: Storefront; branding?: TemplateBranding; palette?: TemplatePalette }) {
  const [locale, setLocale] = useState<TemplateLocale>("de");
  const base = placeholderCopy(locale);
  const profile = storefront.plan.contentProfile && typeof storefront.plan.contentProfile === "object" ? storefront.plan.contentProfile as Record<string, unknown> : {};
  const home = profile.home && typeof profile.home === "object" ? profile.home as Record<string, unknown> : {};
  const services = profile.services && typeof profile.services === "object" ? profile.services as Record<string, unknown> : {};
  const products = storefrontProductsToTemplateItems(storefront.products);
  const serviceItems = storefrontServicesToTemplateItems(storefront.plan);
  const trust = Array.isArray(home.trustItems) ? home.trustItems.map(String).filter(Boolean).slice(0, 5) : base.trust;
  const copy: TemplateCopy = {
    ...base,
    brand: String(profile.siteTitle || storefront.name || base.brand),
    footerBrand: String(profile.siteTitle || storefront.name || base.footerBrand),
    heroEyebrow: String(home.eyebrow || base.heroEyebrow),
    heroTitle: String(home.headline || `${base.preview}: ${storefront.name}`),
    starterCopy: String(home.introduction || base.starterCopy),
    trust,
    servicesTitle: String(services.title || base.servicesTitle),
    servicesBody: String(services.introduction || base.servicesBody),
...
export default function ManagedStorefrontApp({ initialPanel }: { initialPanel?: ManagedWebsitePanel } = {}) {
  ...
  const [templateChoice, setTemplateChoice] = useState<WebsiteTemplateChoice>("auto");
  ...
  const load = async () => {
    ...
    try {
      ...
      if (next) {
        ...
        setJob(active.data);
        const activeTemplate = active.data?.preview && typeof active.data.preview === "object" ? (active.data.preview as Record<string, unknown>).templateChoice : undefined;
        if (activeTemplate === "standard" || activeTemplate === "one-product" || activeTemplate === "auto") setTemplateChoice(activeTemplate);
  ...
  const generate = async () => {
    ...
    try { const response = await websitesApi.createGenerationJob(workspaceId, selectedSite.id, prompt, templateChoice); setJob(response.data); setSelectedSite({ ...selectedSite, status: "generating" }); }
  ...
  return <main className="min-h-screen bg-[var(--background)] px-5 py-7 text-foreground sm:px-8 sm:py-10"><div className="mx-auto max-w-[1400px] space-y-6">
    <header className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-primary">Lulu AI / Online Presence</p><h1 className="mt-2 text-4xl font-semibold tracking-[-.05em]">Website & Shop</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">Lulu erstellt, veröffentlicht und betreibt deine Website und deinen Online-Shop als eigenes, verwaltetes System.</p></div><button type="button" onClick={() => void load()} disabled={busy === "refresh"} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-border bg-card px-4 text-sm font-medium hover:bg-secondary disabled:opacity-60"><RefreshCw size={15} className={busy === "refresh" ? "animate-spin" : ""} /> Aktualisieren</button></header>
