...
function PaygOverview({ payg, currentPlan }: { payg: BillingState["payg"]; currentPlan: string | null }) {
  if (!payg) {
    return <section className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 sm:p-6"><div className="flex items-start gap-3"><ShieldCheck size={20} className="mt-0.5 shrink-0" /><div><h2 className="font-semibold">Pay as you go</h2><p className="mt-1 text-sm leading-6 text-[var(--muted-foreground)]">{currentPlan === "viewer" ? "Viewer is read-only and has no usage charges." : "Usage billing starts after a payment method has been confirmed."}</p></div></div></section>;
  }

  const blockedPaymentLink = safeExternalUrl(payg.paymentLink);
  return <section className="mt-6 overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]">
    {payg.aiAccessBlocked && <div className="flex flex-col gap-4 border-b border-red-200 bg-red-50 px-5 py-5 text-red-900 sm:flex-row sm:items-center sm:justify-between sm:px-6" role="alert"><div><p className="text-xs font-bold uppercase tracking-[.14em]">AI access paused</p><h2 className="mt-1 text-lg font-semibold">{payg.blockReason === "PAYMENT_SOURCE_SETUP_REQUIRED" ? "Connect a card to continue using the Test plan." : "The automatic usage payment was not completed."}</h2><p className="mt-1 text-sm leading-6">{payg.blockReason === "PAYMENT_SOURCE_SETUP_REQUIRED" ? "Select Test below, enter the password and complete the secure Airwallex card setup." : "All AI functions stay blocked until the outstanding usage invoice is paid. Non-AI pages remain available."}</p></div>{blockedPaymentLink && <a href={blockedPaymentLink} target="_blank" rel="noreferrer" className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-lg bg-red-700 px-4 text-sm font-semibold text-white">Open payment link<ExternalLink size={14} /></a>}</div>}
    <div className="flex flex-col gap-4 border-b border-[var(--border)] px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      <div><p className="text-xs font-semibold uppercase tracking-[.14em] text-[var(--muted-foreground)]">Pay as you go · every Monday</p><h2 className="mt-2 text-xl font-semibold">Current usage period</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">{formatDate(payg.periodStart)} – {formatDate(payg.periodEnd)} · Automatic collection {formatDate(payg.nextInvoiceAt)}</p></div>
      <span className="rounded-full border border-[var(--border)] bg-[var(--secondary)] px-3 py-1.5 text-xs font-semibold">{payg.collectionMethod === "AUTO_CHARGE" ? "Automatic card debit" : "Card setup required"}</span>
    </div>
    <div className="grid gap-px bg-[var(--border)] sm:grid-cols-3">
      <div className="bg-[var(--card)] p-5 sm:p-6"><div className="flex items-center gap-2 text-sm font-semibold"><Cpu size={17} />API usage</div><p className="mt-4 text-3xl font-semibold tracking-[-0.04em]">{formatMoney(payg.apiCost)}</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">$5/M input · $10/M output · {payg.apiEvents} calls</p></div>
      <div className="bg-[var(--card)] p-5 sm:p-6"><div className="flex items-center gap-2 text-sm font-semibold"><Server size={17} />AWS usage</div><p className="mt-4 text-3xl font-semibold tracking-[-0.04em]">{formatMoney(payg.serverCost)}</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">{payg.serverDays} allocated days · 2× AWS provider cost</p></div>
      <div className="bg-[var(--card)] p-5 sm:p-6"><div className="flex items-center gap-2 text-sm font-semibold"><Sparkles size={17} />Estimated Monday charge</div><p className="mt-4 text-3xl font-semibold tracking-[-0.04em]">{formatMoney(payg.estimatedTotal)}</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">The saved card is charged automatically when the weekly period closes.</p></div>
    </div>
    {payg.invoices.length > 0 && <div className="border-t border-[var(--border)] px-5 py-5 sm:px-6"><h3 className="text-sm font-semibold">Recent usage charges</h3><div className="mt-3 divide-y divide-[var(--border)]">{payg.invoices.map((invoice) => { const hostedUrl = safeExternalUrl(invoice.hostedInvoiceUrl); const pdfUrl = safeExternalUrl(invoice.invoicePdfUrl); const needsPayment = invoice.status === "payment_due" || invoice.status === "payment_failed"; return <div key={invoice.id} className="flex flex-col gap-2 py-3 text-sm first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{formatDate(invoice.periodStart)} – {formatDate(invoice.periodEnd)}</p><p className="mt-0.5 text-xs text-[var(--muted-foreground)]">{invoice.status.replaceAll("_", " ")} · API {formatMoney(invoice.apiCost)} · AWS {formatMoney(invoice.serverCost)}</p></div><div className="flex items-center gap-3"><span className="font-semibold">{formatMoney(invoice.totalCost)}</span>{needsPayment && hostedUrl && <a href={hostedUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-semibold underline underline-offset-4">Open payment link<ExternalLink size={13} /></a>}{pdfUrl && <a href={pdfUrl} target="_blank" rel="noreferrer" className="font-semibold underline underline-offset-4">PDF</a>}</div></div>; })}</div></div>}
  </section>;
}
