...
const lowStock: Array<Record<string, any>> = [];
const outStock: Array<Record<string, any>> = [];
...
export function LuluInventory() {
  ...
  const { items: inventoryRecords, loading, error, refresh: refreshInventory } = useLiveRecords('ecommerce_inventory');
  const [refreshing, setRefreshing] = React.useState(false);
  const refresh = () => { void refreshInventory(); setRefreshing(true); window.setTimeout(() => setRefreshing(false), 700); };
  ...
  if (loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live inventory records…</main>;
  ...
  if (!loading && !error) return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-6xl"><header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Ecommerce / Inventory</p><h1 className="mt-2 text-3xl font-bold">Inventory</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Verified inventory records from connected stores. Stock levels and warnings appear only when returned by the backend.</p></div><span className="inline-flex items-center rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground">Use Update in the navigation bar</span></header>{inventoryRecords.length === 0 ? <section className="rounded-2xl border border-dashed border-border bg-card p-10 text-center"><Package className="mx-auto mb-4 text-muted-foreground" size={30} /><h2 className="text-xl font-semibold">No verified inventory records yet</h2><p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">Connect a store and synchronize product inventory before reviewing stock levels or inventory warnings.</p></section> : <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Item</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Stage</th><th className="px-4 py-3">Description</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{inventoryRecords.map(record => <tr key={record.id}><td className="px-4 py-3 font-medium">{record.name}</td><td className="px-4 py-3">{record.status}</td><td className="px-4 py-3 text-muted-foreground">{record.stage ?? '—'}</td><td className="max-w-md px-4 py-3 text-muted-foreground">{record.description ?? '—'}</td><td className="px-4 py-3 text-muted-foreground">{new Date(record.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>}</div></main>;
  ...
  <section className="min-w-0">
   ...
   <div className="space-y-5 p-5 lg:p-8">
    ...
    <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border/25 border-l-4 border-l-border bg-[var(--secondary)] px-4 py-3 shadow-[0_0_25px_rgba(0,0,0,.06)]"><AlertTriangle size={18} className="text-foreground" /><div className="flex flex-1 flex-wrap gap-2">{['Premium Hoodie — Black/XL — Out of Stock — Connected store', 'Sync Failure — WooCommerce — 3 records', 'Low Stock Alert — 12 SKUs below threshold'].map(alert => <span key={alert} className="rounded-md border border-border/15 bg-secondary/[.06] px-2.5 py-1.5 text-xs text-foreground">{alert}<button className="ml-2 text-foreground/70 hover:text-foreground"><X size={12} /></button></span>)}</div></div>
