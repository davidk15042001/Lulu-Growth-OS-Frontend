...
export function WorkspaceIntelligencePanel({ workspaceId }: Props) {
  ...
  return <section className="mb-7 rounded-xl border border-border bg-card p-4 sm:p-6">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      ...
      <button type="button" onClick={() => void startRefresh()} disabled={refreshing} aria-label="Update workspace content" className="inline-flex w-full shrink-0 items-center justify-center gap-2 rounded-md border border-border px-3 py-2 text-xs text-foreground hover:bg-secondary disabled:cursor-wait disabled:opacity-60 sm:w-auto"><RefreshCw className={refreshing ? 'animate-spin' : ''} size={13} /> {refreshing ? `Updating ${refreshJob?.progress ?? 0}%` : 'Update workspace'}</button>
    </div>
    {refreshJob && <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2 text-xs text-muted-foreground">{refreshJob.status === 'completed' ? 'Workspace content is up to date.' : refreshJob.status === 'failed' ? (refreshJob.errorMessage || 'Workspace update failed.') : `Updating ${refreshJob.currentPhase} · ${refreshJob.progress}%`}</div>}
