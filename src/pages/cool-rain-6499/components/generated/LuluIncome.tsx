...
export function LuluIncome() {
  ...
  <aside className="w-[238px] shrink-0 bg-[var(--background)] text-foreground flex flex-col min-h-screen p-4 fixed inset-y-0 left-0 z-10"><div className="flex items-center gap-3 px-3 py-3 mb-5"><div className="h-8 w-8 rounded-xl bg-primary flex items-center justify-center shadow-lg shadow-black/40 text-primary-foreground"><Sparkles size={17} /></div><div><strong className="text-[16px] tracking-tight">lulu<span className="text-foreground">.</span></strong><p className="text-[9px] text-muted-foreground tracking-[.18em] uppercase">AI workspace</p></div></div><nav className="space-y-1 text-[12px] font-medium">{navItems.map(item => <button key={item} className="w-full flex items-center gap-3 px-3 py-2.5 text-foreground hover:text-foreground hover:bg-secondary rounded-lg"><Activity size={15} /><span>{item}</span></button>)}<div className="pt-4 pb-1 px-3 text-[10px] tracking-[.15em] uppercase text-muted-foreground">Workspace</div><button className="w-full flex items-center gap-3 px-3 py-2.5 text-foreground bg-secondary/15 rounded-lg"><Wallet size={15} className="text-foreground" /><span>Finance</span><ChevronDown size={14} className="ml-auto" /></button><div className="pl-7 border-l border-border ml-5 mt-1 space-y-0.5">{financeItems.map(item => <button key={item} className={`w-full text-left px-3 py-2 rounded-md text-[11px] ${item === 'Income' ? 'bg-primary text-primary-foreground font-semibold' : 'text-primary-foreground hover:text-primary-foreground hover:bg-secondary'}`}>{item}</button>)}</div><button className="w-full flex items-center gap-3 px-3 py-2.5 mt-3 text-foreground hover:text-foreground rounded-lg"><Settings size={15} /><span>Settings</span></button></nav><div className="mt-auto p-3 rounded-xl bg-secondary border border-border"><div className="flex items-center gap-2"><div className="h-7 w-7 rounded-full bg-primary text-primary-foreground text-[11px] font-bold flex items-center justify-center">JD</div><div><p className="text-[11px] font-semibold">Jordan Davis</p><p className="text-[10px] text-muted-foreground">Administrator</p></div><MoreHorizontal size={15} className="ml-auto text-muted-foreground" /></div></div></aside>
