...
export const LuluFinance = () => {
  const { items, loading, error } = useLiveRecords('finance_transactions');
  return <div className="flex min-h-screen bg-[var(--background)]"><aside className="sticky top-0 hidden h-screen w-60 shrink-0 flex-col overflow-hidden border-r border-border bg-[var(--sidebar)] p-4 lg:flex"><div className="mb-5 flex items-center gap-3 px-2 py-3"><div className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-lg font-bold text-primary-foreground">L</div><span className="font-semibold text-foreground">Lulu AI</span></div><LuluSectionNavigation activeId="cozily-path-5612" /></aside><div className="min-w-0 flex-1"><main className="finance-shell"><header className="header"><div><p className="crumb">Finance <b>/</b> Overview</p><h1>Finance</h1><p className="subtitle">Live financial intelligence from connected workspace sources.</p></div><div className="actions"><button className="btn primary"><Sparkles size={14} />Ask Lulu AI</button><span className="inline-flex items-center rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground">Use Update in the navigation bar</span></div></header><section className="section"><div className="card" style={{ minHeight: 420, display: 'grid', placeItems: 'center', textAlign: 'center' }}><div>{loading ? <h2>Loading live finance data…</h2> : error ? <h2>Finance data could not be loaded</h2> : <h2>No finance data available yet</h2>}<p className="definition">{items.length ? `${items.length} live finance records are available.` : 'Connect a finance system to populate revenue, costs, margins and cash flow. No example metrics are displayed.'}</p></div></div></section></main></div></div>;
};
