  ...
  return <div className="mx-auto max-w-[1500px] space-y-6">
    <header className="flex flex-wrap items-end justify-between gap-5">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button type="button" onClick={onCreate} disabled={!canWrite} className="lulu-workspace-primary-action inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium disabled:opacity-40"><Plus size={16}/>Unternehmen</button>
      </div>
    </header>
...
export default function CrmWorkspacePage({ kind, showEntitySwitcher = true }: { kind: Kind; showEntitySwitcher?: boolean }) {
  ...
  return <WorkspaceSurfaceShell activeSlug={activeSlug}><main className="page-frame min-h-screen min-w-0 overflow-x-clip bg-[var(--background)] p-4 sm:p-8">
    ...
    </div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => { setModal(false); setEditing(null); }} className="rounded-xl border border-[var(--border)] px-4 py-2 text-sm">Abbrechen</button><button type="button" onClick={() => void save()} disabled={busy} className="lulu-workspace-primary-action rounded-xl px-4 py-2 text-sm disabled:opacity-40">{editing ? 'Speichern & neu anreichern' : 'Speichern'}</button></div></Modal>}
    {importOpen && canWrite && <Modal title={`Import ${title}`} onClose={() => setImportOpen(false)}><p className="text-sm text-[var(--muted-foreground)]">CSV, PDF oder Excel-Datei auswählen. Lulu verarbeitet und vervollständigt die Einträge autonom.</p><input ref={fileRef} type="file" accept=".csv,.xlsx,.xls,.pdf,.txt" onChange={(event) => void importFile(event.target.files?.[0])} className="mt-4 block w-full text-sm"/><div className="mt-5 flex justify-end"><button type="button" onClick={() => fileRef.current?.click()} disabled={busy} className="lulu-workspace-primary-action inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm disabled:opacity-40"><FileUp size={15}/>Datei auswählen</button></div></Modal>}
