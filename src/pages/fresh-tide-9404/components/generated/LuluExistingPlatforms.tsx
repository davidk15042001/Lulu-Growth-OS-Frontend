...
const providerKeysByName: Record<string, string> = {
  ...
  WhatsApp: 'twilio', 'Facebook Messenger': 'twilio', Instagram: 'instagram', LinkedIn: 'linkedin',
};
  const guideContent: Record<string, { intro: string; steps: string[]; links?: Array<{ label: string; url: string }> }> = {
    ...
    WhatsApp: { intro: "Lulu uses Twilio Programmable Messaging for WhatsApp delivery and verified inbound webhooks.", steps: ["Complete the WhatsApp Business sender onboarding required by Twilio and Meta.", "Provide or select the approved WhatsApp sender for this workspace.", "Lulu registers the sender against this workspace and verifies the webhook.", "Messages then enter the autonomous OmniChannel workflow."] },
...
export const LuluExistingPlatforms = () => {
  ...
  const guideIsManagedMessaging = guidePlatform === 'WhatsApp' || guidePlatform === 'Facebook Messenger';
            ...
            <div className="space-y-5">
              ...
              return <section key={group.id} className={`rounded-2xl border border-[var(--border)] bg-[var(--secondary)]/35 p-4 shadow-sm sm:p-5 ${comingSoon ? 'opacity-70' : ''}`} aria-labelledby={`${group.id}-heading`} aria-disabled={comingSoon}>
                  ...
                  <div className={`mt-5 grid gap-3 md:grid-cols-2 ${comingSoon ? 'pointer-events-none' : ''}`}>
                    ...
                    const luluManagedMessaging = name === 'WhatsApp' || name === 'Facebook Messenger';
                    const needsAdminApproval = ['instagram', 'linkedin'].includes(provider);
