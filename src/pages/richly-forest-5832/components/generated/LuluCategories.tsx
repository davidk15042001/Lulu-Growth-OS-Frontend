...
export function LuluCategories() {
  ...
  const { items: categoryRecords, loading, error, refresh: refreshCategories } = useLiveRecords('ecommerce_categories');
  const [refreshing, setRefreshing] = React.useState(false);
  const refresh = () => { void refreshCategories(); setRefreshing(true); window.setTimeout(() => setRefreshing(false), 700); };
  if (loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live category records…</main>;
  ...
  return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground"><div className="mx-auto max-w-6xl"><header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Ecommerce / Categories</p><h1 className="mt-2 text-3xl font-bold">Categories</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Verified product category records from connected stores. Hierarchy and product counts appear only when returned by the backend.</p></div><button type="button" onClick={refresh} className="inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm hover:bg-secondary"><RefreshCw size={15} className={refreshing ? 'animate-spin' : ''} />Refresh</button></header>{categoryRecords.length === 0 ? <section className="rounded-2xl border border-dashed border-border bg-card p-10 text-center"><Tags className="mx-auto text-muted-foreground" size={30} /><h2 className="mt-4 text-xl font-semibold">No verified category records yet</h2><p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">Connect a store and synchronize product categories before reviewing hierarchy or product assignments. No example categories are displayed.</p></section> : <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Category</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Stage</th><th className="px-4 py-3">Description</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{categoryRecords.map(record => <tr key={record.id}><td className="px-4 py-3 font-medium">{record.name}</td><td className="px-4 py-3">{record.status}</td><td className="px-4 py-3 text-muted-foreground">{record.stage ?? '—'}</td><td className="max-w-md px-4 py-3 text-muted-foreground">{record.description ?? '—'}</td><td className="px-4 py-3 text-muted-foreground">{new Date(record.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>}</div></main>;
}
