...
const insights: Insight[] = [{
  icon: BarChart3,
  title: 'AI usage increased 18% this billing period',
  detail: 'A steady rise in automated workflows is driving healthy adoption.'
}, {
  icon: Bot,
  title: 'AI Agents account for the largest share of AI consumption at 28%',
  detail: 'Agent activity remains your most active AI surface.'
}, {
  icon: Gauge,
  title: 'Your organization is using only 34% of included API capacity',
  detail: 'There is room to scale your integrations without added cost.'
}];
...
const activeAddOns = [{
  ...
  price: '€20 / month',
  renews: 'Renews September 1, 2026'
}, {
  id: 'priority-support',
...
const comparisonPlans = [{
  ...
  price: '€99',
  cadence: '/mo',
  features: ['5 Users', '20 AI Hours', '10,000 Credits'],
  action: 'Downgrade'
}, {
  id: 'business',
  name: 'Business',
  price: '€299',
  cadence: '/mo',
  features: ['25 Users', '100 AI Hours', '50,000 Credits'],
  current: true
}, {
  id: 'professional',
  name: 'Professional',
  price: '€499',
  cadence: '/mo',
  features: ['75 Users', '300 AI Hours', '150,000 Credits'],
  action: 'Upgrade'
}, {
  id: 'enterprise',
  name: 'Enterprise',
  ...
  cadence: '',
  features: ['Unlimited Users', 'Custom AI'],
...
export function LuluBilling() {
  ...
  return <main className="min-h-screen bg-[var(--background)] text-foreground">
      <aside className="fixed inset-y-0 left-0 hidden w-[228px] border-r border-[var(--muted-foreground)] bg-[var(--sidebar)] px-4 py-5 lg:block">
        <div className="mb-10 flex items-center gap-3 px-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-sm font-bold text-primary-foreground">L</div><span className="text-[15px] font-semibold tracking-tight text-foreground">lulu <span className="font-normal text-muted-foreground">AI</span></span></div>
        ...
        <nav className="space-y-1" aria-label="Primary navigation">
          ...
          return <button key={item.label} type="button" className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition ${isActive ? 'bg-secondary/12 text-foreground' : 'text-foreground hover:bg-secondary hover:text-foreground'}`}><Icon size={16} strokeWidth={1.8} /><span>{item.label}</span>{isActive && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary text-primary-foreground" />}</button>;
        })}
        </nav>
        <div className="mt-auto absolute bottom-5 left-4 right-4 space-y-1"><button type="button" className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-foreground hover:text-foreground"><Settings size={16} /><span>Settings</span></button><div className="mt-4 flex items-center gap-3 border-t border-[var(--muted-foreground)] px-2 pt-4"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--secondary)] text-xs font-semibold text-foreground">AS</div><div className="min-w-0"><p className="truncate text-xs font-medium text-foreground">Acme Systems</p><p className="truncate text-[11px] text-muted-foreground">admin@acme.io</p></div><ChevronDown size={14} className="ml-auto text-muted-foreground" /></div></div>
      </aside>

      <section className="lg:ml-[228px]">
        ...
        {activeTab === 'current-plan' && <div className="space-y-5 px-5 py-6 sm:px-8 lg:px-10 lg:py-8">
          ...
          <article className="rounded-xl border border-[var(--muted-foreground)] bg-[var(--card)] p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-sm font-semibold text-foreground">Recent Billing Activity</h3><p className="mt-1 text-xs text-muted-foreground">Representative sample billing events for this workspace.</p></div><FileText size={17} className="text-muted-foreground" /></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><thead className="border-b border-[var(--muted-foreground)] text-[10px] uppercase tracking-wider text-muted-foreground"><tr><th className="pb-3 font-medium">Date</th><th className="pb-3 font-medium">Activity</th><th className="pb-3 text-right font-medium">Amount</th><th className="pb-3 text-right font-medium">Status</th></tr></thead><tbody className="divide-y divide-[var(--foreground)]">{billingActivity.map(activity => <tr key={activity.id}><td className="py-3 text-muted-foreground">{activity.date}</td><td className="py-3 font-medium text-foreground">{activity.event}</td><td className="py-3 text-right text-foreground">{activity.amount}</td><td className="py-3 text-right"><span className="inline-flex items-center gap-1.5 rounded-full bg-secondary/10 px-2 py-1 text-[10px] font-semibold text-foreground"><Check size={11} /><span>{activity.status}</span></span></td></tr>)}</tbody></table></div><button type="button" className="mt-5 flex items-center gap-1 text-xs font-medium text-foreground hover:text-foreground">View All Billing History <ArrowRight size={13} /></button></article>
        </div>}
        ...
        {activeTab === 'overview' && <div className="space-y-5 px-5 py-6 sm:px-8 lg:px-10 lg:py-8">
          ...
          <div className="grid gap-4 xl:grid-cols-4">
            ...
            <article className="rounded-xl border border-[var(--muted-foreground)] bg-[var(--card)] p-5"><div className="flex items-start justify-between"><p className="text-xs text-muted-foreground">Renewal</p><Zap size={17} className="text-foreground" /></div><p className="mt-4 text-lg font-semibold text-foreground">September 1, 2026</p><p className="mt-1 text-xs text-foreground">14 days remaining</p><div className="mt-5 space-y-2 text-xs"><div className="flex justify-between"><span className="text-muted-foreground">Payment method</span><span className="text-foreground">Visa ···· 4242</span></div><div className="flex justify-between"><span className="text-muted-foreground">Renewal amount</span><span className="text-foreground">~€437.92</span></div></div><button type="button" className="mt-5 w-full rounded-md border border-[var(--muted-foreground)] py-2 text-xs text-foreground hover:border-border/60 hover:text-foreground">Update Payment</button></article>
          </div>
