...
export const LuluGrowth = () => {
  ...
  <aside className={mobileNav ? 'sidebar mobile-open' : 'sidebar'}><div className="brand"><div className="brand-mark">✦</div><span>Lulu AI</span><strong>GROWTH</strong><button className="close-nav" onClick={() => setMobileNav(false)} aria-label="Close navigation"><X size={18} /></button></div><p className="workspace">WORKSPACE</p><nav aria-label="Main navigation">{nav.map(item => {
          const Icon = item.icon;
          return <button key={item.label} className={item.active ? 'nav-item active' : 'nav-item'}><Icon size={17} /><span>{item.label}</span>{item.label === 'Business Intelligence' ? <ChevronDown size={14} className="nav-chevron" /> : null}</button>;
        })}</nav><div className="sidebar-bottom"><button className="nav-item"><Settings size={17} /><span>Settings</span></button><div className="profile"><div className="avatar">AR</div><div><strong>Alex Rivera</strong><small>Administrator</small></div><MoreHorizontal size={17} /></div></div></aside>
