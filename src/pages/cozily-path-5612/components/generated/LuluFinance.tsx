...
export const LuluFinance = () => {
  ...
  return <div className="flex min-h-screen bg-[var(--background)]"><aside className="hidden w-60 shrink-0 flex-col border-r border-border bg-[var(--sidebar)] p-4 lg:flex"><div className="mb-5 flex items-center gap-3 px-2 py-3"><div className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-lg font-bold text-primary-foreground">L</div><span className="font-semibold text-foreground">Lulu AI</span></div><LuluSectionNavigation activeId="cozily-path-5612" /></aside><div className="min-w-0 flex-1"><main className="finance-shell">
    ...
    <Section title="Financial Performance Score"><div className="card score"><div className="score-gauge"><div className="score-ring"><strong>—</strong><span>Live</span></div><div className="score-copy"><strong>Awaiting live data</strong><span>Financial health appears after connection</span></div></div><div><div className="bars">{breakdown.map(([name, value]) => <div className="bar-row" key={name}><span>{name}</span><div className="bar-track"><div className="bar-fill" style={{
                      width: value
                    }} /></div><b>{value}</b></div>)}</div><p className="ai-note"><AlertTriangle size={13} className="tone-brass" /> Financial assessment appears after live data is connected.</p></div></div></Section>
    <Section title="Revenue Performance & Trend"><div className="two-col"><article className="card chart-card"><div className="chart-head"><div><h3>Revenue Trend</h3><p className="caption">Performance appears after live financial data is connected.</p></div><div className="chips">{['Daily', 'Weekly', 'Monthly', 'Quarterly'].map(item => <button className={`chip ${period === item ? 'active' : ''}`} key={item} onClick={() => setPeriod(item)}>{item}{period === item ? ' ✓' : ''}</button>)}</div></div><svg className="line-chart" viewBox="0 0 660 150" preserveAspectRatio="none" role="img" aria-label="Revenue trend area chart"><path d={`${chartPath} L660,150 L0,150 Z`} fill="var(--chart-1)" opacity=".14" /><path d={chartPath} fill="none" stroke="var(--chart-1)" strokeWidth="3" /><path d="" fill="none" stroke="var(--chart-1)" strokeDasharray="5 5" /><path d="" fill="none" stroke="var(--chart-1)" strokeDasharray="2 6" opacity=".7" /></svg><div className="chart-legend"><span className="legend">Live target data required</span><span className="legend">Live comparison data required</span></div><div className="tabs">{['Revenue', 'Gross Profit', 'Net Profit'].map(item => <button className={activeSeries === item ? 'active' : ''} key={item} onClick={() => setActiveSeries(item)}>{item}</button>)}</div></article><article className="card"><h3>Revenue Composition</h3><p className="caption">Current period totals appear after live data is connected.</p><div className="donut-wrap"><div className="donut" aria-label="Revenue composition chart awaiting live data" /><div className="segment-list">{segments.map(item => <div key={item.name}><span><i style={{
    ...
    <Section title="Recurring Revenue"><div className="metric-strip">{([] as string[][]).map(([a, b, c]) => <article className="card" key={a}><span className="kpi-label">{a}</span><div className={`metric-value ${a === 'Churned Revenue' ? 'tone-red' : 'tone-brass'}`}>{b}</div><span className="caption">{c}</span></article>)}</div><p className="definition">Recurring revenue metrics reflect connected subscription data only. Non-subscription revenue not included.</p></Section>
    <Section title="Cost Performance & Profitability Waterfall"><div className="two-col"><article className="card"><h3>Profitability Waterfall</h3><p className="caption">How revenue becomes net profit</p><div className="waterfall">{([] as string[][]).map(([name, value, color, height]) => <div className="wf-item" key={name}><div className={`wf-bar ${color}`} style={{
                    height
                  }}><span>{value}</span></div><span className="wf-label">{name}</span></div>)}</div><p className="definition">Profitability metrics appear after live financial data is connected.</p></article><article className="card"><h3>Cost Breakdown</h3><p className="caption">OPEX by category · % of revenue</p><div className="mini-bars">{costs.map(([name, amount, share, growth]) => <div key={name}><div className="mini-bar-head"><span>{name}</span><span>{amount} · {share} <em className="tone-brass">{growth}</em></span></div><div className="mini-track"><div className="mini-fill" style={{
                      width: share
                    }} /></div></div>)}</div></article></div></Section>
    <Section title="Gross Margin & Operating Margin"><div className="three-col"><article className="card margin-card"><h3>Gross Margin Analysis</h3><div className="metric-value tone-brass">—</div><p className="caption">Overall gross margin</p><div className="mini-bars">{([] as string[][]).map(([a, b]) => <div key={a}><div className="mini-bar-head"><span>{a}</span><span>{b}</span></div><div className="mini-track"><div className="mini-fill" style={{
                      width: b
                    }} /></div></div>)}</div><p className="definition">Margin trend appears after live financial data is connected.</p></article><article className="card margin-card"><h3>OPEX Trend</h3><p className="caption">Monthly operating expenses</p><svg className="line-chart" viewBox="0 0 420 150" preserveAspectRatio="none"><path d="" fill="var(--chart-3)" opacity=".13" /><path d="" fill="none" stroke="var(--chart-3)" strokeWidth="2.5" /></svg><div className="chips"><button className="chip active">Absolute $</button><button className="chip">% of Revenue</button></div><p className="definition">OPEX trend appears after live financial data is connected.</p></article><article className="card margin-card"><h3>Operating & Net Margin</h3><div className="dial-row"><Gauge value="" label="Operating Margin" /><Gauge value="" label="Net Margin" color="var(--foreground)" /></div><table className="table"><thead><tr><th>Metric</th><th>Current</th><th>Target</th></tr></thead><tbody><tr><td>Operating</td><td>—</td><td>—</td></tr><tr><td>Net</td><td>—</td><td>—</td></tr></tbody></table></article></div></Section>
    <Section title="Profitability by Segment"><div className="three-col"><article className="card"><div className="chart-head"><h3>Product Profitability</h3><a className="kpi-explorer" href="#products">→ Products</a></div><div className="table-wrap"><table className="table"><thead><tr><th>Product</th><th>Revenue</th><th>Direct Costs</th><th>Gross Profit</th><th>Margin</th></tr></thead><tbody>{tableProducts.map(row => <tr key={row[0]}>{row.map((cell, i) => <td className={i === 4 ? 'tone-brass' : ''} key={`${row[0]}-${cell}`}>{cell}</td>)}</tr>)}</tbody></table></div></article><article className="card"><h3>Customer Profitability</h3><div className="table-wrap"><table className="table"><thead><tr><th>Segment</th><th>Revenue</th><th>Margin</th><th>Contribution</th></tr></thead><tbody>{([] as string[][]).map(row => <tr key={row[0]}>{row.map(cell => <td key={`${row[0]}-${cell}`}>{cell}</td>)}</tr>)}</tbody></table></div><p className="definition">Customer-level data shown per permissions.</p></article><article className="card"><h3>Market Profitability</h3><div className="table-wrap"><table className="table"><thead><tr><th>Market</th><th>Revenue</th><th>Costs</th><th>Margin</th></tr></thead><tbody>{([] as string[][]).map(row => <tr key={row[0]}>{row.map((cell, i) => <td className={i === 3 ? 'tone-brass' : ''} key={`${row[0]}-${cell}`}>{cell}</td>)}</tr>)}</tbody></table></div></article></div></Section>
    <Section eyebrow="CASH" title="Cash Position & Cash Flow"><div className="cash-summary">{([] as string[][]).map(([a, b, c]) => <article className="card" key={a}><span className="kpi-label">{a}</span><div className={`metric-value tone-${c}`}>{b}</div></article>)}</div><div className="two-col" style={{
            marginTop: 14
          }}><article className="card"><h3>Cash Flow Trend</h3><p className="caption">Cash-flow trend appears after live data is connected.</p><div className="cash-flow-chart">{months.map((month, i) => <div key={month} style={{
                  flex: 1
                ...
                marginTop: 12
                    ...
                    background: 'var(--primary)'
                  }} />Net CF</span></div></article><div className="breakdown-stack">{([] as string[][]).map(([a, b, c, d]) => <article className="card" key={a}><h3 className={`tone-${d}`}>{a} <span style={{
                    float: 'right'
                  }}>{b}</span></h3><p className="definition">{c}</p></article>)}</div></div></Section>
    <Section title="Cash Runway & Burn Rate"><div className="two-col"><article className="card"><h3>Cash Runway</h3><div className="runway"><strong>— <span>months</span></strong><span className="caption">Awaiting live data</span></div><div className="runway-track"><div className="runway-marker" /></div><div className="target-foot"><span>Runway threshold</span><span>Live cash data required</span></div><p className="definition">Cash runway appears after live cash-flow data is connected.</p></article><article className="card"><h3>Burn Rate</h3><svg className="line-chart" viewBox="0 0 600 150" preserveAspectRatio="none"><path d="" fill="none" stroke="var(--chart-1)" strokeWidth="2.5" /><path d="" fill="none" stroke="var(--chart-4)" strokeWidth="2.5" /></svg><div className="metric-strip"><span>Gross <b>—</b></span><span>Net <b className="tone-green">—</b></span></div><p className="definition">Gross Burn = total cash out. Net Burn = cash out minus cash in from operations.</p></article></div></Section>
    <Section title="Accounts Receivable & Payable"><div className="two-col"><article className="card"><div className="chart-head"><h3>Accounts Receivable</h3><strong className="tone-brass">—</strong></div><div className="aging" /><table className="table"><tbody>{([] as string[][]).map(row => <tr key={row[0]}><td>{row[0]}</td><td>{row[1]}</td><td className={row[3]}>{row[2]}</td></tr>)}</tbody></table><p className="definition">Receivables aging appears after live finance data is connected.</p></article><article className="card"><div className="chart-head"><h3>Accounts Payable</h3><strong className="tone-blue">—</strong></div><div className="aging" /><table className="table"><tbody>{([] as string[][]).map(row => <tr key={row[0]}><td>{row[0]}</td><td>{row[1]}</td><td className="tone-blue">{row[2]}</td></tr>)}</tbody></table><p className="definition">Payables aging appears after live finance data is connected.</p></article></div></Section>
    <Section title="Working Capital"><div className="card working"><div><span className="kpi-label">Current Assets</span><div className="metric-value">—</div></div><div><span className="kpi-label">Current Liabilities</span><div className="metric-value">—</div></div><div><span className="kpi-label">Working Capital</span><div className="big tone-green">—</div></div><div><span className="kpi-label">Working Capital Ratio</span><div className="big tone-brass">—</div></div><p className="definition">Working Capital = Current Assets − Current Liabilities.</p><Spark points="" tone="green" /></div></Section>
    <Section title="Financial Efficiency & Unit Economics"><div className="two-col"><article className="card"><h3>Financial Efficiency</h3><div className="eff-grid">{([] as string[][]).map(([a, b]) => <div key={a}><span className="caption">{a}</span><strong>{b}</strong></div>)}</div><p className="definition">Efficiency metrics appear after live finance data is connected.</p></article><article className="card"><h3>Unit Economics</h3><div className="table-wrap"><table className="table"><tbody>{([] as string[][]).map(row => <tr key={row[0]}><td>{row[0]}</td><td><strong>{row[1]}</strong></td><td><span className="tag">{row[2]}</span></td></tr>)}</tbody></table></div><p className="definition">Unit economics appear after live customer and finance data is connected.</p></article></div></Section>
    <Section eyebrow="BUDGET VS ACTUAL"><article className="card"><div className="table-wrap"><table className="table"><thead><tr><th>Category</th><th>Budget</th><th>Actual</th><th>Variance $</th><th>Variance %</th><th>Status</th></tr></thead><tbody>{([] as string[][]).map(row => <tr key={row[0]}>{row.map((cell, i) => <td key={`${row[0]}-${cell}`} className={i === 3 || i === 4 ? 'tone-brass' : ''}>{i === 5 ? <span className={`status ${cell === 'On Budget' ? 'warn' : ''}`}>{cell}</span> : cell}</td>)}</tr>)}</tbody></table></div><p className="definition">Budgets appear after live finance data is connected.</p></article></Section>
    <Section title="Financial Targets, Gap & Benchmarks"><div className="three-col target-grid"><article className="card"><h3>Financial Targets</h3>{([] as string[][]).map(row => <div key={row[0]} style={{
                marginTop: 14
              }}><div className="mini-bar-head"><span>{row[0]} <b>{row[1]}</b></span><span>{row[2]}</span></div><div className={`progress ${row[4]}`}><b style={{
                    width: row[2]
                  }} /></div><div className="target-foot"><span>{row[3]}</span><span>Dec 31</span></div></div>)}<p className="definition">Targets appear after workspace targets are configured.</p></article><article className="card"><h3>Financial Gap</h3><table className="table"><thead><tr><th>Metric</th><th>Target</th><th>Actual</th><th>Gap</th></tr></thead><tbody>{([] as string[][]).map(r => <tr key={r[0]}>{r.map(c => <td key={`${r[0]}-${c}`}>{c}</td>)}</tr>)}</tbody></table></article><article className="card"><h3>Financial Benchmarks</h3><table className="table"><thead><tr><th>Metric</th><th className="tone-brass">Yours</th><th>Internal</th><th>Industry</th></tr></thead><tbody>{benchmarks.map(r => <tr key={r[0]}><td>{r[0]}</td><td className="tone-green">{r[1]}</td><td>{r[2]}</td><td>{r[3]}</td></tr>)}</tbody></table></article></div></Section>
    <Section title="Financial Trends"><article className="card"><div className="chips">{['Revenue', 'Gross Profit', 'Net Profit', 'OPEX', 'Gross Margin', 'Cash Flow', 'Receivables', 'Payables'].map(item => <button className={`chip ${activeSeries === item ? 'active' : ''}`} key={item} onClick={() => setActiveSeries(item)}>{item}{activeSeries === item ? ' ✓' : ''}</button>)}</div><svg className="line-chart" viewBox="0 0 900 180" preserveAspectRatio="none"><path d="" fill="var(--chart-1)" opacity=".15" /><path d="" fill="none" stroke="var(--chart-1)" strokeWidth="3" /><path d="" fill="none" stroke="var(--muted-foreground)" strokeDasharray="4 5" /></svg><div className="chart-legend"><span>Trend appears after live financial data is connected · {activeSeries}</span><span>Comparison overlay available</span></div></article></Section>
    <Section title="Financial Anomalies"><article className="card"><div className="table-wrap"><table className="table"><thead><tr><th>Metric</th><th>Date</th><th>Category</th><th>Observed</th><th>Expected Range</th><th>Difference</th><th></th></tr></thead><tbody>{([] as string[][]).map(r => <tr className="anomaly" key={r[0]}>{r.map(c => <td key={`${r[0]}-${c}`}>{c}</td>)}<td><button className="chip">View</button></td></tr>)}</tbody></table></div><a className="kpi-explorer" style={{
              float: 'none',
              display: 'block',
              marginTop: 15
