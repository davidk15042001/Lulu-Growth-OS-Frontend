  ...
  return <div className="min-h-screen bg-[var(--background)] text-[var(--primary-foreground)] antialiased lg:flex">
      <aside className={`fixed inset-y-0 left-0 z-30 flex w-[246px] flex-col bg-[var(--sidebar)] text-foreground shadow-[20px_0_60px_rgba(0,0,0,.34)] transition-transform lg:sticky lg:top-0 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-5" aria-label="Primary navigation">
          ...
          label,
          icon: Icon
        }) => <button key={label} className="mb-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] font-medium text-foreground transition hover:bg-secondary hover:text-foreground"><Icon size={16} /><span>{label}</span></button>)}
          <div className="mt-4"><div className="flex items-center gap-3 px-3 pb-2 text-[13px] font-bold text-foreground"><ShoppingBag size={16} /><span>Ecommerce</span><ChevronDown size={14} className="ml-auto text-muted-foreground" /></div><div className="ml-3 border-l border-border pl-3">{ecommerceNavItems.map(item => <button key={item.label} className={`mb-0.5 flex w-full items-center rounded-md px-3 py-2 text-[12px] font-medium transition ${item.label === 'Carts' ? 'bg-[var(--primary)] font-extrabold text-primary-foreground shadow-[0_0_20px_rgba(0,0,0,.4)]' : 'text-foreground hover:bg-secondary hover:text-primary-foreground'}`}><span>{item.label}</span></button>)}</div></div>
          <div className="mt-4 border-t border-border pt-4">{secondaryNavItems.map(item => <button key={item.label} className="mb-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] font-medium text-foreground hover:bg-secondary hover:text-foreground"><span>{item.label === 'Reports' ? <FileBarChart size={16} /> : <Settings size={16} />}</span><span>{item.label}</span></button>)}</div>
        </nav>
