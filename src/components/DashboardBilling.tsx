...
function PaygOverview({ payg, currentPlan }: { payg: BillingState["payg"]; currentPlan: string | null }) {
  if (!payg) {
    return <section className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 sm:p-6"><div className="flex items-start gap-3"><ShieldCheck size={20} className="mt-0.5 shrink-0" /><div><h2 className="font-semibold">Pay as you go</h2><p className="mt-1 text-sm leading-6 text-[var(--muted-foreground)]">{currentPlan === "viewer" ? "Viewer is read-only and has no usage charges." : currentPlan === "test" ? "The Test package includes full AI access without billing." : "Storage usage is invoiced by checkout link for explicit payment."}</p></div></div></section>;
  }
  ...
  return <section className="mt-6 overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]">
    <div className="flex flex-col gap-4 border-b border-[var(--border)] px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      <div><p className="text-xs font-semibold uppercase tracking-[.14em] text-[var(--muted-foreground)]">Cloudflare R2 · weekly invoice</p><h2 className="mt-2 text-xl font-semibold">Current storage period</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">{formatDate(payg.periodStart)} – {formatDate(payg.periodEnd)} · next invoice {formatDate(payg.nextInvoiceAt)}</p></div>
      <span className="rounded-full border border-[var(--border)] bg-[var(--secondary)] px-3 py-1.5 text-xs font-semibold">Manual checkout</span>
    </div>
