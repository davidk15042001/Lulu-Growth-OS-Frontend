...
export function ReturnsRefundsPage() {
  ...
  return <main className="min-h-screen bg-[var(--background)] text-foreground selection:bg-[var(--primary)]/30">
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-[236px] border-r border-[var(--muted-foreground)] bg-[var(--sidebar)] lg:flex lg:flex-col">
      ...
      <nav className="flex-1 space-y-1 px-3 py-6"><p className="px-3 pb-3 text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted-foreground)]">Workspace</p>{nav.map(item => {
          const Icon = item.icon;
          const isEcommerce = item.label === 'Ecommerce';
          return <div key={item.label}><button className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${isEcommerce ? 'text-foreground' : 'text-[var(--muted-foreground)] hover:bg-secondary hover:text-foreground'}`}><Icon className="h-4 w-4" /><span>{item.label}</span>{isEcommerce && <ChevronDown className="ml-auto h-3.5 w-3.5" />}</button>{isEcommerce && <div className="ml-5 border-l border-[var(--muted-foreground)] pl-4"><button className="my-1 flex w-full items-center rounded-md border border-[var(--border)]/30 bg-[var(--primary)]/15 px-3 py-2 text-left text-xs font-bold text-[var(--foreground)] shadow-[0_0_18px_rgba(0,0,0,0.16)]">Returns & Refunds</button><button className="w-full px-3 py-2 text-left text-xs text-[var(--muted-foreground)] hover:text-foreground">Store Performance</button></div>}</div>;
        })}</nav>
