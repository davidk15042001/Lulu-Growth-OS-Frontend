  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground font-sans selection:bg-secondary/30">
      ...
      <aside className={`${mobileNav ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed inset-y-0 left-0 z-40 flex w-60 flex-col border-r border-border bg-[var(--sidebar)] transition-transform duration-150`}>
        ...
        <div className="px-3"><div className="mb-3 flex items-center gap-3 px-3"><span className="text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">CRM</span><span className="h-px flex-1 bg-secondary" /></div>
          <nav className="space-y-1" aria-label="CRM navigation">{navItems.map(([label, Icon]) => <button key={label} onClick={() => label === 'Companies' && setMobileNav(false)} className={`group relative flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${label === 'Companies' ? 'bg-secondary/15 font-medium text-foreground' : 'text-foreground hover:bg-secondary hover:text-foreground'}`}><Icon size={17} className={label === 'Companies' ? 'text-foreground' : 'text-muted-foreground'} /><span>{label}</span>{label === 'Companies' && <span className="absolute left-0 h-6 w-0.5 rounded-r bg-primary text-primary-foreground" />}</button>)}</nav>
