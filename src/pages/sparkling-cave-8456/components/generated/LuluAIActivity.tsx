...
export const LuluAIActivity = () => {
  ...
  return <div className="activity-shell">
    <aside className="activity-sidebar" aria-label="Primary navigation">
      ...
      <nav><p className="nav-label">Main</p>{[{
          label: 'Dashboard',
          ...
          return <button className="activity-nav" key={item.label}><Icon size={16} /><span>{item.label}</span></button>;
        })}<p className="nav-label platform-label">AI Platform</p>{platformNav.map(item => {
          const Icon = item.icon;
          return <button className={`activity-nav ${item.label === 'AI Activity' ? 'active' : ''}`} key={item.label} onClick={() => item.label === 'AI Activity' && setDrawerOpen(true)}><Icon size={16} /><span>{item.label}</span>{item.label === 'AI Activity' && <b className="nav-count">Live</b>}</button>;
        })}</nav>
