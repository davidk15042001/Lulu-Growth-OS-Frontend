...
export default function CalendarPortal() {
  ...
  async function createGuestLink(event: NativeCalendarEvent) {
    ...
    try { const result = await calendarApi.createGuestLink(workspaceId, event.id); setCreatedLink(result.data.guestJoinPath); setEvents((current) => current.map((item) => item.id === event.id ? { ...item, guestJoinPath: result.data.guestJoinPath } : item)); setNotice(t('Guest link created.')); }
