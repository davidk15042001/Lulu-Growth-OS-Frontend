...
function Sidebar({
  ...
  return <aside className="w-[220px] shrink-0 border-r border-border/[.07] bg-[var(--background)] px-3 py-5 flex flex-col min-h-screen">
    ...
    <nav className="space-y-1 text-[13px]" aria-label="Primary navigation">
      {main.map(({
        name,
        icon: Icon
      }) => <button key={name} onClick={() => setActive(name)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg ${active === name ? 'bg-secondary/15 text-foreground' : 'text-foreground hover:bg-secondary hover:text-foreground'}`}><Icon size={16} /><span>{name}</span></button>)}
      <div className="pt-5 pb-2 px-3 text-[10px] uppercase tracking-[.18em] font-semibold text-muted-foreground">CRM</div>
      {crm.map(name => <button key={name} onClick={() => setActive(name)} className={`w-full text-left flex items-center gap-3 px-3 py-2 rounded-lg ${active === name ? 'bg-secondary/20 text-foreground border-l-2 border-border' : 'text-foreground hover:bg-secondary hover:text-foreground'}`}><span className={`h-1.5 w-1.5 rounded-full ${active === name ? 'bg-primary' : 'bg-card'}`} /><span>{name}</span>{name === 'Activities' && <span className="ml-auto rounded-full bg-secondary/20 text-foreground px-1.5 text-[10px]">54</span>}</button>)}
      <div className="pt-5 space-y-1"><button className="w-full flex items-center gap-3 px-3 py-2.5 text-foreground hover:text-foreground"><Sparkles size={16} /><span>AI Assistant</span></button><button className="w-full flex items-center gap-3 px-3 py-2.5 text-foreground hover:text-foreground"><Target size={16} /><span>AI Agents</span></button><button className="w-full flex items-center gap-3 px-3 py-2.5 text-foreground hover:text-foreground"><Settings size={16} /><span>Settings</span></button></div>
    </nav>
    <button className="mt-auto flex items-center gap-3 border-t border-border/[.07] pt-4 px-3 text-left"><div className="h-8 w-8 rounded-full bg-gradient-to-br from-primary to-primary flex items-center justify-center text-xs font-bold text-primary-foreground">SC</div><div><p className="text-xs font-medium">Sarah Chen</p><p className="text-[11px] text-muted-foreground">Administrator</p></div><ChevronDown size={14} className="ml-auto text-muted-foreground" /></button>
  </aside>;
}
