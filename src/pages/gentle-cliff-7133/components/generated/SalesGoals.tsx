...
import { Activity, BarChart3, Bell, CheckCircle2, CheckSquare, ChevronDown, CircleDollarSign, Download, FileText, Filter, LayoutDashboard, KanbanSquare, MoreHorizontal, RefreshCw, Search, Settings2, Sparkles, Target, TrendingUp, Upload, Users, X, Zap } from 'lucide-react';
...
const goals = [['Quarterly Revenue', 'Revenue', 'Maria Chen', '—', '—', '73.6%', '—', 'On Track', 'Today'], ['New Customers', 'New Customer', 'Workspace member', '50', '34', '68.0%', '16', 'At Risk', 'Yesterday'], ['Pipeline Value', 'Pipeline', 'David Torres', '—', '—', '77.5%', '—', 'On Track', '2 days ago'], ['DACH Revenue', 'Revenue', 'Sarah Johnson', '—', '—', '108.0%', '—', 'Exceeded', 'Today'], ['Enterprise Deals', 'Deal', 'Maria Chen', '12', '7', '58.3%', '5', 'At Risk', '3 days ago'], ['Product A Revenue', 'Product', 'Workspace member', '—', '—', '68.8%', '—', 'At Risk', 'Yesterday'], ['Team Alpha Target', 'Revenue', 'Team Alpha', '—', '—', '76.0%', '—', 'On Track', 'Today'], ['Activity Goal', 'Activity', 'David Torres', '200', '168', '84.0%', '32', 'On Track', 'Today'], ['Retention Goal', 'Retention', 'Sarah Johnson', '95%', '96.2%', '101.3%', '—', 'Exceeded', 'Today'], ['UK Region', 'Revenue', 'Tom Wilson', '—', '—', '—', '—', 'Draft', '1 week ago']] as const;
const teams = [['Team Alpha', '—', '—', '76.0%', '—', '3', '1'], ['Team Beta', '—', '—', '71.2%', '—', '2', '2'], ['Team DACH', '—', '—', '108.0%', '—', '2', '0']] as const;
const reps = [['Maria Chen', '—', '—', '73.6%', '—', '—', '—', 'On Track'], ['Workspace member', '—', '—', '71.0%', '—', '—', '—', 'At Risk'], ['David Torres', '—', '—', '82.2%', '—', '—', '—', 'On Track'], ['Sarah Johnson', '—', '—', '108.0%', '—', '—', '—', 'Exceeded'], ['Tom Wilson', '—', '—', '70.0%', '—', '—', '—', 'At Risk']] as const;
function Badge({
  kind
}: {
  kind: BadgeKind;
...
export function SalesGoals() {
  const { items: goalRecords, loading: goalsLoading, error: goalsError, refresh: refreshGoals } = useLiveRecords('sales_goals');
  ...
  const recommendations = [['risk', '⚠️ At-Risk Goal', 'Enterprise Deals goal is tracking below expected trajectory. At current pace, 9 deals will be closed vs. target of 12. Recommended action: accelerate pipeline qualification.', '— achievement at 66% of period elapsed.'], ['growth', '💡 Growth Opportunity', 'DACH Region has exceeded its Q3 target by 8%. Historical trend and current pipeline support a higher target for Q4.', '— actual vs — target.'], ['adjust', '📊 Target Adjustment', 'Team Beta target of — is above the 12-month historical average of — for this team. Consider adjusting or redistributing.', 'Rolling 12-month average: —']] as const;
  if (goalsLoading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live sales goals…</main>;
  if (goalsError) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-destructive">{goalsError}</main>;
  const liveGoals = goalRecords.filter(record => !query || `${record.name} ${record.description ?? ''} ${record.status}`.toLowerCase().includes(query.toLowerCase()));
  return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-6xl"><header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Sales</p><h1 className="mt-2 text-3xl font-bold">Sales Goals</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Verified sales goals from the connected workspace. Progress is shown only when returned by the backend.</p></div><button type="button" onClick={() => void refreshGoals()} className="inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm hover:bg-secondary"><RefreshCw size={15} /> Refresh</button></header>{goalRecords.length === 0 ? <section className="rounded-2xl border border-dashed border-border bg-card p-10 text-center"><Target className="mx-auto mb-4 text-muted-foreground" size={28} /><h2 className="text-xl font-semibold">No verified sales goals yet</h2><p className="mx-auto mt-3 max-w-xl text-sm text-muted-foreground">Create or import a sales goal through a connected CRM or the workspace API before reviewing target progress.</p></section> : <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="border-b border-border p-4"><label className="flex items-center gap-2 rounded-lg border border-border bg-secondary px-3 py-2"><Search size={15} className="text-muted-foreground" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search live sales goals" className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" /></label></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Goal</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Stage</th><th className="px-4 py-3">Value</th><th className="px-4 py-3">Description</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{liveGoals.map(record => <tr key={record.id}><td className="px-4 py-3 font-medium">{record.name}</td><td className="px-4 py-3">{record.status}</td><td className="px-4 py-3 text-muted-foreground">{record.stage ?? '—'}</td><td className="px-4 py-3">{record.valueAmount ?? '—'} {record.currency ?? ''}</td><td className="max-w-md px-4 py-3 text-muted-foreground">{record.description ?? '—'}</td><td className="px-4 py-3 text-muted-foreground">{new Date(record.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>}</div></main>;
