...
const plans: Plan[] = [
  ...
  { id: "test", name: "Test", eyebrow: "Full AI access for verification", description: "Use the complete AI package without billing. A Test password is required.", icon: WandSparkles, accent: "bg-[var(--secondary)] text-[var(--foreground)] border border-dashed border-[var(--primary)]/40", features: ["Everything in AI", "Full AI analysis, recommendations and actions", "Full automation of supported workflows", "No package fee and no usage billing", "Immediate activation after password confirmation"], limitations: "Only the confirmation password is required.", price: "Free", pricePeriod: "all included for testing", cta: "Choose Test" },
];
...
function PaygOverview({ payg, currentPlan }: { payg: BillingState["payg"]; currentPlan: string | null }) {
  if (!payg) {
    return <section className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 sm:p-6"><div className="flex items-start gap-3"><ShieldCheck size={20} className="mt-0.5 shrink-0" /><div><h2 className="font-semibold">Pay as you go</h2><p className="mt-1 text-sm leading-6 text-[var(--muted-foreground)]">{currentPlan === "viewer" ? "Viewer is read-only and has no usage charges." : currentPlan === "test" ? "The Test package includes full AI access without billing." : "Usage billing starts after a payment method has been confirmed."}</p></div></div></section>;
  }
...
export function DashboardBilling() {
  ...
  const visiblePlans = plans.filter((plan) => plan.id !== "test");
  ...
  const selected = visiblePlans.find((plan) => plan.id === selectedPlan);
  const activePlan = visiblePlans.find((plan) => plan.id === currentPlan) ?? (currentPlan === "test" ? plans.find((plan) => plan.id === "ai") : undefined);
