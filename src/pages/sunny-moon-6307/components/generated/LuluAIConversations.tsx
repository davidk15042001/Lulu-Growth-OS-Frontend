...
export const LuluAIConversations = () => {
  ...
  const { items: conversationRecords, loading, error, refresh: refreshConversations } = useLiveRecords('ai_conversations');
  const [refreshing, setRefreshing] = useState(false);
  const refresh = () => { void refreshConversations(); setRefreshing(true); window.setTimeout(() => setRefreshing(false), 700); };
  ...
  if (loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live conversations…</main>;
  ...
  return <div className="conversation-shell">
    ...
    {modal === 'export' && <dialog open className="dialog" aria-labelledby="export-title"><header><h2 id="export-title">Export Conversation</h2></header><p>Choose a format for your conversation export.</p><div className="format-options"><label><input type="radio" name="format" defaultChecked />PDF</label><label><input type="radio" name="format" />Text</label><label><input type="radio" name="format" />JSON</label></div><div className="dialog-actions"><button className="secondary" onClick={() => setModal(null)}>Cancel</button></div></dialog>}
