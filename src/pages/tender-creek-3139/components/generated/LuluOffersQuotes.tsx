...
function Sidebar() {
  return <aside className="sticky top-0 flex h-screen w-[244px] shrink-0 flex-col bg-[var(--background)] px-4 py-5 text-foreground">
    ...
    <nav aria-label="Primary navigation" className="space-y-1">{navMain.map(({
        label,
        icon: Icon
      }) => <button key={label} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-[13px] font-medium transition-colors hover:bg-secondary hover:text-foreground"><Icon size={17} /><span>{label}</span></button>)}</nav>
...
export function LuluOffersQuotes() {
  ...
  const visibleOffers = useMemo(() => offers.filter(offer => `${offer.id} ${offer.customer} ${offer.amount}`.toLowerCase().includes(query.toLowerCase())), [query]);
