...
export function LuluPublishingCenter() {
  ...
  return <div className="lulu-app">
    ...
    <main className="main">
      ...
      <section className="kpis">{[['Pending Approval', '4', 'Requires your review', 'amber'], ['Ready to Publish', '2', '2 campaigns validated', 'green'], ['Scheduled', '3', 'Next: Sep 12, 09:00 CET', 'blue'], ['Publishing', '1', 'Google Ads · In progress', 'violet'], ['Published Today', '7', 'Across 3 platforms', 'green'], ['Failed', '1', 'Requires attention', 'red']].map(([label, num, sub, color]) => <article className={`kpi ${color}`} key={label}><strong>{num}</strong><div><h3>{label}</h3><p>{sub}</p></div></article>)}</section>
      <div className="workspace"><div className="left-col">
        <section><SectionTitle title="Pending Approvals" count="4" /><div className="filters"><button>All Platforms <ChevronDown size={13} /></button><button>All Types <ChevronDown size={13} /></button><button>Risk <ChevronDown size={13} /></button><button className="amber-outline" onClick={() => notify('Bulk approve requires confirmation')}><Check size={14} /> Bulk Approve</button></div>
          ...
          {expanded === 'lead-gen' && <article className="detail"><div className="detail-head"><div><span className="eyebrow">REVIEW ADVERTISING CHANGE</span><h3>Q1 Lead Generation Campaign</h3></div><button className="icon-btn" onClick={() => setExpanded('')}><X size={17} /></button></div><div className="diff-head"><span>Current Configuration</span><span>Proposed Configuration <AiBadge>AI Change</AiBadge></span></div>{[['Campaign Name', '—', 'Q1 Lead Gen — DACH — Google + Meta'], ['Platform', '—', 'Google Ads + Meta Ads'], ['Objective', '—', 'Lead Generation'], ['Budget', '—', '€6,000/mo Google + €3,000/mo Meta'], ['Audience', '—', '3 audiences · High-Intent Visitors, CRM Lookalike, DACH Interests'], ['Schedule', '—', 'Jan 6, 2025 → Ongoing'], ['Tracking', '—', 'Google Tag + Meta Pixel + CAPI ✓'], ['Ads', '—', '8 creatives · AI Generated']].map(([a, b, c]) => <div className="diff-row" key={a}><span>{a}</span><span>{b}</span><span>{c}<small>Change</small></span></div>)}<div className="reason"><AiBadge>AI Inferred</AiBadge><p>Lulu AI built this campaign based on your stated Q1 business goal, CRM data, historical Google Ads performance (CPL: €42 avg), and available Meta retargeting audiences. Strategy prioritizes high-intent bottom-funnel search traffic on Google and retargeting on Meta.</p></div><div className="detail-notes"><p><strong>Data Sources</strong> Company Profile · CRM · Google Analytics · Google Ads (Synced 5 min ago) · Meta Ads (Synced 45 min ago) · <span className="observed">Observed Data</span></p><p><strong>Potential Impact</strong> <span className="estimated">Estimated</span> 91–133 leads/month at CPL €38–€55. Not guaranteed.</p><p><strong>Risk level</strong> <span className="risk high">High</span> — New campaign + financial commitment. Requires Marketing Manager approval.</p></div><div className="steps"><span>Lulu AI Recommendation</span><b>1</b><strong>Marketing Manager (You)</strong><b>2</b><span>Finance Approval</span><ArrowRight size={15} /><span>Publish</span></div><div className="detail-actions"><button onClick={() => notify('Change request sent')}>Request Changes</button><button onClick={() => notify('Change rejected')}>Reject Change</button><button className="primary" onClick={() => notify('Forwarded to Finance')}>Approve & Forward to Finance <ArrowRight size={15} /></button></div></article>}
        </section>
