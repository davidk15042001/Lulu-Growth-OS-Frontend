...
type WhatsAppConnectionSummary = {
  ...
  customerConnection: { displayName: string; senderStatus: string; status: string; lastError: string | null } | null;
  adminFallback: { configured: boolean; displayName: string | null; status: string };
...
export function LuluIntegrations() {
  ...
  const load = useCallback(async () => {
    if (!selectedWorkspace) {
      setPlatforms([]);
      setAllowedSelfServiceProviders([]);
    ...
    try {
      const [platformResponse, permissionResponse, whatsappResponse] = await Promise.all([
        onboardingApi.platforms(selectedWorkspace.id),
        onboardingApi.oauthSelfServicePermissions(selectedWorkspace.id),
  ...
  const whatsappApproved = allowedSelfServiceProviders.includes('whatsapp') || whatsappConnection?.selfServiceAllowed === true;
  const showApprovedWhatsApp = whatsappApproved && !whatsappPlatform;
  const configuredCount = platforms.length + (showApprovedWhatsApp ? 1 : 0);
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground">
    ...
    <main className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:ml-64 lg:px-12 lg:py-12">
      ...
      {loading ? <div className="grid min-h-72 place-items-center rounded-2xl border border-border bg-card text-sm text-muted-foreground"><span className="inline-flex items-center gap-2"><LoaderCircle size={17} className="animate-spin" />Loading available connections…</span></div> : <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="border-b border-border p-5"><h2 className="font-semibold">Available connections</h2><p className="mt-1 text-sm text-muted-foreground">Admin-approved providers appear here immediately, even before the workspace has completed its connection.</p></div>{platforms.length === 0 && !showApprovedWhatsApp ? <div className="p-10 text-center"><PlugZap className="mx-auto text-muted-foreground" size={34} /><h3 className="mt-4 text-lg font-semibold">No integrations available</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">Ask your Lulu administrator to enable a managed provider or add a custom integration below.</p></div> : <div className="divide-y divide-border">{showApprovedWhatsApp && <article className="p-5" data-provider="whatsapp" data-admin-approved="true"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-lg bg-secondary"><MessageCircle size={16} /></span><h3 className="font-semibold">WhatsApp</h3><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium text-emerald-700">Approved by admin</span></div><p className="mt-2 text-sm text-muted-foreground">Your workspace is allowed to connect its own WhatsApp Business account.</p><p className="mt-1 text-xs text-muted-foreground">{whatsappConnection?.adminFallback.configured ? `Lulu’s managed sender${whatsappConnection.adminFallback.displayName ? ` (${whatsappConnection.adminFallback.displayName})` : ''} remains active until your account is connected.` : 'Open Connections to finish the secure WhatsApp setup.'}</p></div><button type="button" onClick={() => navigateApp(routes.app.connections)} disabled={!canEdit} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50"><Link2 size={15} />Set up WhatsApp</button></div></article>}{platforms.map((platform) => { const connectedPlatform = connectedStatuses.has(platform.connectionStatus); const busy = busyId === platform.id; const isWhatsApp = platform.integrationKey?.toLowerCase() === 'whatsapp'; return <article key={platform.id} className="p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{platform.name}</h3><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${connectedPlatform ? 'bg-secondary text-foreground' : platform.connectionStatus === 'error' ? 'bg-destructive/10 text-destructive' : 'bg-secondary text-muted-foreground'}`}>{labelStatus(platform.connectionStatus)}</span>{isWhatsApp && whatsappApproved ? <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium text-emerald-700">Approved by admin</span> : null}</div><p className="mt-1 text-sm text-muted-foreground">{platform.category}{platform.integrationKey ? ` · ${platform.integrationKey}` : ''}</p><p className="mt-2 text-xs text-muted-foreground">Last sync: {formatDate(platform.lastSyncedAt)}{platform.lastError ? ` · ${platform.lastError}` : ''}</p></div><div className="flex flex-wrap gap-2">{connectedPlatform ? <><button type="button" onClick={() => void sync(platform)} disabled={busy || !can('administer')} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm disabled:opacity-50"><RefreshCw size={14} className={platform.connectionStatus === 'syncing' ? 'animate-spin' : ''} />Sync</button><button type="button" onClick={() => void disconnect(platform)} disabled={busy || !can('administer')} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm disabled:opacity-50"><Unplug size={14} />Disconnect</button></> : <button type="button" onClick={() => isWhatsApp ? navigateApp(routes.app.connections) : void beginOAuth(platform)} disabled={busy || !canEdit || (isWhatsApp ? !whatsappApproved : !platform.integrationKey || !oauthProviders.has(platform.integrationKey.toLowerCase()))} className="inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50"><Link2 size={14} />{isWhatsApp ? 'Set up WhatsApp' : 'Connect'}</button>}<button type="button" onClick={() => void remove(platform)} disabled={busy || !can('administer')} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm text-destructive disabled:opacity-50"><Trash2 size={14} />Remove</button></div></div></article>; })}</div>}</section>}
