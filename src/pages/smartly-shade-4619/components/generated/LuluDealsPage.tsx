  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground">
    ...
    {!liveLoading && liveDeals.length === 0 && <></>}
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-7 md:px-8">
          <section className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-foreground">Revenue operations</p><h1 className="text-3xl font-semibold tracking-[-0.04em] text-foreground md:text-[34px]">Deals</h1><p className="mt-2 text-sm text-muted-foreground">Manage your sales opportunities, pipeline value and revenue potential.</p></div><div className="flex flex-wrap gap-2"><button className="inline-flex items-center gap-2 rounded-lg border border-border px-3.5 py-2 text-xs font-medium text-foreground hover:bg-secondary"><Import size={14} /> Import</button><button onClick={() => setShowCreate(true)} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground shadow-lg shadow-black/20 hover:bg-primary"><Plus size={15} /> Create Deal</button></div></section>
          ...
          {selected.length > 0 && <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-border/20 bg-secondary/10 px-4 py-3 text-xs"><strong className="text-foreground">{selected.length} selected</strong>{['Assign Owner', 'Change Stage', 'Add Tag', 'Create Task',  'Archive', 'Delete'].map(item => <button key={item} className="rounded-md border border-border bg-[var(--primary)] px-2.5 py-1.5 text-primary-foreground hover:text-primary-foreground">{item}</button>)}</div>}
