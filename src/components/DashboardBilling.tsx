...
import { ArrowRight, Check, Database, ExternalLink, Lock, ShieldCheck, Sparkles, WandSparkles } from "lucide-react";
...
const plans: Plan[] = [
  { id: "ai", name: "AI", eyebrow: "Let Lulu run growth", description: "Give Lulu the authority to recommend, execute and automate the work across your workspace.", icon: WandSparkles, accent: "bg-[var(--secondary)] text-[var(--foreground)] border border-[var(--primary)]/20", features: ["AI insights and recommendations", "AI-assisted content and decisions", "Full automation of supported workflows", "Prepaid AI execution with fixed RMB packages", "5% commission on each Lulu-attributed sale"], limitations: "AI automation starts with confirmed AI funds; Paid Ads and campaigns stay paused until separate advertising funds are available", price: LULU_AI_ANNUAL_PRICE, pricePeriod: "per year", cta: "Activate AI" },
];
...
function PaygOverview({ payg, currentPlan }: { payg: BillingState["payg"]; currentPlan: string | null }) {
  ...
  return <section className="mt-6 overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]">
    <div className="flex flex-col gap-4 border-b border-[var(--border)] px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      <div><p className="text-xs font-semibold uppercase tracking-[.14em] text-[var(--muted-foreground)]">Cloudflare R2 · every Monday</p><h2 className="mt-2 text-xl font-semibold">Current storage period</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">{formatDate(payg.periodStart)} – {formatDate(payg.periodEnd)} · next collection {formatDate(payg.nextInvoiceAt)}</p></div>
    ...
    <div className="grid gap-px bg-[var(--border)] sm:grid-cols-2">
      <div className="bg-[var(--card)] p-5 sm:p-6"><div className="flex items-center gap-2 text-sm font-semibold"><Database size={17} />R2 storage and operations</div><p className="mt-4 text-3xl font-semibold tracking-[-0.04em]">{formatMoney(payg.serverCost)}</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">{payg.serverDays} metered days · no free-tier deduction</p></div>
      <div className="bg-[var(--card)] p-5 sm:p-6"><div className="flex items-center gap-2 text-sm font-semibold"><Sparkles size={17} />Estimated Monday charge</div><p className="mt-4 text-3xl font-semibold tracking-[-0.04em]">{formatMoney(payg.estimatedTotal)}</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">AI and advertising are funded separately through prepaid RMB wallets.</p></div>
    </div>
    {payg.invoices.length > 0 && <div className="border-t border-[var(--border)] px-5 py-5 sm:px-6"><h3 className="text-sm font-semibold">Recent storage charges</h3><div className="mt-3 divide-y divide-[var(--border)]">{payg.invoices.map((invoice) => { const hostedUrl = safeExternalUrl(invoice.hostedInvoiceUrl); const pdfUrl = safeExternalUrl(invoice.invoicePdfUrl); const needsPayment = invoice.status === "payment_due" || invoice.status === "payment_failed"; return <div key={invoice.id} className="flex flex-col gap-2 py-3 text-sm first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{formatDate(invoice.periodStart)} – {formatDate(invoice.periodEnd)}</p><p className="mt-0.5 text-xs text-[var(--muted-foreground)]">{invoice.status.replaceAll("_", " ")} · R2 {formatMoney(invoice.serverCost)}</p></div><div className="flex items-center gap-3"><span className="font-semibold">{formatMoney(invoice.totalCost)}</span>{needsPayment && hostedUrl && <a href={hostedUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-semibold underline underline-offset-4">Open payment link<ExternalLink size={13} /></a>}{pdfUrl && <a href={pdfUrl} target="_blank" rel="noreferrer" className="font-semibold underline underline-offset-4">PDF</a>}</div></div>; })}</div></div>}
  </section>;
}
export function DashboardBilling() {
  ...
  return <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]"><div className="mx-auto w-full max-w-7xl px-5 py-10 sm:px-8 lg:px-12">
    <header className="flex flex-col gap-6 border-b border-[var(--border)] pb-8 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--muted-foreground)]">Workspace billing</p><h1 className="mt-3 text-4xl font-semibold tracking-[-0.05em]">Activate Lulu AI for your workspace.</h1><p className="mt-4 max-w-2xl text-base leading-7 text-[var(--muted-foreground)]">The Lulu subscription, prepaid AI execution and metered Cloudflare R2 storage are separated transparently.</p></div><button type="button" onClick={() => navigateApp(routes.app.dashboard)} className="text-sm font-semibold underline underline-offset-4">Back to dashboard</button></header>
