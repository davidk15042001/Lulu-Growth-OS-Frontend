...
import { Activity, AlertTriangle, ArrowDownRight, ArrowRight, ArrowUpRight, BarChart3, Bell, Check, ChevronDown, ChevronRight, CircleHelp, Clock3, Database, Download, Edit3, FileText, Filter, Gauge, GitBranch, LayoutDashboard, LineChart, Menu, MoreHorizontal, Plus, Search, Settings, ShieldAlert, Sparkles, Target, TrendingUp, Users, X, Zap } from 'lucide-react';
...
export const LuluForecasts = () => {
  ...
  const { items: forecastRecords, loading, error } = useLiveRecords('forecasts');
  if (loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live forecasts…</main>;
...
<Section title="Reference States" className="mb-8"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><div className="rounded-lg border border-dashed border-border p-4"><TrendingUp size={22} className="text-muted-foreground" /><h3 className="mt-3 text-sm font-medium">Create your first forecast</h3><p className="mt-1 text-xs text-muted-foreground">Select a supported KPI and forecast horizon, then use the Update button in the navigation to begin predicting future performance.</p></div><div className="rounded-lg border border-border p-4"><ShieldAlert size={18} className="text-foreground" /><h3 className="mt-3 text-sm font-medium">Insufficient Historical Data</h3><p className="mt-1 text-xs text-muted-foreground">At least 90 days of daily data is required for the selected KPI.</p><div className="mt-3 flex gap-2"><Button>Adjust Configuration</Button><Button>Connect Data</Button></div></div><div className="rounded-lg border border-border p-4"><CircleHelp size={18} className="text-muted-foreground" /><h3 className="mt-3 text-sm font-medium">Forecast Unavailable</h3><p className="mt-1 text-xs text-muted-foreground">The selected KPI or horizon is not currently supported.</p><Button>Try Again</Button></div><div className="rounded-lg border border-chart-5/15 p-4"><X size={18} className="text-chart-5" /><h3 className="mt-3 text-sm font-medium">Forecast Cannot Be Generated</h3><p className="mt-1 text-xs text-muted-foreground">Insufficient historical data · Data gaps exceed —.</p></div><div className="rounded-lg border border-border p-4"><span className="text-lg">🔒</span><h3 className="mt-3 text-sm font-medium">Forecast Access Restricted</h3><p className="mt-1 text-xs text-muted-foreground">You do not have permission to view this forecast.</p></div></div></Section>
</div></main></div>;
};
