...
export default function AdminCommercialDocumentsPage({ kind }: { kind: Kind }) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const result = await requestApi<Row[]>({ path: `/admin/${kind}?limit=5000${query.trim() ? `&search=${encodeURIComponent(query.trim())}` : ''}` });
      setRows(result.data);
    } catch (cause) {
