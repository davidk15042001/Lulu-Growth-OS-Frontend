...
export const CustomerSegments = () => {
  ...
  const filtered = segments.filter(segment => segment.name.toLowerCase().includes(query.toLowerCase()));
  ...
  return <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-sans">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-[232px] flex-col bg-[var(--sidebar)] text-foreground lg:flex">
        ...
        <div className="flex items-center gap-3 border-b border-border px-5 py-5"><div className="grid h-9 w-9 place-items-center rounded-full bg-secondary text-xs font-bold text-foreground">AC</div><div><p className="text-sm font-medium text-foreground">Acme Corp</p><p className="text-xs text-muted-foreground">Core workspace</p></div><ChevronDown className="ml-auto" size={15} /></div>
        <nav aria-label="Primary navigation" className="space-y-1 px-3 py-5">{nav.map(({
          label,
          icon: Icon
        }) => <div key={label}><button className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-[13px] transition ${label === 'Sales' ? 'bg-secondary/18 text-foreground' : 'text-foreground hover:bg-secondary hover:text-foreground'}`}><Icon size={17} /><span>{label}</span>{label === 'Sales' && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary text-primary-foreground" />}</button>{label === 'Sales' && <div className="ml-9 mt-1 space-y-1 border-l border-border pl-3"><button className="block w-full py-1.5 text-left text-xs text-foreground">Overview</button><button className="block w-full py-1.5 text-left text-xs text-foreground">Pipeline</button><button className="block w-full rounded-md bg-secondary px-2 py-1.5 text-left text-xs font-medium text-foreground">Customer Segments</button></div>}</div>)}</nav>
