...
const accounts: Array<{ key: PlatformKey; [key: string]: any }> = [];
...
export function AdAccountsWorkspace() {
  ...
  const { items: accountRecords, loading, error, refresh: refreshAccounts } = useLiveRecords('ad_accounts');
  const refresh = () => { void refreshAccounts(); };
  if (loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live advertising accounts…</main>;
  ...
  if (!loading && !error) return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-6xl"><header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Advertising / Accounts</p><h1 className="mt-2 text-3xl font-bold">Ad Accounts &amp; Platforms</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Verified advertising account records from connected providers. Permissions and publishing capabilities appear only when returned by the backend.</p></div><span className="inline-flex items-center rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground">Use Update in the navigation bar</span></header>{accountRecords.length === 0 ? <section className="rounded-2xl border border-dashed border-border bg-card p-10 text-center"><Link2 className="mx-auto mb-4 text-muted-foreground" size={30} /><h2 className="text-xl font-semibold">No advertising accounts connected</h2><p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">Connect a supported advertising provider before reviewing account permissions, publishing capabilities or spend data.</p></section> : <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Account</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Stage</th><th className="px-4 py-3">Description</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{accountRecords.map(record => <tr key={record.id}><td className="px-4 py-3 font-medium">{record.name}</td><td className="px-4 py-3">{record.status}</td><td className="px-4 py-3 text-muted-foreground">{record.stage ?? '—'}</td><td className="max-w-md px-4 py-3 text-muted-foreground">{record.description ?? '—'}</td><td className="px-4 py-3 text-muted-foreground">{new Date(record.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>}</div></main>;
  ...
  return <div className="workspace">
    ...
    <main className="main-shell"><div className="mobile-top"><button onClick={() => setMobileNav(!mobileNav)} aria-label="Toggle navigation"><Menu size={20} /></button><strong>LULU <span>AI</span></strong><button aria-label="Notifications"><Bell size={18} /></button></div>
      ...
      <div className="content-layout"><div className="center-column">
        ...
        <section className="surface"><SectionHeader title="Account Spending Limits" subtitle="Limits observed from connected platform accounts. Platform limits are set by each advertising platform, not by Lulu AI." /><div className="spend-list">{[['google', 'Daily Limit: — (Platform)', 'Current spend today: —', 'Remaining: —'], ['meta', 'Daily Limit: — (Platform)', 'Current: —', 'Remaining: — · Balance: —'], ['linkedin', 'Daily Limit: No platform limit', 'Monthly budget: — (Campaign-level)', 'Spend: —'], ['tiktok', 'Spending Limits: Unknown (permission error)', 'Unknown', 'Unknown']].map(([key, limit, spend, remain]) => <div className="spend-row" key={key}><PlatformBadge platform={key as PlatformKey} large /><strong>{platformMeta[key as PlatformKey].name}</strong><span>{limit}</span><span>{spend}</span><span className={key === 'tiktok' ? 'bad' : ''}>{remain}</span></div>)}</div><p className="note">All spending data: <b>Observed</b> from platform APIs. Estimates are labeled.</p></section>
