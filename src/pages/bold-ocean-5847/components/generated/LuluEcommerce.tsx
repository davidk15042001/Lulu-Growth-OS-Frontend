...
export function LuluEcommerce() {
  ...
  const visibleItems = useMemo(
    () =>
      items.filter((record) =>
        `${record.name} ${record.description ?? ''} ${record.status} ${textValue(record.data?.category)}`
          .toLowerCase()
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold">Ecommerce</h1>
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              Verified ecommerce product records from the connected workspace.
            </p>
          </div>
        ...
        <section className="overflow-hidden rounded-2xl border border-border bg-card">
          ...
          {items.length === 0 ? (
            <div className="p-10 text-center">
              <Package className="mx-auto mb-4 text-muted-foreground" size={30} />
              <h2 className="text-xl font-semibold">No ecommerce data available yet</h2>
              <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
                Connect an ecommerce platform to populate verified products and categories.
              </p>
            </div>
          ) : (
                  ...
                  {visibleItems.map((record) => (
                    <tr key={record.id}>
                      ...
                      <td className="px-4 py-3 text-muted-foreground">
                        {textValue(record.data?.category) || '—'}
                      </td>
