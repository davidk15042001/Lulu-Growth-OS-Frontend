...
export function LuluAIAssistant() {
  ...
  const [processingState] = useState(true);
  const [showMenu, setShowMenu] = useState(false);
  const [query, setQuery] = useState('');
  return <div className="lulu-app">
    <nav className="left-nav" aria-label="Primary navigation">
      <div className="brand"><span className="brand-spark"><Sparkles size={12} /></span><strong>Lulu AI</strong></div>
      <div className="nav-scroll">
        ...
        <SectionLabel>AI Platform</SectionLabel>
        {navAi.map(item => {
          const Icon = item.icon;
          const active = item.label === 'AI Assistant';
          return <button className={`nav-item ${active ? 'active' : ''}`} key={item.label}><Icon size={16} /><span>{item.label}</span></button>;
        })}
      </div>
      <div className="nav-bottom">
        <button className="nav-item"><Settings size={16} /><span>Settings</span></button><button className="nav-item"><HelpCircle size={16} /><span>Help</span></button>
        <div className="user-profile"><span className="user-avatar">SM</span><span><strong>Sarah Mitchell</strong><small>CEO</small></span><ChevronRight size={14} /></div>
      </div>
    </nav>

    <aside className="history-rail" aria-label="Conversation history">
      <header className="rail-header"><strong>Conversations</strong><button className="new-small"><Plus size={13} /> New</button></header>
    ...
    <main className="chat-column">
      ...
      <div className="messages" role="log" aria-live="polite" aria-label="Conversation">
        ...
        <section className="prompt-grid" aria-label="Suggested prompts">{prompts.map(prompt => <button key={prompt.text} className="prompt-card"><span>{prompt.category}</span><strong>{prompt.text}</strong><ArrowRight size={14} /></button>)}</section>
