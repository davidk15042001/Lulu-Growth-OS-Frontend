...
export const LuluMarketingAnalytics = () => {
  ...
  const liveKpis: Kpi[] = [{ label: 'Marketing Revenue', value: formatMoney(liveRevenue), delta: 'Live records', tone: 'green' }, { label: 'Marketing Spend', value: formatMoney(liveSpend), delta: 'Live records', tone: 'gray' }, { label: 'ROAS', value: liveSpend ? (liveRevenue / liveSpend).toFixed(2) : 'Not available', delta: 'Calculated', tone: 'amber' }, { label: 'CAC', value: liveCustomers ? formatMoney(liveSpend / liveCustomers) : 'Not available', delta: 'Calculated', tone: 'gray' }, { label: 'Leads', value: String(liveLeads || 'Not available'), delta: 'Live records', tone: 'blue' }, { label: 'Customers', value: String(liveCustomers || 'Not available'), delta: 'Live records', tone: 'gray' }, { label: 'Conv. Rate', value: liveLeads ? `${(liveCustomers / liveLeads * 100).toFixed(1)}%` : 'Not available', delta: 'Calculated', tone: 'amber' }, { label: 'Marketing ROI', value: liveSpend ? `${((liveRevenue - liveSpend) / liveSpend * 100).toFixed(0)}%` : 'Not available', delta: 'Calculated', tone: 'green' }];
  return <div className="min-h-screen bg-card font-sans text-[var(--foreground)]">{liveLoading ? <div className="border-b border-border bg-secondary/30 px-5 py-3 text-xs text-muted-foreground lg:pl-[244px]">Loading live marketing analytics…</div> : liveError ? <div className="border-b border-destructive/30 bg-destructive/5 px-5 py-3 text-xs text-destructive lg:pl-[244px]">{liveError}</div> : liveEmpty ? <div className="border-b border-dashed border-border bg-card px-5 py-3 text-xs text-muted-foreground lg:pl-[244px]">No live marketing campaigns are available yet. Connect a marketing platform or add a campaign to begin.</div> : null}
