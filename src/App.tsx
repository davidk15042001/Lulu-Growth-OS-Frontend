...
function pageCode(page: PageDefinition) {
  const match = page.generatedName.match(/(\d{4})$/) ?? page.slug.match(/(\d{4})$/);
  return match?.[1] ?? page.id.slice(-4).padStart(4, "0");
}
...
function AuthenticatedSearchBar() {
  ...
  return <div className="lulu-auth-search-wrap" data-lulu-auth-search="true">
    ...
    {open && query.trim() && <div className="lulu-auth-search-results">{matches.length ? matches.map((item) => <button type="button" key={item.id} aria-label={pageCode(item)} onClick={() => { navigate(pagePath(item.slug)); setQuery(""); setAnswer(null); setOpen(false); }}><strong className="lulu-auth-search-code">{pageCode(item)}</strong></button>) : busy ? <p>Working on your Lulu AI request…</p> : answer ? <p className="lulu-auth-search-answer">{answer}</p> : <p>Ask Lulu AI about your workspace or type a page name.</p>}</div>}
  </div>;
}
