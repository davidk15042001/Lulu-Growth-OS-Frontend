...
export function AdSpendWalletPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (request === loadRequest.current && workspaceRef.current === targetWorkspaceId) {
        setError(getFriendlyErrorMessage(cause, "The advertising wallet could not be loaded."));
      }
    } finally {
  ...
  useEffect(() => {
    ...
    void QRCode.toDataURL(topup.qrPayload, { width: 240, margin: 1, errorCorrectionLevel: "M" })
      ...
      .catch(() => {
        if (active && workspaceRef.current === targetWorkspaceId) setError("The advertising payment QR code could not be rendered.");
      });
    return () => {
      active = false;
    };
  ...
  async function pay() {
    ...
    try {
      ...
      if (request === actionRequest.current && workspaceRef.current === targetWorkspaceId) {
        setError(getFriendlyErrorMessage(cause, "The advertising budget payment could not be created."));
      }
    } finally {
    ...
    <section className="mb-6 overflow-hidden rounded-3xl border border-sky-500/20 bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,.14),transparent_48%),var(--card)] shadow-sm">
      <div className="grid lg:grid-cols-[.9fr_1.1fr]">
        <div className="p-6 sm:p-8">
          <div className="flex items-center gap-2 text-sky-700"><DollarSign size={18} /><p className="text-xs font-semibold uppercase tracking-[.18em]">Prepaid advertising</p></div>
          <h2 className="mt-3 text-2xl font-semibold">Ads budget wallet</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">Advertising has its own prepaid balance. It is completely separate from AI/API funds and Cloudflare R2 storage.</p>
          <p className="mt-7 text-xs uppercase tracking-[.15em] text-muted-foreground">Available budget</p>
          <p className="mt-2 text-4xl font-semibold">{loading ? "—" : money.format(wallet?.availableAmount ?? 0)}</p>
          <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
            <div className="rounded-xl border border-border bg-background/50 p-3"><span className="text-muted-foreground">Reserved</span><strong className="mt-1 block text-sm">{money.format(wallet?.reservedAmount ?? 0)}</strong></div>
            <div className="rounded-xl border border-border bg-background/50 p-3"><span className="text-muted-foreground">Spent</span><strong className="mt-1 block text-sm">{money.format(wallet?.spentAmount ?? 0)}</strong></div>
            <div className="rounded-xl border border-border bg-background/50 p-3"><span className="text-muted-foreground">Funded</span><strong className="mt-1 block text-sm">{money.format(wallet?.totalFundedAmount ?? 0)}</strong></div>
          </div>
          <span className={`mt-4 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold ${ready ? "bg-emerald-500/10 text-emerald-700" : "bg-amber-500/10 text-amber-700"}`}><span className={`h-2 w-2 rounded-full ${ready ? "bg-emerald-500" : "bg-amber-500"}`} />{ready ? "Paid execution can run" : "Waiting for confirmed ad funds"}</span>
          <div className="mt-5 flex items-start gap-2 rounded-xl bg-secondary p-3 text-xs leading-5 text-muted-foreground"><ShieldCheck size={16} className="mt-0.5 shrink-0 text-foreground" />Lulu can optimize campaigns autonomously, but it can never spend beyond the prepaid wallet and the customer-authorized campaign limit.</div>
        </div>
        <div className="border-t border-border bg-background/40 p-6 sm:p-8 lg:border-l lg:border-t-0">
          ...
          <dl className="mt-4 space-y-2 rounded-xl border border-border bg-card p-4 text-sm"><div className="flex justify-between"><dt className="text-muted-foreground">Ad budget credited</dt><dd>{money.format(amount)}</dd></div><div className="flex justify-between"><dt className="text-muted-foreground">Lulu fee (4%)</dt><dd>{money.format(fee)}</dd></div><div className="flex justify-between border-t border-border pt-2 font-semibold"><dt>Total charged</dt><dd>{money.format(total)}</dd></div></dl>
          ...
          {!can("administer") && <p className="mt-2 text-xs text-muted-foreground">Only workspace owners and administrators can add advertising budget.</p>}
        </div>
      </div>
      {topup?.qrPayload && <div className="border-t border-border p-6 text-center"><h3 className="font-semibold">Scan with {topup.paymentMethod === "alipaycn" ? "Alipay" : "WeChat Pay"}</h3>{qr && <img src={qr} alt="Advertising budget payment QR code" className="mx-auto mt-4 h-60 w-60 rounded-xl border bg-white p-3" />}<p className="mt-3 text-sm text-muted-foreground">{topup.status === "SUCCEEDED" ? <span className="inline-flex items-center gap-2 text-emerald-700"><CheckCircle2 size={16} />Advertising budget credited.</span> : "Waiting for confirmed payment…"}</p></div>}
    </section>
  );
}
