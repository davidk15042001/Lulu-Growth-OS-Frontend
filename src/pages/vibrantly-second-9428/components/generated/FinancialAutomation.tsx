...
const approvals = [['High-Value Expense Flag', 'Request approval', '€12,400', 'EXP-9021', 'Finance approver', '08 Aug', 'CFO'], ['Invoice Reminder', 'Create task', '€10,800', 'INV-2821', 'Workspace owner', '07 Aug', 'Finance Manager'], ['Payout Delay Alert', 'Notify team', '€8,200', 'PAY-441', 'Finance operator', '06 Aug', 'CFO']];
const versions = [['v3', '08 Aug 2026', 'Workspace owner', 'Added approval step', 'Active'], ['v2', '02 Aug 2026', 'Finance approver', 'Updated trigger conditions', 'Archived'], ['v1', '18 Jul 2026', 'Workspace owner', 'Initial automation', 'Archived']];
const activity = [['Automation activated', '08 Aug 2026 · Workspace owner'], ['Approval step added', '07 Aug 2026 · Finance approver'], ['Automation tested successfully', '07 Aug 2026 · Workspace owner'], ['Automation created', '18 Jul 2026 · Workspace owner']];
function Badge({
  children,
  tone = 'slate'
}: {
...
export function FinancialAutomation() {
  ...
  const liveAutomations = automationRecords.map(record => [getAutomationField(record, 'name') || getAutomationField(record, 'automationName') || 'Finance automation', getAutomationField(record, 'trigger') || 'Configured trigger', getAutomationField(record, 'actions') || 'Configured actions', getAutomationField(record, 'status') || 'Active', getAutomationField(record, 'lastRun') || '—', getAutomationField(record, 'nextRun') || '—', getAutomationField(record, 'executions') || '0', getAutomationField(record, 'successRate') || '—', getAutomationField(record, 'owner') || 'Workspace owner']);
