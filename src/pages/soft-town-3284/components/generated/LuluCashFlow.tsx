...
const kpis: [string, string, string, string, Tone][] = [];
...
export function LuluCashFlow() {
 ...
 <div className="mb-7 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">{(liveLoading ? [] : kpis).map(([name, value, detail, label, tone]) => <article key={name} className="rounded-xl border border-border bg-card p-4 shadow-[0_2px_8px_rgba(0,0,0,0.03)]"><div className="flex items-start justify-between gap-2"><p className="text-xs font-semibold text-muted-foreground">{name}</p><Badge tone={tone}>{label}</Badge></div><p className="mt-4 text-[22px] font-bold tracking-tight text-foreground">{value}</p><p className={`mt-1 flex items-center gap-1 text-xs font-bold ${detail.startsWith('+') ? 'text-foreground' : detail.includes('Next') ? 'text-foreground' : 'text-muted-foreground'}`}>{detail.startsWith('+') && <ArrowUpRight size={13} />}<span>{detail}</span></p><p className="mt-3 text-[11px] text-muted-foreground">{label === 'Forecast' ? 'Awaiting connected data' : 'Connected account data'}</p></article>)}</div>
 <Card title="Cash Flow" subtitle="Monthly view · EUR · last 30 days compared with previous month"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div className="flex gap-1 rounded-lg bg-secondary p-1">{['Daily', 'Weekly', 'Monthly'].map(x => <button key={x} onClick={() => setView(x)} className={`rounded-md px-3 py-1.5 text-xs font-bold ${view === x ? 'bg-card text-foreground shadow-sm' : 'text-foreground'}`}>{x}</button>)}</div><div className="flex flex-wrap gap-3 text-[11px] text-muted-foreground"><span><i className="mr-1 inline-block w-5 border-t border-dashed border-border" />Opening Cash</span><span className="text-foreground">● Money In</span><span className="text-foreground">● Money Out</span><span className="text-foreground">━ Net Cash Flow</span><span className="text-foreground">┄ Forecast</span></div></div><div className="rounded-lg bg-card/70 p-3"><svg viewBox="0 0 900 290" className="h-[280px] w-full" role="img" aria-label="Cash flow chart"><g stroke="var(--border)" strokeDasharray="3 5"><path d="M55 35H875M55 85H875M55 135H875M55 185H875M55 235H875" /></g><path d="M55 170 C120 150 150 165 200 132 S280 150 330 110 S410 135 465 96 S545 118 600 80 S680 102 740 65 S820 80 875 48 L875 235 L55 235Z" fill="var(--chart-4)" opacity=".12" /><path d="M55 205 C125 188 160 200 210 168 S285 190 340 150 S420 174 475 144 S555 158 610 126 S690 143 750 115 S820 125 875 100 L875 235 L55 235Z" fill="var(--chart-1)" opacity=".12" /><path d="M55 185 C120 180 155 173 205 154 S285 166 340 137 S420 153 470 119 S550 132 610 102 S685 115 745 88 S820 96 875 70" fill="none" stroke="var(--chart-2)" strokeWidth="3" /><path d="M55 195 C150 187 230 192 310 178 S470 184 550 165 S720 174 875 154" fill="none" stroke="var(--muted-foreground)" strokeWidth="2" strokeDasharray="7 6" /><path d="M670 25V245" stroke="var(--chart-2)" strokeDasharray="5 5" /><g fill="var(--muted-foreground)" fontSize="10"><text x="55" y="265">Jan</text><text x="130" y="265">Feb</text><text x="205" y="265">Mar</text><text x="280" y="265">Apr</text><text x="355" y="265">May</text><text x="430" y="265">Jun</text><text x="505" y="265">Jul</text><text x="580" y="265">Aug</text><text x="655" y="265">Sep</text><text x="730" y="265">Oct</text><text x="805" y="265">Nov</text><text x="855" y="265">Dec</text><text x="58" y="18">Historical</text><text x="690" y="18" fill="var(--chart-2)">Forecast</text><text x="5" y="238">—</text><text x="0" y="188">—</text><text x="5" y="138">—</text><text x="0" y="88">—</text><text x="5" y="38">—</text></g>{([] as number[]).map((y, i) => <circle key={i} cx={55 + i * 100} cy={y} r="3.5" fill="var(--foreground)" />)}</svg></div><p className="mt-3 text-[11px] text-muted-foreground">Historical data based on connected financial accounts · Forecast uses recurring data and expected payments</p></Card>
 <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Cash Inflows" subtitle="Money In by source · Last 30 days" badge={<Badge tone="green">Observed</Badge>}>{table(liveLoading ? [] : inflows, ['Source', 'Amount', '%', 'Growth', 'Count'])}<div className="mt-4 flex h-3 overflow-hidden rounded-full"><span className="w-[35%] bg-chart-4" /><span className="w-[30%] bg-chart-4" /><span className="w-[20%] bg-chart-4" /><span className="w-[10%] bg-chart-4/20" /><span className="w-[5%] bg-secondary" /></div><p className="mt-2 text-[11px] font-semibold text-muted-foreground">Cash Inflows by Source <strong className="float-right text-foreground">Total —</strong></p></Card><Card title="Cash Outflows" subtitle="Money Out by category · Last 30 days" badge={<Badge tone="green">Observed</Badge>}>{table(liveLoading ? [] : outflows, ['Category', 'Amount', '%', 'Growth', 'Count'])}<div className="mt-4 flex h-3 overflow-hidden rounded-full"><span className="w-[29%] bg-primary text-primary-foreground" /><span className="w-[18%] bg-primary text-primary-foreground" /><span className="w-[13%] bg-primary text-primary-foreground" /><span className="w-[40%] bg-secondary" /></div><p className="mt-2 text-[11px] font-semibold text-muted-foreground">Cash Outflows by Category <strong className="float-right text-foreground">Total —</strong></p></Card></div>
 <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Net Cash Flow" subtitle="Period breakdown · Last 30 days"><div className="mb-3 flex w-fit gap-1 rounded-lg bg-secondary p-1">{['Daily', 'Weekly', 'Monthly'].map(x => <button key={x} onClick={() => setView(x)} className={`rounded-md px-3 py-1 text-[11px] font-bold ${view === x ? 'bg-card text-foreground shadow-sm' : 'text-foreground'}`}>{x}</button>)}</div>{table(liveLoading ? [] : periods, ['Period', 'Money In', 'Money Out', 'Net Cash Flow', 'Change'], true)}</Card><Card title="Cash Flow by Account" subtitle="Live account data unavailable · Last 30 days" badge={<Badge tone="green">Observed</Badge>}>{table(liveLoading ? [] : accounts, ['Account', 'Money In', 'Money Out', 'Net Movement', 'Closing Balance', 'Action'])}<p className="mt-3 text-[11px] text-muted-foreground">Values converted to EUR · Do not duplicate Accounts module</p></Card></div>
 <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Cash Flow Forecast" subtitle="Next 30 days · Based on recurring data and expected payments" badge={<Badge tone="violet">Forecast</Badge>}><div className="mb-4 flex gap-1 rounded-lg bg-secondary p-1 w-fit">{['7d', '30d', '60d', '90d', '6M', '12M'].map(x => <button key={x} onClick={() => setHorizon(x)} className={`rounded-md px-2.5 py-1 text-[11px] font-bold ${horizon === x ? 'bg-card text-foreground shadow-sm' : 'text-foreground'}`}>{x}</button>)}</div><div className="space-y-2 text-xs">{([] as string[][]).map(([a, b, c]) => <div key={a} className="flex justify-between border-b border-border pb-2"><span>{a}</span><strong className={c === 'Forecast' ? 'text-foreground' : b.startsWith('+') ? 'text-foreground' : ''}>{b} <span className="ml-2 text-[10px] font-normal text-muted-foreground">{c}</span></strong></div>)}</div><div className="mt-5"><div className="flex justify-between text-xs"><span>Forecast Confidence</span><Badge tone="green">HIGH</Badge></div><div className="mt-2 h-2 rounded-full bg-secondary"><div className="h-full w-0 rounded-full bg-chart-4" /></div><p className="mt-2 text-[11px] italic text-muted-foreground">Confidence is unavailable until live cash-flow history is connected.</p></div><div className="mt-4 flex flex-wrap gap-1.5">{['Historical cash flow', 'Recurring income', 'Recurring expenses', 'Outstanding invoices', 'Scheduled payouts'].map(x => <Badge key={x}>{x}</Badge>)}</div></Card><Card title="Upcoming Cash Movements" subtitle="Expected in the next 30 days">{table(liveLoading ? [] : movements, ['Date', 'Description', 'Type', 'Amount', 'Account', 'Status'])}</Card></div>
 <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Cash Flow Risks" subtitle="No live risks detected yet · AI-analyzed"><div className="space-y-3">{([] as string[][]).map(r => <div key={r[0]} className="rounded-lg border border-border p-3"><div className="flex items-start gap-2"><AlertTriangle size={16} className="mt-0.5 text-foreground" /><div className="flex-1"><p className="text-xs font-bold">{r[0]}</p><p className="mt-1 text-[11px] text-muted-foreground">{r[1]} · Evidence: {r[3]} · Affected: {r[4]}</p></div><Badge tone={r[5] as Tone}>{r[2]}</Badge></div><div className="mt-3 flex gap-2"><button className="btn-secondary">Investigate</button></div></div>)}</div><p className="mt-4 text-[11px] italic text-muted-foreground">AI-detected · Does not constitute regulated financial advice.</p></Card><Card title="Cash Flow Anomalies" subtitle="No live anomalies detected yet"><div className="space-y-3">{([] as string[][]).map(r => <div key={r[0]} className="rounded-lg border border-border p-3"><p className="text-xs font-bold">{r[0]} <span className="ml-2 font-normal text-muted-foreground">{r[1]}</span></p><p className="mt-1 text-[11px] text-muted-foreground">{r[2]}</p><div className="mt-3 flex flex-wrap gap-2"><button className="btn-secondary">Investigate</button><button className="btn-secondary">Open Transactions</button></div></div>)}</div></Card></div>
 <Card title="Cash Flow Breakdown" subtitle="How your cash position changed this period · Calculated" badge={<Badge tone="violet">Calculated</Badge>} className="mt-6"><div className="grid max-w-3xl gap-2 text-sm sm:grid-cols-2">{([] as string[][]).map(r => <div key={r[0]} className="flex justify-between border-b border-border pb-1"><span>{r[0]}</span><strong className={r[2]}>{r[1]}</strong></div>)}</div><p className="mt-4 text-[11px] text-muted-foreground">Every value traceable to connected financial accounts · Observed and Calculated data only.</p></Card>
 <Card title="Scenario Analysis" subtitle="Explore hypothetical changes to cash flow" badge={<Badge tone="slate">Scenario unavailable</Badge>} className="mt-6"><p className="mb-4 rounded-lg bg-secondary p-3 text-xs font-semibold text-foreground">Scenario analysis will be available when live cash-flow records are connected.</p><div className="grid gap-4 xl:grid-cols-2">{([] as string[][]).map(r => <div key={r[0]} className="rounded-lg border border-border p-4"><h3 className="text-sm font-bold">{r[0]}</h3><div className="mt-3 space-y-2 text-xs">{[['Money In', r[1]], ['Money Out', r[2]], ['Net Cash Flow', r[3]], ['Forecasted Closing', r[4]]].map(x => <div key={x[0]} className="flex justify-between"><span>{x[0]}</span><strong className={x[0] === 'Net Cash Flow' ? 'text-chart-5' : ''}>{x[1]}</strong></div>)}</div><Badge tone="slate">No live scenario result</Badge></div>)}</div><button className="btn-secondary mt-4 inline-flex items-center gap-2"><RefreshCw size={14} /><span>Reset Scenario</span></button></Card>
 <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="AI Cash Flow Insights" subtitle="AI-generated · Evidence-backed" badge={<Badge tone="violet">AI Insight</Badge>}><div className="space-y-3">{([] as string[]).map(t => <div key={t} className="rounded-lg border border-border bg-secondary/60 p-3"><p className="text-xs font-semibold leading-5 text-foreground">{t}</p><p className="mt-2 text-[11px] text-foreground">Evidence will appear when live cash-flow records are available.</p></div>)}</div><p className="mt-4 text-[11px] italic text-muted-foreground">AI-generated · Does not claim unsupported causes.</p></Card><Card title="AI Recommendations" subtitle="AI Recommendation" badge={<Badge tone="violet">AI Recommendation</Badge>}><div className="space-y-3">{([] as string[]).map((t, i) => <button key={t} className="flex w-full items-center gap-3 rounded-lg border border-border p-3 text-left"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-secondary text-xs font-bold text-foreground">{i + 1}</span><span className="text-xs font-semibold">{t}</span><ChevronRight size={14} className="ml-auto text-muted-foreground" /></button>)}</div><p className="mt-4 text-[11px] italic text-muted-foreground">AI Recommendation · AI does not initiate transfers or financial commitments.</p></Card></div>
 <div className="h-12" /></div></main><style>{`.btn-primary{border-radius:.5rem;background:var(--primary);padding:.65rem .85rem;font-size:.7rem;font-weight:700;color:white}.btn-primary:hover{background:var(--primary);color:var(--primary-foreground)}.btn-secondary{border-radius:.5rem;border:1px solid var(--border);background:white;padding:.55rem .7rem;font-size:.68rem;font-weight:700;color:var(--muted-foreground)}.btn-secondary:hover{border-color:var(--foreground);color:var(--foreground)}`}</style></div>;
}
