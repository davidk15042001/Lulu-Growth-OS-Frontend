...
export const LuluTerritories = () => {
  ...
  return <div className="min-h-screen bg-[var(--background)] font-sans text-foreground">
    <aside className={`fixed inset-y-0 left-0 z-30 w-[244px] bg-[var(--sidebar)] px-4 py-5 text-foreground transition-transform lg:translate-x-0 ${mobileNav ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="space-y-7">{navGroups.map(group => <div key={group.label}><p className="mb-2 px-2 text-[10px] font-semibold uppercase tracking-[.16em] text-muted-foreground">{group.label}</p><ul className="space-y-1">{group.items.map(({
              label,
              icon: Icon
            }) => <li key={label}><button className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm transition ${label === 'Territories' ? 'bg-secondary/20 font-semibold text-foreground ring-1 ring-ring/30' : 'text-foreground hover:bg-primary hover:text-primary-foreground'}`}><Icon size={16} /><span>{label}</span></button></li>)}</ul></div>)}</nav>
