...
const vendors: Vendor[] = [{
  ...
  spend: '14,200 EUR',
  current: '1,800 EUR',
  outstanding: '1,800 EUR',
  recurring: '1,800 EUR / mo',
  expenses: '18',
  invoices: '9',
  payment: '01 Aug 2026',
  status: 'Active'
  ...
  current: '330 EUR',
  outstanding: '0 EUR',
  recurring: '330 EUR / mo',
  expenses: '12',
  invoices: '12',
  payment: '05 Aug 2026',
  status: 'Active'
}, {
  name: 'OldSupplier Ltd',
  type: 'Supplier',
  spend: '1,200 EUR',
  current: '0 EUR',
  outstanding: '0 EUR',
...
const expenses = [['Creative Cloud renewal', '08 Aug 2026', 'Software', '1,250 EUR', 'Operations', 'Approved'], ['Express shipping', '06 Aug 2026', 'Logistics', '480 EUR', 'Operations', 'Approved'], ['CRM seats', '01 Aug 2026', 'Software', '1,800 EUR', 'Sales', 'Pending']];
const invoices = [['INV-2026-0841', '08 Aug 2026', '22 Aug 2026', '1,250 EUR', 'Received', 'Unpaid'], ['SF-44820', '01 Aug 2026', '15 Aug 2026', '1,800 EUR', 'Received', 'Due soon'], ['DHL-90831', '22 Jul 2026', '05 Aug 2026', '480 EUR', 'Received', 'Paid']];
const payments = [['Adobe payment', '08 Aug 2026', '1,250 EUR', 'Operations', 'Bank transfer', 'Completed'], ['Vodafone direct debit', '05 Aug 2026', '330 EUR', 'Operations', 'Direct debit', 'Completed'], ['DHL Express payment', '22 Jul 2026', '480 EUR', 'Operations', 'Card', 'Completed']];
const categories = [['Software', '36,800 EUR', '39%', '+18%', '48'], ['Professional Services', '24,200 EUR', '26%', '+6%', '22'], ['Logistics', '18,460 EUR', '20%', '-4%', '31'], ['Utilities', '9,680 EUR', '10%', '+2%', '18'], ['Other', '4,920 EUR', '5%', '+1%', '12']];
const nav = [{
  label: 'Overview',
...
export function LuluVendors() {
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground">
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-[226px] border-r border-border/80 bg-card lg:flex lg:flex-col"><div className="flex h-[72px] items-center gap-3 border-b border-border px-6"><div className="grid h-8 w-8 place-items-center rounded-xl bg-[var(--primary)] text-primary-foreground"><Sparkles size={16} /></div><span className="text-[17px] font-bold tracking-tight text-[var(--foreground)]">lulu<span className="text-foreground">.</span>ai</span></div><div className="px-3 py-5"><p className="px-3 pb-3 text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground">Workspace</p>{nav.map(({
          label,
          icon: Icon,
          open
        }) => <div key={label}><button className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] font-medium transition ${label === 'Finance' ? 'bg-secondary text-foreground' : 'text-foreground hover:bg-card hover:text-foreground'}`}><Icon size={17} strokeWidth={1.8} /><span>{label}</span>{open && <ChevronDown className="ml-auto" size={14} />}</button>{open && <div className="ml-9 border-l border-border py-1 pl-3"><button className="w-full rounded-md bg-primary px-3 py-2 text-left text-[12px] font-semibold text-primary-foreground">Vendors</button><button className="w-full px-3 py-2 text-left text-[12px] text-foreground">Transactions</button><button className="w-full px-3 py-2 text-left text-[12px] text-foreground">Accounts</button></div>}</div>)}</div><div className="mt-auto border-t border-border p-4"><div className="flex items-center gap-3 rounded-xl bg-card p-3"><div className="grid h-8 w-8 place-items-center rounded-full bg-[var(--secondary)] text-xs font-bold text-foreground">JD</div><div><p className="text-xs font-semibold text-foreground">Jordan Davis</p><p className="text-[10px] text-muted-foreground">Administrator</p></div><MoreHorizontal size={15} className="ml-auto text-muted-foreground" /></div></div></aside>
    <main className="lg:pl-[226px]"><header className="sticky top-0 z-10 flex h-[72px] items-center justify-between border-b border-border/80 bg-secondary px-5 backdrop-blur lg:px-8"><div className="flex items-center gap-3"><button className="rounded-lg p-2 text-foreground lg:hidden"><Menu size={19} /></button><div className="flex items-center gap-2 text-xs text-muted-foreground"><span>Finance</span><ChevronRight size={13} /><strong className="font-semibold text-foreground">Vendors</strong></div></div><div className="flex items-center gap-4"><button aria-label="Help" className="text-foreground hover:text-foreground"><CircleHelp size={18} /></button><button aria-label="Notifications" className="relative text-foreground hover:text-foreground"><Bell size={18} /><span className="absolute -right-1 -top-1 h-1.5 w-1.5 rounded-full bg-primary text-primary-foreground" /></button><div className="hidden h-7 w-px bg-secondary sm:block" /><button className="flex items-center gap-2 text-xs font-semibold text-foreground"><span className="grid h-7 w-7 place-items-center rounded-full bg-[var(--primary)] text-[10px] text-primary-foreground">JD</span><span className="hidden sm:block">Jordan Davis</span><ChevronDown size={13} /></button></div></header>
