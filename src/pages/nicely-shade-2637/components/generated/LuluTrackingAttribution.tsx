...
export function LuluTrackingAttribution() {
 ...
 <section className="section health-card"><div className="health-layout"><div><div className="ring"><div className="ring-text"><strong>94 / 100</strong><span>Tracking Health Score</span></div></div></div><div><span className="healthy">Healthy</span><p className="support">Advertising conversion tracking is operating normally across connected platforms.</p><div className="stat-grid"><div><strong>4</strong><span>Platforms Monitored</span></div><div><strong>7</strong><span>Sources Active</span></div><div><strong>2</strong><span>Issues Detected</span></div><div><strong>Today 09:42</strong><span>Last Check</span></div></div><div className="ai-note">AI Inferred · Score based on connected source health and event completeness</div></div><Button primary onClick={runCheck}>Run Tracking Check</Button></div></section>
 <section className="kpi-grid section"><div className="kpi"><label>Conversion Events</label><strong>18</strong><span>Active tracked events</span></div><div className="kpi"><label>Active Tracking Sources</label><strong>7</strong><span>Sending data</span></div><div className="kpi"><label>Tracking Issues</label><strong>2</strong><span>Require attention</span></div><div className="kpi"><label>Data Freshness</label><strong>22 min</strong><span>Latest sync</span></div><div className="kpi"><label>Attribution Coverage</label><strong>91%</strong><span>Conversions attributed</span></div></section>
 <section className="section"><div className="section-head"><h2>Tracking Sources</h2><Button primary icon={<Plus size={13} />}>Connect Source</Button></div><div className="table-wrap"><table className="data-table"><thead><tr>{['Source', 'Platform', 'Status', 'Last Sync', 'Events Received', 'Data Freshness', 'Issues', 'Actions'].map(x => <th key={x}>{x}</th>)}</tr></thead><tbody>{sources.map(row => <tr key={row[0]}><td><strong>{row[0]}</strong></td><td>{row[1]}</td><td><Status>{row[2]}</Status></td><td>{row[3]}</td><td>{row[4]}</td><td><Status>{row[5]}</Status></td><td>{row[6]}</td><td><button className="row-action">View</button>{row[6] !== 'None' && <button className="row-action" onClick={() => setModal('issue')}>Investigate</button>}</td></tr>)}</tbody></table></div></section>
 <section className="section"><div className="section-head"><h2>Conversion Events</h2><div className="heading-actions"><div className="searchbox"><Search size={13} /><input placeholder="Search events..." value={search} onChange={e => setSearch(e.target.value)} /></div><Button primary icon={<Plus size={13} />}>Add Event</Button></div></div><div className="table-wrap"><table className="data-table"><thead><tr>{['Event', 'Source', 'Platform', 'Status', 'Events (30d)', 'Last Received', 'Attribution', 'Issues', 'Actions'].map(x => <th key={x}>{x}</th>)}</tr></thead><tbody>{events.filter(r => r[0].toLowerCase().includes(search.toLowerCase())).map(row => <tr key={row[0] + row[1]}><td><strong>{row[0]}</strong></td><td>{row[1]}</td><td>{row[2]}</td><td><Status>{row[3]}</Status></td><td>{row[4]}</td><td>{row[5]}</td><td>{row[6]}</td><td>{row[7]}</td><td><button className="row-action" onClick={() => setModal('event')}>View</button>{row[7] !== 'None' && <button className="row-action" onClick={() => setModal('issue')}>Investigate</button>}</td></tr>)}</tbody></table></div><div className="chart-label">Showing 8 of 18 events</div></section>
 <section className="section"><div className="section-head"><h2>Tracking Issues</h2><div className="tabs">{['All', 'Critical', 'High', 'Medium', 'Low'].map(x => <button className={'tab model-tabs ' + (severity === x ? 'active' : '')} onClick={() => setSeverity(x)} key={x}>{x}</button>)}</div></div><div className="issue-list"><article className="issue"><div><span className="badge">MEDIUM</span><h3>Meta Conversions API reporting delay</h3><p>Source: Meta Conversions API · Platform: Meta Ads · Detected: Jun 22, 2025 07:28 · Last event: 2h 14m ago</p><p><strong>Impact:</strong> Contact Request and Purchase events from Meta CAPI may be underreported in the last 2 hours.</p></div><div><Button onClick={() => setModal('issue')}>Investigate</Button><Button onClick={() => setModal('task')}>Create Task</Button></div></article><article className="issue"><div><span className="badge">MEDIUM</span><h3>TikTok Pixel event delay detected</h3><p>Source: TikTok Pixel · Platform: TikTok Ads · Detected: Jun 22, 2025 05:10 · Last event: 4h 32m ago</p><p><strong>Impact:</strong> Registration events from TikTok Pixel have not been received for over 4 hours.</p></div><div><Button onClick={() => setModal('issue')}>Investigate</Button><Button onClick={() => setModal('task')}>Create Task</Button></div></article></div></section>
 <section className="section"><div className="section-head"><h2>Conversion Anomalies</h2><span className="badge"><Sparkles size={11} /> AI DETECTED</span></div>{['TikTok Pixel Registration events have not been received for 4+ hours.', 'Meta Conversions API Purchase events are 22% lower than the platform-reported 7-day average.'].map(x => <article className="anomaly" key={x}><div><strong>{x}</strong><p>Severity: Medium · Platform: {x.startsWith('TikTok') ? 'TikTok Ads' : 'Meta Ads'} · Detected: Jun 22, 09:42</p><Integrity>AI Detected</Integrity></div><div><Button onClick={() => setModal('issue')}>Investigate</Button><Button onClick={() => setModal('task')}>Create Task</Button></div></article>)}<div className="chart-label">Anomalies are detected by Lulu AI based on available event data. Label: AI Detected.</div></section>
