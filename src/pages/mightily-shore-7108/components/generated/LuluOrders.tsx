...
export function LuluOrders() {
  ...
  <main className="lg:ml-[246px]"><header className="border-b border-border/80 bg-card px-5 py-5 sm:px-8"><div className="flex items-start justify-between gap-4"><div className="flex items-start gap-3"><button onClick={() => setMobileNav(true)} className="mt-1 rounded-lg p-1.5 hover:bg-sidebar lg:hidden" aria-label="Open navigation"><Menu className="h-5 w-5" /></button><div><p className="mb-2 text-xs font-medium text-muted-foreground">Ecommerce <span className="px-1 text-foreground">/</span> <span className="text-muted-foreground">Orders</span></p><h1 className="text-[27px] font-bold tracking-[-.045em] text-foreground">Orders</h1><p className="mt-1 text-[13px] text-muted-foreground">Manage and monitor orders across all connected ecommerce stores.</p></div></div><div className="hidden items-center gap-2 sm:flex"><button aria-label="Refresh" className="rounded-lg border border-border p-2 text-foreground hover:bg-card"><RefreshCw className="h-4 w-4" /></button><button aria-label="More actions" className="rounded-lg border border-border p-2 text-foreground"><Ellipsis className="h-4 w-4" /></button></div></div></header>
