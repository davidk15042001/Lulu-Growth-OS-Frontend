...
export const LuluAudiencesWorkspace = () => {
  ...
  <aside className={`${mobileNav ? 'flex' : 'hidden'} fixed inset-y-0 left-0 z-20 w-[220px] flex-col bg-[var(--sidebar)] px-3 py-5 lg:flex`}>
   ...
   <nav className="flex-1 space-y-5" aria-label="Main navigation">{navGroups.map(group => <div key={group.label}><p className="mb-2 px-3 text-[10px] font-medium tracking-[.12em] text-[var(--muted-foreground)]">{group.label}</p><div className="space-y-0.5">{group.items.map(([label, Icon]) => <button key={label as string} onClick={() => setMobileNav(false)} className={`flex h-9 w-full items-center gap-3 border-l-2 px-3 text-left text-sm transition ${label === 'Audiences' ? 'border-[var(--border)] bg-[var(--primary)] text-[var(--primary-foreground)]' : 'border-transparent text-[var(--primary-foreground)] hover:bg-[var(--primary)] hover:text-primary-foreground'}`}><Icon size={15} /><span>{label as string}</span></button>)}</div></div>)}</nav>
