  ...
  return <div className="lulu-app">
    ...
    <main className="chat-column">
      ...
      <div className="messages" role="log" aria-live="polite" aria-label="Conversation">
        ...
        <article className="ai-message"><Avatar /><div className="ai-content"><div className="response-card"><header><strong>Business Performance Overview</strong><span className="ai-badge">AI</span></header><p className="summary">Your business is performing well today. Revenue is up — compared to the same period last month, driven primarily by strong organic search performance and a recovery in email marketing conversions.</p><div className="kpi-row">{kpis.map(kpi => <div className="mini-kpi" key={kpi.label}><span>{kpi.label}</span><strong>{kpi.value}</strong><em className={kpi.tone}>{kpi.delta}</em><small className={kpi.tone}>{kpi.tone === 'good' ? 'Good' : 'Watch'}</small></div>)}</div><h2>Key Highlights</h2><ul className="highlights"><li>Organic search traffic up —</li><li>Email campaign ROAS improved to 7.2x</li><li>Advertising spend efficiency slightly decreased (—)</li><li>Customer lifetime value trending upward</li></ul><div className="risk-callout"><strong>⚠ Advertising ROAS has declined — compared to last period.</strong> Review recommended.</div><div className="source-row"><SectionLabel>SOURCES</SectionLabel>{sources.map(source => <span className="source-pill" key={source.name}><i style={{
