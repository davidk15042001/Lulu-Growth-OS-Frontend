...
const metrics: Array<{ tone: Tone; [key: string]: any }> = [];
...
export function LuluPaymentsWorkspace() {
  ...
  const { items: paymentRecords, loading, error, refresh: refreshPayments } = useLiveRecords('ecommerce_payments');
  const [refreshing, setRefreshing] = useState(false);
  const refresh = () => { void refreshPayments(); setRefreshing(true); window.setTimeout(() => setRefreshing(false), 700); };
  const shown = payments.filter(row => `${row.id} ${row.customer} ${row.order} ${row.store}`.toLowerCase().includes(query.toLowerCase()));
  if (loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live payment records…</main>;
