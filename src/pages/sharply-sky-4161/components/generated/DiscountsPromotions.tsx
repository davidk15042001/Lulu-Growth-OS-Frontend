...
function Sidebar() {
  return <aside className="fixed inset-y-0 left-0 z-20 flex w-14 flex-col items-center bg-[var(--sidebar)] py-4 text-muted-foreground"><div className="mb-7 flex h-8 w-8 items-center justify-center rounded-lg bg-primary font-bold text-primary-foreground">L</div><nav className="flex flex-1 flex-col items-center gap-2" aria-label="Primary navigation">{navItems.map(({
        label,
        icon: Icon
      }) => <button key={label} aria-label={label} title={label} className={`relative flex h-9 w-9 items-center justify-center rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${label === 'Ecommerce' ? 'bg-secondary/20 text-foreground before:absolute before:-left-2 before:h-6 before:w-0.5 before:rounded-full before:bg-primary' : 'hover:bg-secondary hover:text-primary-foreground'}`}><Icon size={18} /></button>)}</nav><div className="h-8 w-8 rounded-full bg-sidebar text-center text-xs leading-8 text-foreground">JD</div></aside>;
}
