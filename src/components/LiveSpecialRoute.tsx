...
type SpecialKind = "metrics" | "integrations";

export function LiveSpecialRoute({ kind }: { kind: SpecialKind }) {
  const { selectedWorkspace, loading, error, refresh } = useLuluApp();
  const label = kind === "metrics" ? "Metrics" : "Integrations";
  const Icon = kind === "metrics" ? BarChart3 : GitBranch;
  ...
  return <main className="min-h-screen bg-[var(--background)] px-5 py-8 text-[var(--foreground)] sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl"><header className="mb-6 border-b border-[var(--border)] pb-6"><p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--muted-foreground)]">Live workspace data</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.04em]">{label}</h1><p className="mt-2 max-w-2xl text-sm text-[var(--muted-foreground)]">Only data returned by the connected workspace is displayed. No sample data is used.</p></header>{kind === "metrics" ? <MetricsPanel workspaceId={selectedWorkspace.id} onClose={() => undefined} /> : <IntegrationsPanel workspaceId={selectedWorkspace.id} onClose={() => undefined} />}</div></main>;
}
