...
export function LuluCreatives() {
  ...
  if (liveEmpty) return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-3xl rounded-2xl border border-dashed border-border bg-card p-8 text-center"><Sparkles className="mx-auto mb-4 text-muted-foreground" size={28} /><h1 className="text-2xl font-semibold">Creatives</h1><p className="mt-3 text-sm text-muted-foreground">No live creative assets are available yet. Connect a verified advertising platform before creating or syncing creatives.</p><button type="button" onClick={() => void refreshCreatives()} className="mt-5 inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm hover:bg-secondary"><RefreshCw size={15} /> Refresh</button></div></main>;
