...
const configRows = [['Connected store', 'Shopify', 'Tax-Exclusive', 'Enabled', '8 jurisdictions', 'Shopify Tax', '2 min ago', 'Configured'], ['Lulu WooCommerce', 'WooCommerce', 'Tax-Inclusive', 'Enabled', '3 jurisdictions', 'WooCommerce Tax', '15 min ago', 'Partially Configured'], ['Lulu Webflow', 'Webflow', 'Tax-Exclusive', 'Disabled', '—', '—', '—', 'Not Configured']];
...
const jurisdictionRows: any[][] = [];
const collectionRows = [['Germany', '—', '—', '19%', '1,042', '—', 'Healthy'], ['France', '—', '—', '20%', '486', '—', 'Healthy'], ['United Kingdom', '—', '—', '20%', '390', '—', 'Healthy'], ['Austria', '—', '—', '20%', '214', '—', 'Healthy']];
const transactionRows = [['#10482', 'Max Mustermann', 'Connected store', 'Germany', '—', '19%', '—', 'Taxable', 'Platform', '10 Aug'], ['#10481', 'Sophie Martin', 'Connected store', 'France', '—', '20%', '—', 'Taxable', 'Platform', '10 Aug'], ['#10480', 'James Wilson', 'Connected store', 'United Kingdom', '—', '20%', '—', 'Taxable', 'Platform', '9 Aug'], ['#10479', 'Anna Schmidt', 'Lulu WooCommerce', 'Germany', '—', '0%', '—', 'Zero Rated', 'Platform', '9 Aug'], ['#10478', 'Carlos Rivera', 'Connected store', 'United States — CA', '—', '8.25%', '—', 'Taxable', 'Configured Rule', '8 Aug']];
...
export function LuluTaxes() {
  ...
  const { items: taxRecords, loading, error, refresh: refreshTaxes } = useLiveRecords('finance_taxes');
  const [refreshing, setRefreshing] = useState(false);
  const refresh = () => { void refreshTaxes(); setRefreshing(true); window.setTimeout(() => setRefreshing(false), 700); };
  if (loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-sm text-muted-foreground">Loading live tax records…</main>;
  ...
  if (!loading && !error) return <main className="min-h-screen bg-[var(--background)] p-6 text-foreground sm:p-10"><div className="mx-auto max-w-6xl"><header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Ecommerce / Taxes</p><h1 className="mt-2 text-3xl font-bold">Taxes</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Verified tax records from connected finance sources. Rates, jurisdictions and obligations appear only when returned by the backend.</p></div><button type="button" onClick={refresh} className="inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm hover:bg-secondary"><RefreshCw size={15} className={refreshing ? 'animate-spin' : ''} /> Refresh</button></header>{taxRecords.length === 0 ? <section className="rounded-2xl border border-dashed border-border bg-card p-10 text-center"><TriangleAlert className="mx-auto mb-4 text-muted-foreground" size={30} /><h2 className="text-xl font-semibold">No verified tax records yet</h2><p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">Connect a finance platform and synchronize tax data before reviewing rates, jurisdictions or obligations.</p></section> : <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Tax record</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Stage</th><th className="px-4 py-3">Description</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{taxRecords.map(record => <tr key={record.id}><td className="px-4 py-3 font-medium">{record.name}</td><td className="px-4 py-3">{record.status}</td><td className="px-4 py-3 text-muted-foreground">{record.stage ?? '—'}</td><td className="max-w-md px-4 py-3 text-muted-foreground">{record.description ?? '—'}</td><td className="px-4 py-3 text-muted-foreground">{new Date(record.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>}</div></main>;
