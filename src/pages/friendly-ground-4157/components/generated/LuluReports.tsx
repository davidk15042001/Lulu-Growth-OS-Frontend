...
function Sidebar() {
  return <aside className="sidebar"><div className="brand"><span className="brand-mark">L</span><strong>LULU <em>AI</em></strong></div><div className="workspace"><span className="workspace-mark">N</span><span><strong>Connected workspace</strong><small>Enterprise workspace</small></span><ChevronDown size={14} /></div><LuluSectionNavigation activeId="friendly-ground-4157" /><div className="sidebar-bottom"><a><Database size={16} /> Settings</a><div className="user"><span className="avatar">AS</span><span><strong>Workspace member</strong><small>Admin</small></span><MoreHorizontal size={16} /></div></div></aside>;
}
function Status({
  status
}: {
...
export function LuluReports() {
  const { items, loading, error } = useLiveRecords('reports');
  return <div className="app-shell"><Sidebar /><main className="page"><header className="page-header"><div><div className="breadcrumbs"><span>Intelligence</span><ChevronRight size={13} /><span>Analytics</span><ChevronRight size={13} /><strong>Reports</strong></div><h1>Reports</h1><p>Create, analyze and share reports from connected workspace data.</p></div><div className="top-actions"><button className="button primary"><Sparkles size={15} /> Ask Lulu AI</button><button className="button accent"><Plus size={15} /> Create Report</button><button className="icon-button" aria-label="Refresh reports"><RefreshCw size={16} /></button></div></header>{loading ? <div className="rounded-xl border border-border bg-card p-8 text-center text-sm text-muted-foreground">Loading live reports…</div> : error ? <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center text-sm text-destructive">{error}</div> : !items.length ? <section className="flex min-h-[560px] items-center justify-center rounded-xl border border-dashed border-border bg-card p-10 text-center"><div><FileBarChart className="mx-auto text-muted-foreground" size={38} /><h2 className="mt-4 text-xl font-semibold text-foreground">No reports available yet</h2><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Create a report or connect data sources to populate this page. No example schedules, metrics, report narratives or dates are displayed.</p><button className="button accent mt-5"><Plus size={15} /> Create Report</button></div></section> : <section className="overflow-x-auto rounded-xl border border-border bg-card"><table className="w-full min-w-[820px] text-left"><thead className="border-b border-border bg-secondary text-[10px] uppercase tracking-wider text-muted-foreground"><tr><th className="px-5 py-3">Report</th><th className="px-3 py-3">Type</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Description</th><th className="px-3 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{items.map(record => <tr key={record.id}><td className="px-5 py-3 text-xs font-semibold text-foreground">{record.name}</td><td className="px-3 py-3 text-xs text-muted-foreground">{String(record.data?.type || 'Report')}</td><td className="px-3 py-3 text-xs text-muted-foreground">{record.status || 'Recorded'}</td><td className="px-3 py-3 text-xs text-muted-foreground">{record.description || 'Live report record'}</td><td className="px-3 py-3 text-xs text-muted-foreground">{record.updatedAt}</td></tr>)}</tbody></table></section>}</main></div>;
}
