...
const dueToday = [['09:00', 'Call Maria Chen about renewal', 'High', 'Nova Commerce', 'David Lee', 'Enterprise Platform'], ['10:30', 'Send enterprise proposal to Apex', 'Critical', 'Apex Global', 'Sarah Chen', 'Apex Expansion'], ['11:00', 'Follow up on pricing objection', 'High', 'DataFlow Inc', 'Marcus R.', 'DataFlow SaaS'], ['14:00', 'Demo with RetailMax stakeholders', 'High', 'RetailMax', 'James P.', 'RetailMax Q3'], ['15:00', 'Follow up on enterprise proposal', 'High', 'Nova Commerce', 'David Lee', 'Enterprise Platform']] as const;
const overdue = [['Reconnect with Apex decision maker', 'David Lee', 'Apex Global', 'Apex Expansion', 'Apex Platform', '3 days ago', '3 days', 'Critical', 'Contact decision maker before close date'], ['Update deal timeline for TechVision', 'Sarah C.', 'TechVision', 'TechVision Renewal', 'TechVision Ent.', '5 days ago', '5 days', 'High', 'Update deal record and reschedule'], ['Send ROI analysis to DataFlow', 'Marcus R.', 'DataFlow Inc', 'DataFlow SaaS', 'DataFlow Ent.', '2 days ago', '2 days', 'High', 'Prepare and send analysis'], ['Confirm meeting with CloudBase exec', 'Emma W.', 'CloudBase', 'CloudBase Growth', 'CloudBase Pro', '1 day ago', '1 day', 'Medium', 'Confirm attendance']] as const;
const tasks: Task[] = [{
  id: 't1',
...
export function SalesTasks() {
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground">
    <aside className={`fixed inset-y-0 left-0 z-30 w-[220px] border-r border-border bg-card transition-transform lg:translate-x-0 ${mobileNav ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="px-3 py-5" aria-label="Main navigation">{navGroups.map(group => <div key={group.label} className="mb-6"><p className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-[.16em] text-muted-foreground">{group.label}</p>{group.items.map(item => {
            const label = item[0];
            const Icon = item[1];
            return <button key={label} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-[13px] transition ${label === 'Tasks' ? 'bg-secondary font-semibold text-foreground' : 'text-foreground hover:bg-card hover:text-foreground'}`}><Icon size={16} strokeWidth={1.8} /><span>{label}</span>{label === 'Tasks' && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary text-primary-foreground" />}</button>;
          })}</div>)}</nav>
      <div className="absolute bottom-0 w-full border-t border-border p-4"><button className="flex items-center gap-3 text-sm text-foreground hover:text-foreground"><Settings2 size={17} /><span>Settings</span></button><div className="mt-4 flex items-center gap-3"><Avatar initials="JD" color="var(--muted-foreground)" /><div><p className="text-xs font-semibold">Jordan Davis</p><p className="text-[11px] text-muted-foreground">Administrator</p></div><MoreHorizontal className="ml-auto text-muted-foreground" size={17} /></div></div>
    </aside>
