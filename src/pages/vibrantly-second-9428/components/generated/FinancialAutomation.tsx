...
export function FinancialAutomation() {
  ...
  const { items: automationRecords, loading: automationsLoading, error: automationsError } = useLiveRecords('finance_automations');
  const getAutomationField = (record: typeof automationRecords[number], key: string) => String((record as unknown as Record<string, unknown>)[key] ?? '');
  const liveAutomations = automationRecords.map(record => [getAutomationField(record, 'name') || getAutomationField(record, 'automationName') || '—', getAutomationField(record, 'trigger') || '—', getAutomationField(record, 'actions') || '—', getAutomationField(record, 'status') || '—', getAutomationField(record, 'lastRun') || '—', getAutomationField(record, 'nextRun') || '—', getAutomationField(record, 'executions') || '—', getAutomationField(record, 'successRate') || '—', getAutomationField(record, 'owner') || '—']);
  const activeAutomations = liveAutomations.filter(row => row[3] === 'Active').length;
  const failedAutomations = liveAutomations.filter(row => row[3] === 'Failed').length;
  const successfulAutomations = liveAutomations.filter(row => row[3] === 'Successful' || row[3] === 'Completed').length;
  return <div className="min-h-screen bg-[var(--background)] text-foreground">
    <aside className="fixed inset-y-0 left-0 hidden w-[244px] flex-col bg-[var(--sidebar)] text-foreground lg:flex"><div className="flex h-16 items-center gap-3 border-b border-border px-6"><div className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-primary-foreground"><Sparkles size={17} /></div><strong className="text-lg tracking-tight text-foreground">LULU <span className="font-normal text-foreground">AI</span></strong></div><div className="px-4 py-5"><div className="mb-3 flex items-center justify-between px-2 text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground"><span>Workspace</span><Plus size={13} /></div><LuluSectionNavigation activeId="vibrantly-second-9428" /></div><div className="mt-auto border-t border-border p-4"><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-xs text-foreground hover:bg-secondary"><Settings2 size={15} />Workspace settings</button><div className="mt-3 flex items-center gap-3 rounded-lg bg-secondary p-3"><div className="grid h-7 w-7 place-items-center rounded-full bg-secondary text-xs font-bold text-foreground">—</div><div><p className="text-xs font-semibold text-foreground">Workspace owner</p><p className="text-[10px] text-muted-foreground">Admin</p></div><MoreHorizontal size={15} className="ml-auto" /></div></div></aside>
    <main className="lg:ml-[244px]">{automationsError && <div role="alert" className="mx-5 mt-4 rounded-lg border border-chart-5/30 bg-chart-5/10 px-4 py-3 text-sm text-chart-5 lg:ml-0">Finance automation data could not be loaded. Check finance automation records and try again.</div>}{!automationsLoading && !automationsError && liveAutomations.length === 0 && <></>}<div className="flex h-16 items-center justify-between border-b border-border bg-card px-5 lg:px-8"><div className="flex items-center gap-3"><PanelLeft size={18} className="text-muted-foreground lg:hidden" /><span className="text-xs text-muted-foreground">Business /</span><span className="text-xs font-semibold text-muted-foreground">Finance</span></div><div className="flex items-center gap-3"><div className="hidden items-center gap-2 rounded-lg border border-border px-3 py-2 md:flex"><Search size={14} className="text-muted-foreground" /><input placeholder="Search finance..." className="w-40 bg-transparent text-xs outline-none placeholder:text-muted-foreground" /></div><Activity size={17} className="text-muted-foreground" /><div className="grid h-7 w-7 place-items-center rounded-full bg-secondary text-[10px] font-bold text-muted-foreground">AM</div></div></div>
      <div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-8"><header className="mb-7 flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><p className="mb-3 text-xs font-medium text-muted-foreground">Finance <span className="mx-1">/</span> Financial Automation</p><h1 className="text-3xl font-bold tracking-[-0.04em] text-foreground">Financial Automation</h1><p className="mt-2 text-sm text-muted-foreground">Run repetitive financial processes autonomously with deterministic controls and complete audit trails.</p></div><div className="flex flex-wrap gap-2"><button className="inline-flex items-center gap-2 rounded-lg bg-primary px-3.5 py-2.5 text-xs font-bold text-primary-foreground shadow-sm hover:bg-primary"><Plus size={15} />Create Automation</button><button className="rounded-lg border border-border bg-card px-3 py-2.5 text-xs font-semibold text-foreground">Automation Templates</button><button className="rounded-lg border border-border bg-card p-2.5 text-foreground"><MoreHorizontal size={16} /></button></div></header>
        <div className="mb-7 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">{[['Active Automations', String(automationsLoading ? '—' : activeAutomations), 'green'], ['Executions', String(automationsLoading ? '—' : liveAutomations.length), 'slate'], ['Successful', String(automationsLoading ? '—' : successfulAutomations), 'green'], ['Failed', String(automationsLoading ? '—' : failedAutomations), 'red'], ['Human Approvals', '0', 'green'], ['Estimated Time Saved', '—', 'violet']].map(([label, value, tone]) => <article key={label} className="rounded-xl border border-border bg-card p-4 shadow-[0_2px_8px_rgba(0,0,0,0.03)]"><div className="flex items-start justify-between"><p className="text-xs font-semibold text-muted-foreground">{label}</p><span className={`h-2 w-2 rounded-full ${tone === 'green' ? 'bg-chart-4' : tone === 'red' ? 'bg-destructive' : tone === 'amber' ? 'bg-chart-1' : 'bg-primary'}`} /></div><p className="mt-4 text-[22px] font-bold tracking-[-0.04em] text-foreground">{value}</p><p className="mt-1 text-[11px] text-muted-foreground">{label === 'Successful' ? 'Live successful records' : label === 'Failed' ? 'Live failed records' : label === 'Executions' ? 'Live execution records' : label === 'Estimated Time Saved' ? 'Available with live execution data' : 'Live workspace data'}</p></article>)}</div>
        ...
        <div className="my-6 grid grid-cols-1 gap-6 xl:grid-cols-2"><Card title="Automation Performance" action="This Month"><div className="grid grid-cols-2 gap-4 md:grid-cols-4">{([] as string[][]).map(item => <div key={item[0]}><p className="text-[11px] text-muted-foreground">{item[0]}</p><strong className="mt-2 block text-xl tracking-tight">{item[1]}</strong>{item[0] === 'Estimated time saved' && <span className="text-[10px] text-muted-foreground">Estimated</span>}</div>)}</div><div className="mt-6 flex h-32 items-end gap-3 border-b border-border px-3">{([] as number[]).map((height, i) => <div key={`bar-${height}-${i}`} className="group flex flex-1 items-end"><div style={{
                  height: `${height}px`
                }} className="w-full rounded-t-md bg-secondary transition group-hover:bg-primary" /></div>)}</div><div className="mt-2 flex justify-center text-[10px] text-muted-foreground"><span>Live time range appears after automation data is available.</span></div></Card><Card title="Versions & Activity"><div className="mb-4 flex gap-1 rounded-lg bg-secondary p-1"><button className="rounded-md bg-card px-3 py-1.5 text-xs font-bold text-foreground shadow-sm">Versions</button><button className="px-3 py-1.5 text-xs font-semibold text-foreground">Activity</button></div><div className="space-y-2">{versions.map(row => <div key={row[0]} className="grid grid-cols-[40px_1fr_1fr_auto] items-center gap-3 border-b border-border py-2 text-[11px] last:border-0"><strong>{row[0]}</strong><span className="text-muted-foreground">{row[1]} · {row[2]}</span><span className="text-muted-foreground">{row[3]}</span><Badge tone={row[4] === 'Active' ? 'green' : 'slate'}>{row[4]}</Badge></div>)}</div><div className="mt-4 border-t border-border pt-4">{activity.map(item => <div key={item[0]} className="flex items-center gap-3 py-2 text-[11px]"><div className="h-2 w-2 rounded-full bg-primary text-primary-foreground" /><strong>{item[0]}</strong><span className="ml-auto text-muted-foreground">{item[1]}</span></div>)}</div></Card></div>
        
        
        <footer className="pb-8 text-center text-[11px] text-muted-foreground">Financial automations execute autonomously inside system permissions and audit policies. Only adding paid-media budget requires customer authorization.</footer>
      </div></main>
  </div>;
}
