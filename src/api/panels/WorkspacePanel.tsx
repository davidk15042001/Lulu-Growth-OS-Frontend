...
type Tab = "overview" | "notifications" | "team" | "audit";
...
export function WorkspacePanel({ workspaceId, onClose }: { workspaceId: string; onClose: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [notificationResponse, memberResponse] = await Promise.all([
        notificationApi.list(workspaceId), workspaceAppApi.members(workspaceId),
      ]);
      setNotifications(notificationResponse.data.items); setUnread(notificationResponse.data.unread);
      setMembers(memberResponse.data.members); setInvitations(memberResponse.data.invitations);
  ...
  return <LivePanelShell title="Live workspace" subtitle={bootstrap?.workspace.companyName ?? "Workspace operations"} onClose={onClose}>
    ...
    <nav className="lulu-live-tabs" aria-label="Workspace live-data sections">{(["overview", "notifications", "team", "audit"] as Tab[]).map((item) => <button type="button" key={item} aria-selected={tab === item} onClick={() => setTab(item)}>{item}</button>)}</nav>
    {tab === "overview" && <LiveSection title="Workspace summary" action={<span className="lulu-live-message">Use Update in the navigation bar.</span>}>
      {!bootstrap ? <LiveEmpty>Loading workspace…</LiveEmpty> : <><div className="lulu-live-kpis"><div className="lulu-live-kpi"><span>Records</span><strong>{bootstrap.records.total}</strong></div><div className="lulu-live-kpi"><span>Members</span><strong>{bootstrap.members.total}</strong></div><div className="lulu-live-kpi"><span>Unread</span><strong>{bootstrap.notifications.unread}</strong></div><div className="lulu-live-kpi"><span>Approval gates</span><strong>0</strong></div></div><div className="lulu-live-row"><strong>Role: {bootstrap.permissions.role}</strong><span>AI {bootstrap.capabilities.aiGeneration ? "enabled" : "not configured"} · Email {bootstrap.capabilities.transactionalEmail ? "enabled" : "not configured"}</span></div>{bootstrap.recentActivity.map((entry) => <div className="lulu-live-row" key={entry.id}><strong>{entry.action}</strong><span>{entry.entityType}</span><small>{formatLiveDate(entry.createdAt)}</small></div>)}</>}
    </LiveSection>}
