...
export function LuluCollectionsPage() {
  ...
  return <main className="min-h-screen bg-[var(--background)] text-foreground">{liveLoading ? <div className="border-b border-border bg-secondary/30 px-5 py-3 text-xs text-muted-foreground lg:pl-[268px]">Loading live collections…</div> : liveError ? <div className="border-b border-destructive/30 bg-destructive/5 px-5 py-3 text-xs text-destructive lg:pl-[268px]">{liveError}</div> : liveEmpty ? <div className="border-b border-dashed border-border bg-card px-5 py-3 text-xs text-muted-foreground lg:pl-[268px]">No live collections are available yet. Add a collection or connect your commerce platform to begin.</div> : null}
    ...
    <div><div className="mx-auto max-w-[1540px] px-5 py-5 sm:px-8 lg:px-10">
      ...
      <section className="ai-panel mt-7"><div className="flex items-start gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-secondary"><Sparkles className="h-5 w-5" /></span><div className="flex-1"><h2 className="text-xl font-bold">Ask Lulu AI</h2><p className="mt-1 text-sm text-foreground">Review and manage your collections with AI.</p><label className="mt-5 flex items-center gap-3 rounded-lg bg-card p-3 text-muted-foreground"><Bot className="h-5 w-5 text-muted-foreground" /><input className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="Ask Lulu AI about this collection..." /><button type="button" className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Ask</button></label><div className="mt-4 flex flex-wrap gap-2">{['Find empty collections', 'Find collections with incomplete information', 'Find duplicate collections', 'Find outdated collections', 'Compare collections across stores', "Explain this collection's rules"].map(prompt => <button key={prompt} className="rounded-full border border-border bg-secondary px-3 py-1.5 text-xs text-foreground hover:bg-secondary">{prompt}</button>)}</div><p className="mt-5 rounded-lg border border-primary/20 bg-primary/5 px-4 py-3 text-xs text-muted-foreground">Collection analyses and AI generation now run centrally via the Update button in the navigation.</p><p className="mt-4 text-xs text-foreground">AI suggestions require user review before saving or publishing. <span className="ai-badge">AI Generated</span></p></div></div></section>
    </div></div>
