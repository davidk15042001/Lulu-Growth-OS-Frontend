...
export function LuluPipeline() {
  ...
  return <div className="min-h-screen bg-[var(--background)] text-foreground">
    <aside className={`fixed inset-y-0 left-0 z-40 w-[220px] border-r border-border bg-[var(--sidebar)] px-3 py-5 transition-transform lg:translate-x-0 ${mobileNav ? 'translate-x-0' : '-translate-x-full'}`}><div className="px-3"><Logo /></div><nav className="mt-9 space-y-6">{navGroups.map(group => <div key={group.label}><p className="mb-2 px-3 text-[9px] font-bold uppercase tracking-[0.18em] text-muted-foreground">{group.label}</p><div className="space-y-0.5">{group.items.map(item => <button key={item.label} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[12px] font-medium transition ${item.active ? 'bg-secondary/15 text-foreground' : 'text-foreground hover:bg-secondary hover:text-foreground'}`}><item.icon size={15} />{item.label}{item.active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary text-primary-foreground" />}</button>)}</div></div>)}<div className="space-y-0.5"><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[12px] font-medium text-foreground"><Settings size={15} />Settings</button></div></nav><div className="absolute bottom-5 left-4 right-4 border-t border-border pt-4"><div className="flex items-center gap-2.5 px-2"><div className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-primary to-primary text-[11px] font-bold text-primary-foreground">SC</div><div><p className="text-[11px] font-semibold text-foreground">Sarah Chen</p><p className="text-[10px] text-muted-foreground">Admin</p></div><MoreHorizontal size={15} className="ml-auto text-muted-foreground" /></div></div></aside>
