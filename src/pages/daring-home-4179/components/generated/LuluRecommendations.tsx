...
export const LuluRecommendations = () => {
  const { items, loading, error } = useLiveRecords('ai_recommendations');
  return <main className="min-h-screen bg-[var(--background)] text-foreground selection:bg-secondary/30"><aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-border/[.07] bg-[var(--sidebar)] px-4 py-5 lg:block"><div className="mb-10 flex items-center gap-3 px-2"><div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-primary to-primary text-primary-foreground"><Bot size={20} /></div><span className="text-lg font-bold tracking-tight">lulu<span className="text-foreground">.ai</span></span></div><LuluSectionNavigation activeId="daring-home-4179" /></aside><section className="lg:ml-64"><header className="flex min-h-[72px] flex-wrap items-center justify-between gap-4 border-b border-border/[.07] px-5 py-4 md:px-8"><div className="flex items-center gap-2 text-xs text-muted-foreground"><span>Intelligence</span><ChevronRight size={13} /><strong className="font-medium text-foreground">AI Recommendations</strong></div><button className="rounded-lg border border-border px-3 py-2 text-xs text-foreground"><RefreshCw size={14} className="mr-2 inline" /> Refresh Recommendations</button></header><div className="mx-auto max-w-[1500px] px-5 py-7 md:px-8"><div className="mb-7"><Badge className="mb-3 border border-border/20 bg-secondary/10 text-foreground"><Sparkles size={12} className="mr-1.5" /> Intelligence workspace</Badge><h1 className="text-3xl font-bold tracking-[-.03em] text-foreground md:text-4xl">AI Recommendations</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Live recommendations based on connected workspace data and authorized AI capabilities.</p></div>{loading ? <div className="rounded-xl border border-border bg-card p-8 text-center text-sm text-muted-foreground">Loading live recommendations…</div> : error ? <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center text-sm text-destructive">{error}</div> : !items.length ? <div className="flex min-h-[560px] items-center justify-center rounded-xl border border-dashed border-border bg-card p-10 text-center"><div><Sparkles className="mx-auto text-muted-foreground" size={38} /><h2 className="mt-4 text-xl font-semibold text-foreground">No live recommendations available yet</h2><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Connect data sources and complete an authorized analysis to populate recommendations. No example impacts, scores, priorities or actions are displayed.</p></div></div> : <section className="overflow-x-auto rounded-xl border border-border bg-card"><table className="w-full min-w-[980px] text-left"><thead className="border-b border-border bg-secondary text-[10px] uppercase tracking-wider text-muted-foreground"><tr><th className="px-5 py-3">Recommendation</th><th className="px-3 py-3">Area</th><th className="px-3 py-3">Priority</th><th className="px-3 py-3">Impact</th><th className="px-3 py-3">Confidence</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Updated</th></tr></thead><tbody className="divide-y divide-border">{items.map(record => <tr key={record.id}><td className="px-5 py-3 text-xs font-semibold text-foreground">{record.name}</td><td className="px-3 py-3 text-xs text-muted-foreground">{String(record.data?.area || 'Connected workspace')}</td><td className="px-3 py-3 text-xs text-muted-foreground">{String(record.data?.priority || 'Recorded')}</td><td className="px-3 py-3 text-xs text-muted-foreground">{String(record.data?.impact || record.valueAmount || '—')}</td><td className="px-3 py-3 text-xs text-muted-foreground">{String(record.data?.confidence || '—')}</td><td className="px-3 py-3 text-xs text-muted-foreground">{record.status || 'Recorded'}</td><td className="px-3 py-3 text-xs text-muted-foreground">{record.updatedAt}</td></tr>)}</tbody></table></section>}</div></section></main>;
};
