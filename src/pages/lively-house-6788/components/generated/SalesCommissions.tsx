  ...
  return <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)] antialiased">
      <div className="flex min-h-screen">
        <aside className="hidden w-[240px] shrink-0 flex-col bg-[var(--sidebar)] text-foreground lg:flex">
          ...
          <div className="px-4 pt-7"><p className="px-3 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Workspace</p><nav className="mt-3 space-y-1" aria-label="Main navigation">{navItems.map(item => <button key={item} onClick={() => showNotice(`${item} selected`)} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition ${item === 'Commissions' ? 'bg-[var(--primary)] font-semibold text-primary-foreground shadow-[inset_3px_0_0_var(--primary-foreground)]' : 'text-foreground hover:bg-secondary hover:text-primary-foreground'}`}><span className={`h-1.5 w-1.5 rounded-full ${item === 'Commissions' ? 'bg-[var(--primary)]' : 'bg-muted'}`} />{item}</button>)}</nav></div>
