...
export const LuluKeywordsWorkspace = () => {
  ...
  return <div className="min-h-screen bg-[var(--background)] font-sans text-[var(--foreground)]">
    <aside className={`${mobile ? 'flex' : 'hidden'} fixed inset-y-0 left-0 z-40 w-[220px] flex-col bg-[var(--sidebar)] px-3 py-5 lg:flex`}><div className="mb-7 flex items-center gap-2 px-2"><span className="flex h-7 w-7 items-center justify-center rounded-md bg-[var(--primary)] font-bold text-primary-foreground">L</span><strong className="text-foreground">Lulu AI</strong></div><LuluSectionNavigation activeId="kind-time-4492" /><div className="flex items-center gap-2 border-t border-[var(--muted-foreground)] pt-4"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--muted)] text-xs text-foreground">DM</span><div><p className="text-xs text-foreground">David M</p><p className="text-[10px] text-[var(--muted-foreground)]">Growth operator</p></div><MoreHorizontal size={15} className="ml-auto text-[var(--muted-foreground)]" /></div></aside>
