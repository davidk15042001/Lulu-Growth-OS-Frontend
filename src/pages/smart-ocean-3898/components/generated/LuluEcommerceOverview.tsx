...
export function LuluEcommerceOverview() {
  ...
  return <div className="min-h-screen bg-[var(--background)] font-sans text-foreground">
    ...
    <main className="mx-auto max-w-[1480px] space-y-7 p-5 lg:p-8">{ecommerceError && <div role="alert" className="rounded-lg border border-chart-5/30 bg-chart-5/10 px-4 py-3 text-sm text-chart-5">Ecommerce overview data could not be loaded. Check connected stores, orders and customers, then try again.</div>}{!ecommerceLoading && !ecommerceError && orderRecords.length === 0 && customerRecords.length === 0 && storeRecords.length === 0 && <div className="rounded-lg border border-dashed border-border bg-card px-4 py-3 text-sm text-muted-foreground">No ecommerce records are available yet.</div>}<p className="-mt-3 max-w-2xl text-sm text-muted-foreground">Monitor your ecommerce business, stores, orders, customers, products and operations from one place.</p>
      ...
      <div className="grid gap-5 xl:grid-cols-2">{['Pending Orders', 'Unfulfilled Orders'].map((title, idx) => <Card key={title} className="p-5"><SectionTitle title={title} badge={idx ? '89' : '34'} /><div className="mb-3 flex gap-1 overflow-x-auto border-b border-border pb-2 text-xs font-semibold">{(idx ? ['All', '<24h', '24-48h', '>48h'] : ['Payment Pending (12)', 'Processing (18)', 'Fulfillment Pending (4)']).map(tab => <button key={tab} className="whitespace-nowrap rounded px-3 py-1.5 text-foreground hover:bg-secondary hover:text-foreground">{tab}</button>)}</div><div className="space-y-2">{(idx ? [['#10831', 'Olivia Martin', '3', '52h', 'Critical'], ['#10826', 'Noah Williams', '2', '31h', 'Attention'], ['#10819', 'Liam Davis', '1', '18h', 'Queued']] : [['#10844', 'Workspace contact', '$67.00', 'Payment pending', '22m'], ['#10838', 'Ava Smith', '$118.00', 'Address review', '1h'], ['#10831', 'Olivia Martin', '$89.00', 'Payment pending', '2h']]).map(row => <div key={row[0]} className={`grid grid-cols-[auto_1fr_auto] items-center gap-3 border-l-2 ${idx && row[3] === '52h' ? 'border-chart-5' : 'border-chart-1'} bg-card px-3 py-2.5 text-xs`}><strong className="text-foreground">{row[0]}</strong><span><b className="block text-foreground">{row[1]}</b><small className="text-muted-foreground">{idx ? `${row[2]} items · ${row[3]}` : row[3]}</small></span><strong className="text-foreground">{idx ? row[4] : row[2]}</strong></div>)}</div><button className="mt-4 rounded-md border border-border px-3 py-2 text-xs font-bold">View Orders</button></Card>)}</div>
      ...
      <div className="grid gap-5 xl:grid-cols-2"><Card className="p-5"><SectionTitle title="Cart Activity" /><div className="grid grid-cols-2 gap-2"><MiniMetric label="Active Carts" value="284" /><MiniMetric label="Abandoned Carts" value="891" tone="amber" /><MiniMetric label="Active Cart Value" value="$21,847" /><MiniMetric label="Abandoned Value" value="$67,293" tone="amber" /></div><div className="mt-4 space-y-2 text-xs text-muted-foreground"><p>🛒 <strong>Connected customer added Wireless Earbuds</strong> <span className="text-muted-foreground">2m ago</span></p><p>⚠ <strong>Cart abandoned — $234.00</strong> <span className="text-muted-foreground">42m ago</span></p><p>↗ <strong>Checkout started by Workspace contact</strong> <span className="text-muted-foreground">1h ago</span></p></div><div className="mt-4 flex gap-2"><button className="rounded-md border border-border px-3 py-2 text-xs font-bold">View Carts</button><button className="rounded-md border border-border px-3 py-2 text-xs font-bold">View Abandoned Carts</button></div></Card><Card className="p-5"><SectionTitle title="Abandoned Carts" /><p className="text-3xl font-bold tracking-tight text-foreground">891 <span className="text-sm font-medium text-muted-foreground">abandoned</span></p><p className="mt-1 text-sm text-muted-foreground">$67,293 in cart value · <strong className="text-foreground">75.8%</strong> abandonment rate</p><div className="mt-4 space-y-2 text-xs">{[['$234.00', '3 items', '42 min ago', 'Connected store'], ['$189.50', '2 items', '1h ago', 'Brand Store'], ['$96.00', '1 item', '2h ago', 'Connected store']].map(row => <div key={row[0]} className="flex justify-between rounded bg-card px-3 py-2"><b>{row[0]}</b><span>{row[1]}</span><span className="text-muted-foreground">{row[2]}</span><span className="text-foreground">{row[3]}</span></div>)}</div><p className="mt-4 rounded-md bg-secondary px-3 py-2 text-xs font-semibold text-foreground">✨ AI Insight · Abandonment rate is 8% above baseline this week</p><button className="mt-4 rounded-md border border-border px-3 py-2 text-xs font-bold">View Abandoned Carts</button></Card></div>
