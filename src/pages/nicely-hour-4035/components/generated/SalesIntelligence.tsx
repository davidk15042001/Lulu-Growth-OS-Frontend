...
<div className="mb-5 grid gap-5 xl:grid-cols-2"><Section title="Pipeline Value" eyebrow="Open Pipeline"><div className="grid grid-cols-2 gap-4"><div><Badge>Observed</Badge><p className="mt-3 text-2xl font-semibold text-foreground">€6,240,000</p><p className="text-xs text-muted-foreground">Open Pipeline</p></div><div><Badge>Calculated</Badge><p className="mt-3 text-2xl font-semibold text-foreground">€2,620,800</p><p className="text-xs text-muted-foreground">Weighted Pipeline</p></div></div><div className="mt-6 space-y-3">{stages.slice(0, 5).map(stage => <div key={stage.name} className="flex items-center gap-3 text-xs"><span className="w-20 text-muted-foreground">{stage.name}</span><div className="h-2 flex-1 rounded-full bg-secondary"><div className="h-full rounded-full bg-primary text-primary-foreground" style={{
                      width: stage.width
                    }} /></div><span className="w-24 text-right text-foreground">{stage.value}</span></div>)}</div></Section><Section title="Sales Funnel" eyebrow="Conversion from previous stage"><div className="space-y-2">{[['Leads', '1,248', '100%', '€31,200,000'], ['Qualified Leads', '624', '50.0%', '€15,600,000'], ['Opportunities', '312', '50.0%', '€7,800,000'], ['Proposals', '186', '59.6%', '€4,650,000'], ['Negotiations', '96', '51.6%', '€2,496,000'], ['Won Deals', '156', '31.9%', '€3,840,000']].map((item, index) => <div key={item[0]} className="flex items-center gap-3"><div className="grid h-8 place-items-center rounded-r-lg text-[11px] font-semibold text-foreground" style={{
                    width: `${100 - index * 13}%`,
                    background: `rgba(124,58,237,${0.86 - index * .08})`
                  }}>{item[0]}</div><span className="w-10 text-right text-xs text-foreground">{item[1]}</span><span className="w-12 text-right text-[10px] text-foreground">{item[2]}</span><span className="ml-auto text-[11px] text-muted-foreground">{item[3]}</span></div>)}</div></Section><Section title="Conversion Performance" eyebrow="12-month trend"><DataTable headers={['Stage', 'Current', 'Previous', 'Change']} rows={['Lead → Qualified|50.0%|44.8%|+5.2pp ↑', 'Qualified → Opportunity|50.0%|48.2%|+1.8pp ↑', 'Opportunity → Proposal|59.6%|54.1%|+5.5pp ↑', 'Proposal → Won|45.2%|40.6%|+4.6pp ↑', 'Overall Lead → Customer|12.5%|10.8%|+1.7pp ↑'].map(row => row.split('|'))} /></Section><Section title="Win Rate" eyebrow="Calculated"><div className="mb-5 flex items-end justify-between"><div><p className="text-3xl font-semibold text-foreground">38.4%</p><p className="text-xs text-muted-foreground">Prev. 34.2% · <span className="text-chart-4">+4.2pp ↑</span></p></div><Badge>Calculated</Badge></div><div className="grid grid-cols-2 gap-4 text-xs">{[['By Product', 'Enterprise 48.2% · Pro 42.6% · Business 34.8%'], ['By Segment', 'Enterprise 52.4% · Mid-Market 38.6% · SMB 28.4%'], ['By Channel', 'Direct 44.2% · Partner 38.6% · Referral 52.8%'], ['By Market', 'EMEA 40.2% · Americas 36.8% · APAC 34.4%']].map(item => <div key={item[0]}><p className="text-muted-foreground">{item[0]}</p><p className="mt-1 leading-5 text-foreground">{item[1]}</p></div>)}</div><div className="mt-5 flex h-12 items-end gap-1">{[44, 52, 48, 58, 55, 63, 68, 62, 74, 70, 82, 88].map((h, i) => <div key={`win-${i}`} className="flex-1 rounded-t bg-secondary/70" style={{
                  height: `${h}%`
                }} />)}</div></Section></div>
<div className="mb-5 grid gap-5 xl:grid-cols-3"><Section title="Loss Rate" eyebrow="Improving"><div className="flex items-end justify-between"><p className="text-3xl font-semibold text-foreground">35.0%</p><span className="text-xs text-foreground">−5.2pp ↓</span></div><div className="mt-5 space-y-3 text-xs">{[['At Qualified', '22.4%'], ['At Opportunity', '18.6%'], ['At Proposal', '28.4%'], ['At Negotiation', '31.2%']].map(item => <div key={item[0]} className="flex justify-between"><span className="text-muted-foreground">{item[0]}</span><strong className="text-foreground">{item[1]}</strong></div>)}</div></Section><Section title="Lost Deal Analysis" eyebrow="Connected CRM"><div className="space-y-3">{lossReasons.map(item => <div key={item[0]} className="grid grid-cols-[80px_1fr_75px] items-center gap-3 text-xs"><span className="text-muted-foreground">{item[0]}</span><div className="h-2 rounded-full bg-secondary"><div className="h-full rounded-full bg-chart-5/80" style={{
                      width: `${item[4]}%`
                    }} /></div><span className="text-right text-foreground">{item[1]}</span></div>)}</div><p className="mt-5 text-[11px] text-muted-foreground">Total Lost: 84 deals · €2,184,000. Loss reasons reflect data recorded in the connected CRM.</p></Section><Section title="Average Deal Value" eyebrow="Calculated"><Badge>Calculated</Badge><p className="mt-3 text-3xl font-semibold text-foreground">€24,800</p><p className="text-xs text-muted-foreground">Prev. €22,400 · <span className="text-chart-4">+10.7% ↑</span></p><div className="mt-5 space-y-3 text-xs">{[['Enterprise', '€64,200'], ['Pro', '€28,400'], ['Business', '€14,800'], ['Median Deal Value', '€18,400']].map(item => <div key={item[0]} className="flex justify-between border-b border-border/[.05] pb-2"><span className="text-muted-foreground">{item[0]}</span><strong className="text-foreground">{item[1]}</strong></div>)}</div><Sparkline color="var(--chart-3)" /></Section><Section title="Sales Cycle" eyebrow="Lead-to-Close duration"><div className="flex items-end justify-between"><div><p className="text-3xl font-semibold text-foreground">48 days</p><p className="text-xs text-muted-foreground">Median 38 days · <span className="text-foreground">−8 days ↓ improving</span></p></div><Badge>Calculated</Badge></div><div className="mt-5 grid grid-cols-2 gap-3 text-xs">{[['Enterprise', '84 days'], ['Pro', '42 days'], ['Business', '24 days'], ['SMB', '22 days']].map(item => <div key={item[0]} className="rounded-lg bg-secondary p-3"><span className="text-muted-foreground">{item[0]}</span><strong className="mt-1 block text-foreground">{item[1]}</strong></div>)}</div></Section><Section title="Sales Velocity" eyebrow="Calculated composite metric"><Badge>Calculated</Badge><p className="mt-3 text-sm text-muted-foreground">Sales Velocity = Opportunities × Avg Deal Value × Win Rate ÷ Sales Cycle</p><p className="mt-4 text-2xl font-semibold text-foreground">€49,280/day</p><p className="text-xs text-chart-4">Prev. €38,640 · +27.5% ↑</p><Sparkline color="var(--chart-1)" /><p className="text-[11px] text-muted-foreground">A calculated composite metric.</p></Section></div>
<Section title="Sales by Representative" eyebrow="Permission required" className="mb-5"><div className="mb-4 flex items-center justify-between"><p className="text-xs text-muted-foreground">Top performers · access to rep-level data is permission gated.</p><button className="rounded-lg border border-border px-3 py-2 text-xs text-foreground"><Filter size={13} className="mr-1 inline" /> Sort: Revenue</button></div><DataTable headers={['Representative', 'Revenue', 'Won Deals', 'Win Rate', 'Avg Deal Value', 'Sales Cycle', 'Pipeline', 'Target Attainment']} rows={reps} /></Section><div className="mb-5 grid gap-5 xl:grid-cols-2"><Section title="Sales by Team" eyebrow="Team performance"><DataTable headers={['Team', 'Revenue', 'Pipeline', 'Win Rate', 'Deals', 'Avg Deal', 'Cycle', 'Target']} rows={teams} /></Section><Section title="Sales by Product" eyebrow="Product performance"><DataTable headers={['Product', 'Revenue', 'Deals', 'Pipeline', 'Win Rate', 'Avg Deal', 'Growth']} rows={products} /></Section></div><div className="mb-5 grid gap-5 xl:grid-cols-2"><Section title="Sales by Geography" eyebrow="Regional performance"><div className="mb-4 flex gap-1 rounded-lg bg-secondary p-1 w-fit">{['Map', 'Ranking', 'Comparison'].map(item => <button key={item} onClick={() => setGeoMode(item)} className={`rounded-md px-3 py-1.5 text-xs ${geoMode === item ? 'bg-primary text-primary-foreground' : 'text-primary-foreground'}`}>{item}</button>)}</div><div className="grid gap-4 md:grid-cols-[.9fr_1.3fr]"><div className="grid min-h-40 place-items-center rounded-xl border border-border/[.06] bg-[var(--secondary)]"><div className="text-center"><div className="text-5xl opacity-80">🌐</div><p className="mt-2 text-[11px] text-muted-foreground">Revenue intensity map</p></div></div><div className="space-y-3 text-xs">{[['United Kingdom', '€1,344,000', '35%', '42.4%'], ['Germany', '€768,000', '20%', '38.6%'], ['United States', '€576,000', '15%', '36.2%'], ['France', '€384,000', '10%', '34.8%'], ['Netherlands', '€307,200', '8%', '40.2%']].map((item, index) => <div key={item[0]} className="flex items-center gap-2"><span className="w-4 text-muted-foreground">0{index + 1}</span><span className="flex-1 text-foreground">{item[0]}</span><span className="text-foreground">{item[1]}</span><span className="text-foreground">{item[3]}</span></div>)}</div></div></Section><Section title="New vs Existing Business" eyebrow="Revenue composition"><div className="mb-5 flex h-11 overflow-hidden rounded-xl"><div className="grid place-items-center bg-primary text-xs font-semibold text-primary-foreground" style={{
                  width: '50%'
                }}>50%</div><div className="grid place-items-center bg-secondary/80 text-xs font-semibold text-[var(--foreground)]" style={{
                  width: '40%'
                }}>40%</div><div className="grid place-items-center bg-primary text-xs font-semibold text-[var(--primary-foreground)]" style={{
                  width: '10%'
                }}>10%</div></div><div className="grid grid-cols-3 gap-3 text-xs">{[['New Business Revenue', '€1,920,000', '+22.6%'], ['Existing Customer Revenue', '€1,536,000', '+8.4%'], ['Expansion Revenue', '€384,000', '+18.2%']].map(item => <div key={item[0]}><p className="text-[11px] text-muted-foreground">{item[0]}</p><p className="mt-1 font-semibold text-foreground">{item[1]}</p><p className="mt-1 text-chart-4">{item[2]}</p></div>)}</div><p className="mt-5 text-xs text-muted-foreground">New customer count: 92 · Existing customer growth: +8.4%</p></Section></div>
<div className="mb-5 grid gap-5 xl:grid-cols-3"><Section title="Sales Forecast" eyebrow="Forecast"><Badge>Forecast</Badge><p className="mt-3 text-2xl font-semibold text-foreground">€5,280,000</p><div className="mt-4 space-y-2 text-xs">{[['Closed Revenue (YTD)', '€3,840,000'], ['Remaining Pipeline', '€6,240,000'], ['Forecast Range', '€4.8M — €5.76M']].map(item => <div key={item[0]} className="flex justify-between"><span className="text-muted-foreground">{item[0]}</span><strong className="text-foreground">{item[1]}</strong></div>)}</div><button className="mt-5 text-xs font-semibold text-foreground">View Forecast <ChevronRight size={14} className="inline" /></button></Section><Section title="Sales Targets" eyebrow="Organization-defined"><div className="space-y-3 text-xs">{[['Revenue Target', '€3.84M / €4M', '96.0%', 'On Track'], ['Deal Target', '156 / 180', '86.7%', 'Behind'], ['New Customer Target', '92 / 100', '92.0%', 'On Track'], ['Pipeline Target', '€6.24M / €7.5M', '83.2%', 'Behind']].map(item => <div key={item[0]} className="flex items-center gap-3"><span className="flex-1 text-muted-foreground">{item[0]}</span><span className="text-foreground">{item[1]}</span><span className="text-foreground">{item[2]}</span><span className={item[3] === 'Behind' ? 'text-foreground' : 'text-foreground'}>{item[3]}</span></div>)}</div><p className="mt-5 text-[11px] text-muted-foreground">Targets are organization-defined. No targets are created automatically.</p></Section><Section title="Sales Gap" eyebrow="Annual Revenue Target"><div className="flex items-center gap-5"><div className="grid h-28 w-28 place-items-center rounded-full" style={{
                  background: 'conic-gradient(var(--primary) 0 80%, var(--secondary) 80% 100%)'
                }}><div className="grid h-20 w-20 place-items-center rounded-full bg-[var(--secondary)]"><strong className="text-xl text-foreground">80%</strong></div></div><div className="space-y-2 text-xs"><p className="text-muted-foreground">Target <strong className="ml-2 text-foreground">€4,800,000</strong></p><p className="text-muted-foreground">Actual <strong className="ml-2 text-foreground">€3,840,000</strong></p><p className="text-foreground">Gap −€960,000</p></div></div><p className="mt-4 text-[11px] text-muted-foreground">Remaining pipeline covers the gap 6.5× if win rate holds.</p></Section></div>
<div className="mb-5 grid gap-5 xl:grid-cols-2"><Section title="Sales Benchmark" eyebrow="Actual vs reference"><DataTable headers={['Metric', 'Actual', 'Benchmark', 'Target', 'Signal']} rows={['Win Rate|38.4%|35%|45%|Above benchmark ✓', 'Avg Deal Value|€24,800|€18,400|€30,000|Room to grow', 'Sales Cycle|48 days|62 days|40 days|Below benchmark ✓', 'Pipeline Coverage|1.6×|2.5×|3.0×|Gap', 'Conversion Rate|12.5%|10.2%|15%|Above benchmark ✓'].map(row => row.split('|'))} /><div className="mt-4 flex gap-2"><Badge>Actual</Badge><Badge>Target</Badge><Badge>Benchmark</Badge></div></Section><Section title="Sales Comparison" eyebrow="Context"><DataTable headers={['Metric', 'Current', 'Prev Period', 'Prev Year', 'Target', 'Benchmark']} rows={['Sales Revenue|€3.84M|€3.28M|€2.76M|€4.8M|—', 'Pipeline Value|€6.24M|€5.12M|€4.08M|€7.5M|—', 'Win Rate|38.4%|34.2%|29.8%|45%|35%', 'Avg Deal Value|€24,800|€22,400|€19,200|€30,000|€18,400', 'Sales Cycle|48 days|56 days|72 days|40 days|62 days', 'Conversion Rate|12.5%|10.8%|8.4%|15%|10.2%'].map(row => row.split('|'))} /></Section></div><Section title="Key Sales KPIs" eyebrow="Executive scorecard" className="mb-5"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[['Sales Revenue', '€3.84M', '+17.1%'], ['Sales Growth', '+17.1%', 'Accelerating'], ['Pipeline Value', '€6.24M', '+21.9%'], ['Pipeline Growth', '+21.9%', 'Accelerating'], ['Win Rate', '38.4%', '+4.2pp'], ['Conversion Rate', '12.5%', '+1.7pp'], ['Average Deal Value', '€24,800', '+10.7%'], ['Sales Cycle', '48 days', '−8 days'], ['Sales Velocity', '€49,280/day', '+27.5%'], ['Pipeline Coverage', '1.6×', '−1.4×'], ['Closed Deals', '156', '+12.4%'], ['Forecasted Revenue', '€5.28M', 'Forecast']].map(item => <article key={item[0]} className="rounded-xl border border-border/[.06] bg-secondary p-4"><p className="text-[11px] text-muted-foreground">{item[0]}</p><p className="mt-2 text-lg font-semibold text-foreground">{item[1]}</p><p className="mt-1 text-xs text-chart-4">{item[2]}</p><Sparkline /></article>)}</div><button className="mt-5 rounded-lg bg-secondary px-3 py-2 text-xs font-semibold text-foreground">Open KPI Explorer <ChevronRight size={14} className="inline" /></button></Section>
<Section title="Explain Sales Performance" eyebrow="AI Explanation" className="mb-5"><div className="flex gap-4"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-primary to-primary text-[var(--primary-foreground)]"><Bot size={20} /></div><div><p className="text-sm leading-6 text-foreground">Sales revenue grew <strong className="text-foreground">17.1% year-over-year</strong> to €3,840,000, accelerating from 12.4% in the prior period. Win rate improved to 38.4%, driven by stronger Enterprise conversion (52.4%) and a 10-day reduction in average sales cycle. Pipeline grew 21.9% to €6,240,000, with the most growth in the Enterprise and Healthcare segments. New business contributed 50% of revenue, while Organic Search showed the highest lead-to-customer conversion improvement at +5.2pp.</p><button className="mt-4 rounded-lg bg-secondary/15 px-3 py-2 text-xs font-semibold text-foreground">View AI Recommendations <ChevronRight size={14} className="inline" /></button></div></div></Section><Section title="Ask Lulu AI" eyebrow="Explore your sales performance" className="mb-5"><form onSubmit={submit} className="flex gap-2"><div className="flex flex-1 items-center gap-3 rounded-xl border border-border bg-[var(--secondary)] px-4"><MessageSquare size={16} className="text-muted-foreground" /><input value={ask} onChange={e => setAsk(e.target.value)} placeholder="Ask Lulu AI about your sales performance..." className="w-full bg-transparent py-3 text-sm text-foreground outline-none placeholder:text-muted-foreground" aria-label="Ask Lulu AI about sales" /></div><button className="rounded-xl bg-primary px-4 text-xs font-semibold text-primary-foreground" type="submit">Ask</button></form><div className="mt-4 flex flex-wrap gap-2">{prompts.map(prompt => <button key={prompt} onClick={() => setAsk(prompt)} className="rounded-full border border-border/[.08] px-3 py-1.5 text-[11px] text-foreground hover:border-border/40 hover:text-foreground">{prompt}</button>)}</div></Section><details className="rounded-2xl border border-border/[.08] bg-[var(--card)] p-5"><summary className="cursor-pointer list-none text-[17px] font-semibold text-foreground">Data Sources <span className="ml-2 text-xs font-normal text-muted-foreground">Connected business systems</span></summary><div className="mt-5 grid gap-2 sm:grid-cols-5">{[['CRM', 'Connected'], ['Billing', 'Connected'], ['Marketing Analytics', 'Connected'], ['Finance', 'Partial'], ['Ecommerce', 'Not Connected']].map(item => <div key={item[0]} className="flex items-center justify-between rounded-lg bg-secondary px-3 py-3"><span className="text-xs text-foreground">{item[0]}</span><span className={item[1] === 'Connected' ? 'text-chart-4' : item[1] === 'Partial' ? 'text-foreground' : 'text-muted-foreground'}>{item[1]}</span></div>)}</div></details><div className="mt-6 flex flex-wrap justify-between gap-3 border-t border-border/[.07] pt-5 text-[11px] text-muted-foreground"><span>Sales Intelligence · Lulu AI Business OS</span><span>Observed · Calculated · Estimated · Target · Benchmark · Forecast</span></div></div></div></div></main>;
}