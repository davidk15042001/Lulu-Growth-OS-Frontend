...
export function LuluEcommerceOverview() {
  ...
  const getEcommerceField = (record: typeof orderRecords[number], key: string) => String((record as unknown as Record<string, unknown>)[key] ?? record.data?.[key] ?? '');
  ...
  return <div className="min-h-screen bg-[var(--background)] font-sans text-foreground">
    ...
    <div><header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-border bg-secondary px-5 backdrop-blur lg:px-8"><div className="flex items-center gap-3"><button className="rounded-md p-2 hover:bg-sidebar lg:hidden" onClick={() => setMobileNav(true)} aria-label="Open navigation"><Menu size={20} /></button><div><p className="text-xs font-medium text-muted-foreground">Website &amp; Commerce <span className="px-1">/</span> <span className="text-foreground">Overview</span></p><h1 className="mt-1 text-xl font-bold tracking-tight text-foreground">Commerce Overview</h1></div></div><div className="flex items-center gap-2"><button className="hidden rounded-md bg-primary px-3 py-2 text-xs font-bold text-primary-foreground shadow-sm hover:bg-primary sm:block">Create Report</button><button className="rounded-md border border-border p-2 text-foreground" aria-label="More actions"><MoreHorizontal size={17} /></button></div></header>
