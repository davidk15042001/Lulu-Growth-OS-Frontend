...
function Sidebar({
  ...
  return <aside className={`${open ? 'flex' : 'hidden'} lg:flex fixed z-20 inset-y-0 left-0 w-60 flex-col border-r border-border bg-[var(--sidebar)] p-4`}>
    ...
    <nav className="space-y-1" aria-label="CRM navigation">{navigation.map(({
        label,
        icon: NavIcon
      }) => <button key={label} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${label === 'CRM' ? 'bg-secondary/15 text-foreground' : 'text-foreground hover:border-border hover:bg-secondary hover:text-foreground'}`}><NavIcon size={17} /><span>{label}</span>{label === 'CRM' && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary text-primary-foreground" />}</button>)}</nav>
