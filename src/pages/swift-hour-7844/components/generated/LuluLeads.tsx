...
export function LuluLeads() {
 ...
 {selected.length > 0 && <div className="sticky bottom-4 z-10 flex flex-wrap items-center gap-2 rounded-xl border border-border/30 bg-[var(--secondary)] p-3 shadow-2xl"><strong className="mr-2 text-sm text-foreground">{selected.length} leads selected</strong><button onClick={() => setSelected([])} aria-label="Deselect leads" className="mr-2 text-foreground"><X size={16} /></button>{['Assign Owner', 'Change Status', 'Add Tag', 'Create Task',  'Archive', 'Delete'].map(x => <button key={x} className="rounded-lg border border-border px-2.5 py-1.5 text-xs text-foreground hover:bg-secondary">{x}</button>)}<button className="ml-auto rounded-lg border border-border/30 px-3 py-1.5 text-xs text-foreground">Bulk Qualify</button></div>}
