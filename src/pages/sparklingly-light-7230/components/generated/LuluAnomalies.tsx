...
const summary = [['Active Anomalies', '12', 'slate'], ['Critical', '2', 'red'], ['High Priority', '4', 'orange'], ['Medium Priority', '3', 'amber'], ['Low Priority', '3', 'blue'], ['New', '5', 'violet'], ['Investigating', '4', 'blue'], ['Resolved', '23 today', 'green']] as const;
const anomalies: Row[] = [{
  kpi: 'Revenue',
  area: 'Business',
  description: 'Significant revenue drop below expected range',
  detected: 'Today 10:32',
  magnitude: '-28.3%',
  severity: 'orange',
  confidence: '94%',
  status: 'Action Required'
}, {
  kpi: 'Advertising Spend',
  area: 'Advertising',
  description: 'Abnormal spend spike without conversion uplift',
  ...
  kpi: 'Customer Churn',
  area: 'Customer',
  description: 'Churn rate rose sharply above normal pattern',
  detected: 'Yesterday',
  magnitude: '+34.1%',
  severity: 'orange',
  confidence: '87%',
  ...
  kpi: 'ROAS',
  area: 'Advertising',
  description: 'ROAS collapsed below historical lower bound',
  detected: 'Today 09:45',
  magnitude: '-41.8%',
  severity: 'red',
  confidence: '96%',
  status: 'Action Required'
  ...
  area: 'Marketing',
  description: 'Mobile conversion dropped sharply',
  detected: 'Today 11:00',
  magnitude: '-31.2%',
  severity: 'orange',
  confidence: '89%',
  status: 'New'
}, {
  kpi: 'Product Demand — SKU-4421',
  area: 'Ecommerce',
  description: 'Unexpected 46% demand spike above normal',
  detected: 'Yesterday',
  magnitude: '+46.3%',
  severity: 'amber',
  confidence: '82%',
  ...
  kpi: 'Organic Traffic',
  area: 'Marketing',
  description: 'Unusually strong traffic increase detected',
  detected: '2 days ago',
  magnitude: '+38.7%',
  severity: 'blue',
  confidence: '78%',
  ...
  area: 'Ecommerce',
  description: 'Abandonment rate jumped above expected range',
  detected: 'Today 07:30',
  magnitude: '+22.8%',
  severity: 'amber',
  confidence: '85%',
  status: 'New'
}];
const causes = [['Mobile conversion decline', 'Mobile conversion -31.2% detected simultaneously', 'High', 'Correlated'], ['Advertising campaign change', 'No active campaign change recorded', 'Low', 'AI Inferred'], ['Product availability issue', 'No inventory alerts detected', 'Low', 'AI Inferred'], ['Data synchronization issue', 'All sources synced within 15 min', 'Very Low', 'Checked'], ['External traffic event', 'Traffic volume stable', 'Low', 'AI Inferred']];
const related = [['Revenue', '↓ -28.3%', 'Observed'], ['Mobile Conversion', '↓ -31.2%', 'Correlated'], ['Desktop Conversion', '→ -1.1%', 'Observed'], ['Traffic', '→ Stable (+0.8%)', 'Observed'], ['Average Order Value', '↑ +3.2%', 'Observed'], ['Advertising Spend', '↑ +47.2%', 'Correlated']];
const sources = [['Shopify', '12 min ago', 'Excellent', 'green'], ['Google Analytics', '45 min ago', 'Good', 'green'], ['Google Ads', '6h ago', 'Limited', 'amber'], ['Meta Ads', '1h ago', 'Good', 'green'], ['CRM', '20 min ago', 'Excellent', 'green']];
const history = [['Revenue anomaly detected', 'System', 'Today 10:32'], ['Anomaly acknowledged', 'James K.', 'Today 10:48'], ['Investigation started', 'Sarah M.', 'Today 11:03'], ['Task created: Investigate mobile conversion', 'James K.', 'Today 11:15'], ['Comment added: Checked Google Ads — no changes', 'Sarah M.', 'Today 11:22'], ['Report created', 'James K.', 'Today 11:30']];
...
export const LuluAnomalies = () => {
  ...
  return <div className="min-h-screen bg-[var(--background)] font-sans text-foreground">
    <aside className={`${mobileNav ? 'flex' : 'hidden'} fixed inset-y-0 left-0 z-30 w-[68px] flex-col items-center border-r border-border bg-[var(--sidebar)] py-5 lg:flex`}><div className="mb-8 flex h-9 w-9 items-center justify-center rounded-xl bg-primary text-lg font-bold text-primary-foreground">L</div><nav className="flex flex-1 flex-col items-center gap-3" aria-label="Collapsed primary navigation"><button className="rounded-lg p-3 text-foreground hover:bg-secondary" aria-label="Workspace"><LayoutGrid size={19} /></button><button className="rounded-lg bg-secondary/20 p-3 text-foreground" aria-label="Intelligence"><Sparkles size={19} /></button><button className="rounded-lg p-3 text-foreground" aria-label="Reports"><FileText size={19} /></button><button className="rounded-lg p-3 text-foreground" aria-label="Users"><Users size={19} /></button><button className="rounded-lg p-3 text-foreground" aria-label="Settings"><SlidersHorizontal size={19} /></button></nav><button className="rounded-lg p-3 text-foreground" aria-label="Help"><CircleHelp size={19} /></button><div className="mt-4 flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-[10px] font-semibold text-foreground">AM</div></aside>
    <main className="lg:ml-[68px]"><div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-8 lg:px-10">
      <header className="mb-6 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><button onClick={() => setMobileNav(true)} className="mb-3 lg:hidden" aria-label="Open navigation"><Menu size={22} /></button><p className="mb-3 text-xs text-muted-foreground">Intelligence <span className="mx-2">/</span> Analytics <span className="mx-2">/</span> <span className="font-medium text-foreground">Anomalies</span></p><h1 className="text-3xl font-bold tracking-[-.045em] text-foreground sm:text-[38px]">Anomalies</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Automatically detect unusual business behavior, understand what changed, and identify what requires attention.</p></div><div className="flex flex-wrap gap-2"><button className="flex h-9 items-center gap-2 rounded-lg bg-primary px-3 text-xs font-semibold text-primary-foreground shadow-sm hover:bg-primary"><Sparkles size={14} />Ask Lulu AI</button><button className="flex h-9 items-center gap-2 rounded-lg border border-border bg-secondary px-3 text-xs font-medium text-foreground">Explore Anomalies</button><button className="flex h-9 items-center gap-2 rounded-lg border border-border bg-card px-3 text-xs text-foreground"><Download size={14} />Export</button><button onClick={refresh} className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-card text-foreground" aria-label="Refresh anomalies"><RefreshCw size={15} className={refreshing ? 'animate-spin' : ''} /></button></div></header>
      <section className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-8">{summary.map(([label, value, tone]) => <article key={label} className="rounded-xl border border-border bg-card px-4 py-4 shadow-[0_3px_12px_rgba(0,0,0,.035)]"><p className="text-[11px] leading-4 text-muted-foreground">{label}</p><div className="mt-2 flex items-end justify-between"><strong className="text-xl font-semibold tracking-[-.04em] text-foreground">{value}</strong><Badge tone={tone}>{label === 'Active Anomalies' ? 'Live' : label === 'Resolved' ? 'Today' : 'Now'}</Badge></div></article>)}</section>
      ...
      <Section title="Revenue Anomaly" subtitle="Revenue · Detected Today, 10:32" tone="orange" action={<div className="flex flex-wrap gap-2"><Badge tone="orange" icon={<AlertTriangle size={12} />}>High</Badge><select value={status} onChange={e => setStatus(e.target.value)} className="rounded-full border border-chart-5/30 bg-chart-5/10 px-2 py-1 text-[11px] font-medium text-chart-5"><option>Action Required</option><option>Acknowledged</option><option>Investigating</option><option>Resolved</option><option>Dismissed</option></select></div>}><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6"><div><p className="text-[11px] text-muted-foreground">Observed Value <span className="text-foreground">· Observed</span></p><strong className="mt-1 block text-2xl text-foreground">€42,000</strong></div><div><p className="text-[11px] text-muted-foreground">Expected Range <span className="text-foreground">· Model-derived</span></p><strong className="mt-1 block text-sm text-foreground">€55,000 – €62,000</strong><p className="text-[10px] text-muted-foreground">Model-derived expected range</p></div><div><p className="text-[11px] text-muted-foreground">Deviation</p><strong className="mt-1 block text-2xl text-chart-5">-28.3%</strong></div><div><p className="text-[11px] text-muted-foreground">Duration</p><strong className="mt-1 block text-sm text-foreground">Ongoing · 3h 28min</strong></div><div><p className="text-[11px] text-muted-foreground">Confidence</p><strong className="mt-1 block text-2xl text-foreground">94%</strong><p className="text-[10px] text-muted-foreground">Detection confidence</p></div><div><p className="text-[11px] text-muted-foreground">Anomaly Type</p><strong className="mt-1 block text-sm text-foreground">Point Anomaly</strong><p className="text-[10px] text-muted-foreground">Specific point in time</p></div></div><div className="mt-5 flex flex-wrap gap-2 border-t border-border pt-4"><button className="rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground">Acknowledge</button><button className="rounded-lg border border-border px-3 py-2 text-xs">Investigate</button><button className="rounded-lg border border-border px-3 py-2 text-xs">Resolve</button><button className="rounded-lg border border-border px-3 py-2 text-xs">Dismiss</button><button onClick={() => setTaskOpen(true)} className="rounded-lg border border-border px-3 py-2 text-xs text-foreground">Create Task</button><button className="rounded-lg border border-border p-2" aria-label="Share anomaly"><Share2 size={14} /></button><button className="rounded-lg border border-border p-2" aria-label="Export anomaly"><Download size={14} /></button><button className="rounded-lg border border-border px-3 py-2 text-xs">Create Report</button></div></Section>
      ...
      <div className="grid gap-5 md:grid-cols-3"><Section title="Magnitude" action={<span className="text-[11px] text-muted-foreground">Calculated</span>}><strong className="text-2xl text-chart-5">-€16,500</strong><p className="mt-2 text-xs text-muted-foreground">Relative: -28.3% · Peak deviation: -31.4%</p></Section><Section title="Duration"><strong className="text-xl text-foreground">Ongoing</strong><p className="mt-2 text-xs text-muted-foreground">Started 3h 28min ago · First detected Today 10:32</p></Section><Section title="Detection Confidence" action={<Badge tone="violet" icon={<Sparkles size={12} />}>Calculated</Badge>}><strong className="text-2xl text-foreground">94%</strong><p className="mt-2 text-xs text-muted-foreground">How strongly observed behavior differs from modeled normal.</p></Section></div>
      ...
      <div className="grid gap-5 xl:grid-cols-2"><Section title="Affected Segments" subtitle="Only segments with sufficient data shown." action={<div className="flex gap-1 overflow-x-auto">{['Device', 'Country', 'Channel', 'Product', 'Customer Segment'].map((tab, index) => <button key={tab} className={`rounded-md px-2 py-1 text-[11px] ${index === 0 ? 'bg-secondary text-foreground' : 'text-foreground'}`}>{tab}</button>)}</div>}><div className="space-y-4">{[['Mobile', '-34.2%', 'bg-destructive', 88], ['Tablet', '-12.1%', 'bg-primary', 42], ['Desktop', '-9.1%', 'bg-muted', 30]].map(item => <div key={item[0]} className="flex items-center gap-3 text-xs"><span className="w-20 text-muted-foreground">{item[0]}</span><div className="h-2 flex-1 rounded-full bg-secondary"><div className={`h-2 rounded-full ${item[2]}`} style={{
                    width: `${item[3]}%`
                  }} /></div><strong className="w-14 text-right text-chart-5">{item[1]}</strong></div>)}</div></Section><Section title="Business Impact" action={<Badge tone="amber">Estimated</Badge>}><div className="grid gap-3 sm:grid-cols-2"><div><p className="text-xs text-muted-foreground">Revenue Impact · Estimated</p><strong className="text-xl text-chart-5">-€16,500</strong><p className="text-[11px] text-muted-foreground">3.5h ongoing</p></div><div><p className="text-xs text-muted-foreground">Projected if unresolved 24h</p><strong className="text-xl text-chart-5">-€113,000</strong><p className="text-[11px] text-muted-foreground">Estimated · Not a forecast</p></div><div><p className="text-xs text-muted-foreground">Conversion Impact</p><strong className="text-sm text-foreground">Mobile conversion loss</strong></div><div><p className="text-xs text-muted-foreground">Customer Impact</p><strong className="text-sm text-foreground">~2,400 sessions</strong></div></div><p className="mt-4 rounded-lg bg-secondary p-3 text-[11px] text-foreground">Impact figures are estimates based on current deviation rate. Actual impact may differ.</p></Section></div>
      ...
      <section className="mb-5 rounded-xl border border-border bg-card p-5 shadow-[0_3px_16px_rgba(0,0,0,.045)]"><button onClick={() => setTaskOpen(!taskOpen)} className="flex w-full items-center gap-2 text-left text-[16px] font-semibold text-foreground"><Target size={17} className="text-foreground" />Create Task from Anomaly <ChevronDown className={`ml-auto transition ${taskOpen ? 'rotate-180' : ''}`} size={16} /></button>{taskOpen && <div className="mt-4 grid gap-3 border-t border-border pt-4 md:grid-cols-2"><label className="text-xs text-muted-foreground">Task name<input defaultValue="Investigate Revenue Drop" className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-sm text-foreground" /></label><label className="text-xs text-muted-foreground">Owner<select className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm"><option>James K.</option><option>Sarah M.</option></select></label><label className="text-xs text-muted-foreground">Priority<select className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm"><option>High</option><option>Medium</option></select></label><label className="text-xs text-muted-foreground">Due date<input type="date" className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-sm" /></label><label className="text-xs text-muted-foreground md:col-span-2">Description<textarea defaultValue="Investigate the mobile conversion path and document the root cause of the Revenue anomaly." className="mt-1 h-16 w-full rounded-lg border border-border px-3 py-2 text-sm" /></label><div className="md:col-span-2"><button className="rounded-lg bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground">Create Task</button><button onClick={() => setTaskOpen(false)} className="ml-2 rounded-lg border border-border px-4 py-2 text-xs text-muted-foreground">Cancel</button><span className="ml-3 text-[11px] text-muted-foreground">Confirmation required</span></div></div>}</section>
      <Section title="Ask Lulu AI" tone="violet" action={<Badge tone="violet" icon={<Sparkles size={12} />}>AI-generated</Badge>}><div className="flex items-center gap-3 rounded-lg border border-border bg-secondary/40 px-4 py-3"><Search size={17} className="text-muted-foreground" /><input className="flex-1 bg-transparent text-sm outline-none" placeholder="Ask about this anomaly..." /><button className="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground">Ask</button></div><div className="mt-3 flex flex-wrap gap-2">{['What happened?', 'Why is this unusual?', 'When did it start?', 'What caused it?', 'Which segments are affected?', 'How serious is it?', 'Business issue or data issue?', 'What should I do?', 'Could this be an opportunity?', 'Create a task', 'Create a report'].map(q => <button key={q} className="rounded-full border border-border px-3 py-1.5 text-xs text-foreground hover:bg-secondary">{q}</button>)}</div><div className="mt-4 rounded-lg bg-secondary p-4 text-sm leading-6 text-muted-foreground"><strong className="text-foreground">Lulu AI · </strong>Revenue dropped below its modeled range at 10:32 today, concentrated in mobile traffic. I recommend investigating the mobile conversion path first and checking the advertising spend change. <span className="text-[11px] text-foreground">AI-generated</span></div></Section>
