...
export function LuluInventory() {
  ...
  <aside className="fixed inset-y-0 left-0 z-20 hidden w-[232px] border-r border-[var(--muted-foreground)] bg-[var(--sidebar)] px-4 py-5 lg:block">
   ...
   <nav aria-label="Primary navigation" className="space-y-1">{navPrimary.map(item => {
          const Icon = item.icon;
          return <button key={item.label} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-[13px] text-foreground transition hover:bg-secondary hover:text-foreground"><Icon size={16} /><span>{item.label}</span></button>;
        })}</nav>
