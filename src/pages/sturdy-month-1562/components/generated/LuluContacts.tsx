...
function ContactsView() {
  ...
  return <>
    ...
    <div className="flex items-center justify-end gap-2 border-b border-border px-5 py-3 lg:px-8"><button onClick={() => setModal('import')} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm text-foreground hover:border-border hover:bg-secondary focus-visible:ring-2 focus-visible:ring-ring"><Upload size={15} />Import</button><button onClick={() => setModal('create')} className="inline-flex items-center gap-2 rounded-lg bg-primary px-3.5 py-2 text-sm font-medium text-primary-foreground hover:bg-primary"><Plus size={16} />Create Contact</button></div>
    <div className="mx-auto max-w-[1600px] p-5 lg:p-8">
      ...
      {selected.length > 0 && <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-border/30 bg-[var(--secondary)] px-4 py-3 text-sm"><strong className="mr-2 text-foreground">{selected.length} contacts selected</strong>{['Assign Owner', 'Add Tag', 'Change Status', 'Add to Segment',  'Archive'].map(action => <button key={action} className="rounded-md border border-border px-2.5 py-1.5 text-xs text-foreground hover:bg-secondary">{action}</button>)}<button onClick={() => setModal('delete')} className="rounded-md px-2.5 py-1.5 text-xs text-chart-5 hover:bg-chart-5/10">Delete</button><button onClick={() => setSelected([])} className="ml-auto rounded p-1 text-foreground hover:text-foreground" aria-label="Deselect all"><X size={16} /></button></div>}
...
function CompaniesView() {
  ...
  const liveCompanies: Company[] = liveRecords.map((record, index) => {
    const fields = record as unknown as Record<string, unknown>;
    const enrichment = fields.enrichment && typeof fields.enrichment === 'object' && !Array.isArray(fields.enrichment) ? fields.enrichment as Record<string, unknown> : {};
    const name = record.name || String(fields.companyName ?? 'Unnamed company');
    const initials = name.split(' ').map(part => part[0]).join('').slice(0, 2).toUpperCase();
    const numeric = (value: unknown) => typeof value === 'number' && Number.isFinite(value) ? value : null;
    return { id: index + 1, name, industry: String(fields.industry ?? '—'), city: String(fields.city ?? '—'), country: String(fields.country ?? '—'), contacts: numeric(fields.contacts) ?? 0, deals: numeric(fields.deals) ?? 0, revenue: record.valueAmount ?? '—', owner: String(fields.owner ?? '—'), initials, type: (String(fields.type ?? 'Prospect') as Company['type']), status: (String(record.status ?? 'Active') as Company['status']), activity: String(fields.activity ?? '—'), color: 'var(--foreground)', domain: String(fields.domain ?? fields.websiteUrl ?? '—'), description: String(record.description ?? ''), aiDescription: String(fields.aiDescription ?? ''), aiNotes: String(fields.aiNotes ?? ''), intelligenceStatus: String(enrichment.status ?? ''), completeness: numeric(enrichment.completeness), confidence: String(enrichment.confidence ?? ''), missingFields: Array.isArray(enrichment.missingFields) ? enrichment.missingFields.map(String) : [] };
  });
  ...
  return <>
    ...
    <div className="flex items-center justify-end gap-2 border-b border-border px-5 py-3 lg:px-8"><button onClick={() => setModal('import')} className="flex items-center gap-2 rounded-lg border border-border px-3.5 py-2 text-sm text-foreground transition-colors duration-150 hover:border-border hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Upload size={16} />Import</button><button onClick={() => setModal('create')} className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors duration-150 hover:bg-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Plus size={17} />Create Company</button></div>
    ...
    {preview && <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[390px] flex-col overflow-y-auto border-l border-border bg-[var(--sidebar)] shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-[var(--background)]/95 px-5 py-4 backdrop-blur"><div><p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Company preview</p><h2 className="mt-1 font-semibold text-foreground">{preview.name}</h2></div><button onClick={() => setPreview(null)} aria-label="Close preview" className="rounded-md p-2 text-foreground hover:bg-secondary hover:text-foreground"><X size={18} /></button></div><div className="p-5"><div className="flex h-[72px] w-[72px] items-center justify-center rounded-xl text-3xl font-semibold text-foreground" style={{
      backgroundColor: preview.color
    }}>{preview.name[0]}</div><h2 className="mt-4 text-2xl font-semibold text-foreground">{preview.name}</h2><p className="mt-1 text-sm text-muted-foreground">{preview.industry} · {preview.city}, {preview.country}</p><a href="#website" className="mt-3 inline-flex items-center gap-1 text-sm text-foreground hover:text-foreground">{preview.domain}<ExternalLink size={13} /></a><div className="mt-4 flex items-center gap-3"><span className={`rounded-md px-2 py-1 text-xs ${companyTypeClass[preview.type]}`}>{preview.type}</span><span className="text-xs text-foreground">● {preview.status}</span><span className="text-xs text-muted-foreground">Owner {preview.owner}</span></div><div className="my-6 border-t border-border pt-5"><p className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Key Metrics</p><div className="mt-3 grid grid-cols-2 gap-2">{[['Contacts', String(preview.contacts)], ['Open Deals', String(preview.deals)], ['Total Revenue', preview.revenue], ['Revenue This Period', '—']].map(([label, value]) => <div key={label} className="rounded-lg bg-[var(--secondary)] p-3"><p className="text-[11px] text-muted-foreground">{label}</p><p className="mt-1 font-semibold text-foreground">{value}</p></div>)}</div></div><div className="border-t border-border pt-5"><p className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Relationships</p><div className="mt-3 flex flex-wrap gap-2">{[`${preview.contacts} Contacts →`, `${preview.deals} Open Deals →`, 'View Revenue →'].map(item => <button key={item} className="rounded-md border border-border bg-secondary px-2.5 py-2 text-xs text-foreground hover:border-border/40 hover:text-foreground">{item}</button>)}</div></div><div className="mt-6 border-t border-border pt-5"><div className="flex items-center justify-between"><p className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Recent Activity</p><Activity size={15} className="text-muted-foreground" /></div>{preview.activity !== '—' ? <div className="mt-4 flex gap-3"><div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-secondary/15"><Zap size={13} className="text-foreground" /></div><div><p className="text-xs font-medium text-foreground">Activity</p><p className="mt-0.5 text-xs text-muted-foreground">{preview.activity}</p></div></div> : <p className="mt-4 text-sm text-muted-foreground">No recent activity</p>}</div><div className="mt-6 rounded-xl border border-border/25 bg-secondary/[0.08] p-4"><div className="flex items-center gap-2 text-foreground"><Sparkles size={15} /><span className="text-xs font-medium">AI Company Intelligence</span>{preview.intelligenceStatus ? <span className="ml-auto rounded bg-secondary/20 px-1.5 py-0.5 text-[10px]">{preview.intelligenceStatus}</span> : null}</div>{preview.aiDescription || preview.aiNotes ? <><p className="mt-3 text-sm leading-6 text-foreground">{preview.aiDescription || preview.aiNotes}</p><div className="mt-3 flex justify-between text-[11px] text-muted-foreground"><span>{preview.completeness == null ? '—' : `${preview.completeness}%`}</span><span>{preview.confidence || '—'}</span></div></> : <p className="mt-3 text-sm text-muted-foreground">No recent company result</p>}</div><div className="mt-6 border-t border-border pt-5"><p className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Intelligence</p>{preview.missingFields.length ? <div className="mt-3 space-y-2">{preview.missingFields.map(field => <p key={field} className="text-xs text-muted-foreground"><span className="mr-2 text-chart-1">●</span>{field}</p>)}</div> : <p className="mt-3 text-sm text-muted-foreground">—</p>}</div><div className="mt-6 flex flex-wrap gap-2"><button className="rounded-lg bg-primary px-3 py-2 text-xs font-medium text-primary-foreground hover:bg-primary">Open Company</button><button className="rounded-lg border border-border px-3 py-2 text-xs text-foreground hover:bg-secondary">Add Contact</button><button className="rounded-lg border border-border px-3 py-2 text-xs text-foreground hover:bg-secondary">Create Deal</button><button onClick={() => setModal('edit')} className="rounded-lg border border-border px-3 py-2 text-xs text-foreground hover:bg-secondary"><Pencil size={12} className="mr-1 inline" />Edit</button></div></div></aside>}
