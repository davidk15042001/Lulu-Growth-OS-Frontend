...
export function LuluCampaigns() {
  ...
  return <main className="min-h-screen bg-[var(--background)] text-foreground font-sans">
      <header className="h-[68px] border-b border-border bg-[var(--card)]/95 px-5 flex items-center justify-between sticky top-0 z-20">
        ...
        <div className="flex items-center gap-5"><button aria-label="Notifications" className="relative text-foreground hover:text-foreground"><Bell size={18} /><span className="absolute -right-1 -top-1 h-1.5 w-1.5 rounded-full bg-primary text-primary-foreground" /></button><div className="h-8 w-8 rounded-full bg-gradient-to-br from-secondary to-primary grid place-items-center text-xs font-bold text-foreground">MC</div><div className="hidden sm:block text-xs"><strong className="block font-medium">Morgan Cole</strong><span className="text-muted-foreground">Growth Team</span></div><ChevronDown size={14} className="text-muted-foreground" /></div>
      </header>
      <div className="flex">
        <aside className={`${mobileNav ? 'flex' : 'hidden'} lg:flex fixed lg:sticky top-[68px] z-10 lg:z-0 h-[calc(100vh-68px)] w-[244px] shrink-0 flex-col border-r border-border bg-[var(--sidebar)] px-3 py-5`}>
          ...
          <nav className="space-y-1"><button className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-foreground hover:bg-secondary"><LayoutGrid size={16} /><span>Command Center</span></button><button className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-foreground hover:bg-secondary"><Activity size={16} /><span>Operations</span></button><div className="mt-5 mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Marketing</div>{navItems.map(item => <button key={item} onClick={() => item === 'Campaigns' && setSelectedId('summer')} className={`w-full flex items-center justify-between rounded-lg px-3 py-2.5 text-sm transition ${item === 'Campaigns' ? 'bg-secondary/15 text-foreground ring-1 ring-ring/20' : 'text-foreground hover:bg-secondary'}`}><span className="flex items-center gap-3">{item === 'Campaigns' ? <Target size={16} /> : item === 'Content' ? <FileText size={16} /> : item === 'Analytics' ? <Gauge size={16} /> : <Zap size={16} />}<span>{item}</span></span>{item === 'Campaigns' && <span className="rounded bg-secondary/15 px-1.5 py-0.5 text-[10px] text-foreground">12</span>}</button>)}</nav>
        ...
        <section className="min-w-0 flex-1 px-4 py-6 sm:px-7 lg:px-9">
          <div className="mx-auto max-w-[1420px]">
            ...
            <div className="mt-7 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">{[{
              ...
              color: 'amber'
            }].map(metric => <article key={metric.label} className="rounded-xl border border-border bg-secondary p-4"><div className="flex items-center justify-between"><span className={`grid h-7 w-7 place-items-center rounded-lg bg-${metric.color}-400/10 text-${metric.color}-300`}><metric.icon size={15} /></span><span className={`text-[11px] ${metric.color === 'amber' ? 'text-chart-1' : 'text-muted-foreground'}`}>{metric.trend}</span></div><strong className="mt-4 block text-xl font-semibold tracking-tight text-foreground">{metric.value}</strong><span className="mt-1 block text-xs text-muted-foreground">{metric.label}</span></article>)}</div>
