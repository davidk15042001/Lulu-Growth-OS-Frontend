...
const periods = ['Current Month', 'Previous Month', 'Current Quarter', 'Previous Quarter', 'Current Year', 'Previous Year', 'Custom Range'];
const horizons = ['30 Days', '60 Days', '90 Days', '6 Months', '12 Months'];
const kpis = [['MRR', '$284,500', '↑ 6.8%', 'vs last month', 'Observed'], ['ARR', '$3.41M', '↑ 8.2%', 'vs last year', 'Calculated'], ['MRR Growth', '+6.8%', '↑ 1.2pp', 'vs last month', 'Calculated'], ['New MRR', '$24,300', '↑ 12.4%', 'this month', 'Observed'], ['Expansion MRR', '$11,200', '↑ 4.1%', 'this month', 'Observed'], ['Contraction MRR', '-$4,800', '↓ 2.1%', 'this month', 'Observed'], ['Churned MRR', '-$8,100', '↑ 0.8%', 'churn rate', 'Observed'], ['Net Revenue Retention', '108%', '↑ 2pp', 'this quarter', 'Calculated']];
const customers = [['Northstar Labs', 'Growth plan · 42 seats', '$6,400', 'Jun 28'], ['Morrow & Co.', 'Pro plan · 18 seats', '$5,800', 'Jun 24'], ['Kite Systems', 'Enterprise plan', '$4,600', 'Jun 21'], ['Juniper Health', 'Growth plan · 26 seats', '$4,100', 'Jun 18'], ['Studio Meridian', 'Pro plan · 14 seats', '$3,400', 'Jun 12']];
const expansions = [['Northstar Labs', '$4,800', '$6,400', '+$1,600', 'Jun 28', 'Upgrade'], ['Kite Systems', '$3,900', '$4,600', '+$700', 'Jun 21', 'Additional seats'], ['Lumen Legal', '$2,100', '$3,100', '+$1,000', 'Jun 18', 'Additional product'], ['Morrow & Co.', '$4,900', '$5,800', '+$900', 'Jun 14', 'Upgrade'], ['Juniper Health', '$3,200', '$4,100', '+$900', 'Jun 06', 'Additional seats']];
const contractions = [['Valley Works', '$4,200', '$3,400', '-$800', 'Jun 27'], ['Brightwell', '$2,900', '$2,200', '-$700', 'Jun 20'], ['Orbit House', '$1,800', '$1,200', '-$600', 'Jun 16'], ['Cedar & Pine', '$3,100', '$2,400', '-$700', 'Jun 05']];
const products = [['Lulu Analytics', '$94,200', '$1.13M', '+8.4%', '186', '-$2,100'], ['Lulu Automate', '$71,800', '$861K', '+12.1%', '124', '-$1,400'], ['Lulu Insights', '$58,400', '$701K', '+5.7%', '98', '-$2,800'], ['Lulu Connect', '$36,700', '$440K', '+4.2%', '76', '-$900'], ['Lulu Assist', '$23,400', '$281K', '+15.8%', '64', '-$900']];
const segments = [['Enterprise', '$128,400', '$1.54M', '+10.2%', '84', '98.1%'], ['Mid-Market', '$91,600', '$1.10M', '+7.8%', '212', '97.4%'], ['SMB', '$56,200', '$674K', '+3.1%', '252', '95.8%'], ['Custom', '$8,300', '$100K', '+1.9%', '18', '96.2%']];
const channels = [['Organic', '$8,600', '$104K', '+14.2%', '92'], ['Paid', '$5,900', '$71K', '+8.4%', '64'], ['Referral', '$4,800', '$58K', '+18.1%', '42'], ['Partner', '$3,700', '$44K', '+9.8%', '31'], ['Direct', '$1,300', '$16K', '+2.6%', '14'], ['Other', '$0', '$0', '—', '—']];
const cohortRows = [['Jan 24', '100%', '91%', '86%', '81%', '78%', '75%', '73%', '71%', '69%', '68%', '67%', '66%'], ['Feb 24', '100%', '93%', '88%', '84%', '80%', '78%', '76%', '74%', '72%', '71%', '70%', '—'], ['Mar 24', '100%', '89%', '84%', '80%', '76%', '73%', '71%', '69%', '68%', '67%', '—', '—'], ['Apr 24', '100%', '95%', '90%', '87%', '83%', '80%', '78%', '—', '—', '—', '—', '—'], ['May 24', '100%', '92%', '87%', '82%', '79%', '—', '—', '—', '—', '—', '—', '—'], ['Jun 24', '100%', '94%', '89%', '85%', '—', '—', '—', '—', '—', '—', '—', '—']];
...
export function RecurringRevenue() {
  ...
  return <div className="rr-shell"><Sidebar /><main className="rr-main"><Header /><div className="rr-content"><section className="kpi-scroll" aria-label="Recurring revenue summary">{kpis.map(([label, value, trend, period, badge]) => <article className={`kpi-card ${label.includes('Churn') || label.includes('Contraction') ? 'risk-kpi' : ''}`} key={label}><div className="kpi-top"><span>{label}</span><span className={badge === 'Calculated' ? 'badge calculated' : 'badge observed'}>{badge}</span></div><strong>{value}</strong><div><span className={trend.startsWith('↓') ? 'trend negative' : 'trend'}>{trend.startsWith('↓') ? <ArrowDownRight size={13} /> : <ArrowUpRight size={13} />} {trend}</span><small>{period}</small></div></article>)}</section>
<section className="panel chart-panel"><div className="section-head"><div><h2>MRR Growth</h2><p className="note">Monthly recurring revenue movement · illustrative sample data</p></div><div className="segmented">{(['Daily', 'Weekly', 'Monthly'] as Tab[]).map(item => <button key={item} className={granularity === item ? 'selected' : ''} onClick={() => setGranularity(item)}>{item}</button>)}</div></div><AreaChart /></section>
<div className="two-col"><section className="panel chart-panel"><div className="section-head"><div><h2>ARR Trend</h2><span className="badge calculated">Calculated</span></div><div className="segmented"><button className="selected">Monthly</button><button>Quarterly</button><button>Annual</button></div></div><div className="mini-bars"><div className="bar b1"><span>$2.58M</span></div><div className="bar b2"><span>$2.74M</span></div><div className="bar b3"><span>$2.91M</span></div><div className="bar b4"><span>$3.12M</span></div><div className="bar b5"><span>$3.41M</span></div></div><div className="bar-labels"><span>Feb</span><span>Mar</span><span>Apr</span><span>May</span><span>Jun</span></div><p className="note chart-note">ARR is a calculated metric based on current MRR × 12</p></section><section className="panel bridge-panel"><div className="section-head"><div><h2>MRR Movement</h2><p className="note">Current month bridge</p></div><span className="badge observed">Observed</span></div><div className="bridge"><div className="bridge-item"><i className="bridge-neutral" style={{
                  height: '54%'
                }} /><span>Beginning<br /><b>$266.9k</b></span></div><div className="bridge-item"><i className="bridge-green" style={{
                  height: '78%'
                }} /><span>New<br /><b>+$24.3k</b></span></div><div className="bridge-item"><i className="bridge-blue" style={{
                  height: '61%'
                }} /><span>Expansion<br /><b>+$11.2k</b></span></div><div className="bridge-item"><i className="bridge-amber" style={{
                  height: '30%'
                }} /><span>Contraction<br /><b>−$4.8k</b></span></div><div className="bridge-item"><i className="bridge-red" style={{
                  height: '42%'
                }} /><span>Churned<br /><b>−$8.1k</b></span></div><div className="bridge-item"><i className="bridge-purple" style={{
                  height: '82%'
                }} /><span>Ending<br /><b>$289.5k</b></span></div></div></section></div>
<div className="two-col"><Table title="New MRR" total="New MRR: $24,300" headers={['Customer', 'New recurring contract', 'New MRR', 'Date', 'Action']} rows={customers} /><Table title="Expansion MRR" total="Expansion MRR: $11,200" headers={['Customer', 'Previous MRR', 'Current MRR', 'Expansion', 'Date', 'Cause']} rows={expansions} /></div>
<div className="two-col"><Table title="Contraction MRR" headers={['Customer', 'Previous MRR', 'Current MRR', 'Reduction', 'Date', 'Action']} rows={contractions} note="Cause not automatically classified without source data" /><section className="panel table-panel"><div className="section-head"><div><h2>Churned Revenue</h2><p className="note">7 customers · illustrative sample data</p></div><span className="badge negative-badge">Revenue impact</span></div><div className="churn-stats"><div><b>7</b><small>Churned customers</small></div><div><b>$8,100</b><small>Churned MRR</small></div><div><b>2.8%</b><small>Churn rate</small></div></div><div className="tab-row">{['By Product', 'By Plan', 'By Segment', 'By Geography'].map(item => <button key={item} className={churnTab === item ? 'active-tab' : ''} onClick={() => setChurnTab(item)}>{item}</button>)}</div><table className="small-table"><thead><tr><th>{churnTab.replace('By ', '')}</th><th>Churned MRR</th><th>Customers</th></tr></thead><tbody>{[['Lulu Insights', '$2,800', '3'], ['Growth plan', '$2,100', '2'], ['Mid-Market', '$1,900', '1']].map(r => <tr key={r[0]}><td>{r[0]}</td><td className="negative-cell">{r[1]}</td><td>{r[2]}</td></tr>)}</tbody></table></section></div>
<div className="two-col"><section className="panel retention"><div className="section-head"><div><h2>Revenue Retention</h2><p className="note">Retention health across the current period</p></div><span className="badge calculated">Calculated</span></div><div className="retention-grid"><div><span>Gross Revenue Retention</span><b className="good">97.2%</b><small>Calculated</small></div><div><span>Net Revenue Retention</span><b className="purple-text">108%</b><small>Calculated</small></div><div><span>Customer Retention</span><b>97.0%</b><small>Calculated</small></div><div><span>Churn Rate</span><b>2.8%</b><small>Calculated</small></div></div><p className="method">Methodology: retention metrics are calculated from observed recurring revenue records, expansion, contraction and churn movements.</p></section><Table title="Revenue by Product" headers={['Product', 'MRR', 'ARR', 'Growth', 'Customers', 'Churned MRR', 'Action']} rows={products} /></div>
...
<section className="panel cohort"><div className="section-head"><div><h2>Cohort Analysis <span className="badge calculated">Calculated</span></h2><p className="note">Recurring revenue retention by customer start month</p></div><div className="segmented"><button className={cohort === 'Monthly' ? 'selected' : ''} onClick={() => setCohort('Monthly')}>Monthly</button><button className={cohort === 'Quarterly' ? 'selected' : ''} onClick={() => setCohort('Quarterly')}>Quarterly</button></div></div><div className="cohort-wrap"><table className="cohort-table"><thead><tr><th>Cohort</th>{Array.from({
                    length: 12
                  }, (_, i) => <th key={i}>M{i}</th>)}</tr></thead><tbody>{cohortRows.map(row => <tr key={row[0]}>{row.map((cell, i) => <td key={`${row[0]}-${i}`} className={i === 0 ? '' : cell === '—' ? 'empty-cell' : `heat-${Math.min(5, Math.max(1, Math.ceil(parseInt(cell) / 20)))}`}>{cell}</td>)}</tr>)}</tbody><tfoot><tr><th>Average</th><td>100%</td><td>92%</td><td>87%</td><td>83%</td><td>79%</td><td>77%</td><td>75%</td><td>72%</td><td>70%</td><td>69%</td><td>68%</td><td>66%</td></tr></tfoot></table></div><p className="method">Cohorts group customers by their first recurring revenue month. Retention shows % of starting MRR retained each subsequent month.</p></section>
