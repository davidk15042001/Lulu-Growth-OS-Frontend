...
export function LuluAIOptimization() {
  const { items, loading, error } = useLiveRecords('ad_optimizations');
  return <div className="lulu"><aside className="sidebar"><div className="brand"><span className="mark"><Sparkles size={15} /></span><b>Lulu AI</b></div><LuluSectionNavigation activeId="zesty-grass-9196" /></aside><main className="main"><header className="topbar"><div className="crumb"><span>Marketing</span><ChevronRight size={13} /><strong>AI Optimization</strong></div><Button primary icon={<Sparkles size={13} />}>Ask Lulu AI</Button></header><div className="content"><div className="page-heading"><div><p className="eyebrow">AI OPTIMIZATION</p><h1>AI Optimization</h1><p>Live optimization opportunities from connected advertising and marketing sources.</p></div><div className="heading-actions"><span className="inline-flex items-center rounded-lg border border-border bg-card px-3 py-2 text-sm text-muted-foreground">Use Update in the navigation bar</span><Button icon={<Download size={13} />}>Export</Button></div></div><section className="section"><div className="empty"><Target size={34} /><h2>{loading ? 'Loading live optimization data…' : error ? 'Optimization data could not be loaded' : 'No optimization data available yet'}</h2><p>{items.length ? `${items.length} live optimization records are available.` : 'Connect advertising platforms to populate optimization opportunities, risks and performance analysis. No example metrics are displayed.'}</p></div></section></div></main></div>;
}
