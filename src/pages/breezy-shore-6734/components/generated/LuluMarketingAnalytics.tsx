...
export const LuluMarketingAnalytics = () => {
  ...
  return <div className="min-h-screen bg-card font-sans text-[var(--foreground)]">
    <aside className={`${mobileNav ? 'flex' : 'hidden'} fixed inset-y-0 left-0 z-30 w-[220px] flex-col bg-[var(--sidebar)] px-3 py-5 lg:flex`}>
      ...
      <nav className="flex-1 space-y-5" aria-label="Main navigation">{navGroups.map(group => <section key={group.label}><p className="mb-2 px-3 text-[10px] font-medium tracking-[.12em] text-[var(--muted-foreground)]">{group.label}</p>{group.items.map(item => <button key={item} className={`flex h-9 w-full items-center gap-3 border-l-2 px-3 text-left text-sm ${item === 'Marketing Analytics' ? 'border-[var(--border)] bg-[var(--primary)] text-[var(--primary-foreground)]' : 'border-transparent text-[var(--primary-foreground)] hover:bg-[var(--primary)] hover:text-primary-foreground'}`}><Activity size={14} /><span>{item}</span></button>)}</section>)}</nav>
