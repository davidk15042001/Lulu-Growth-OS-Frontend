...
import { composioApi, type AdminComposioToolkit } from "../api/composio";
...
export function AdminComposioCatalog() {
  ...
  return <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
    ...
    {loading && items.length === 0 ? <p className="mt-5 text-sm text-muted-foreground">Katalog wird geladen…</p> : items.length === 0 ? <p className="mt-5 text-sm text-muted-foreground">Keine Composio-Apps gefunden.</p> : <div className="mt-5 divide-y divide-border overflow-hidden rounded-xl border border-border">{items.map((item) => <article key={item.slug} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong>{item.name}</strong><span className="font-mono text-xs text-muted-foreground">{item.slug}</span>{item.customerRestricted ? <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800">Lulu managed</span> : null}</div><p className="mt-1 text-xs text-muted-foreground">{item.certificationStatus}{item.publishedAt ? ` · published ${new Date(item.publishedAt).toLocaleDateString()}` : ""}</p></div><button type="button" onClick={() => void toggle(item)} disabled={busySlug === item.slug || item.customerRestricted} className={`inline-flex w-fit items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50 ${item.customerAvailable ? "border border-border" : "bg-primary text-primary-foreground"}`}>{item.customerAvailable ? <><ShieldOff size={14} />Für Kunden deaktivieren</> : <><ShieldCheck size={14} />Für Kunden veröffentlichen</>}</button></article>)}</div>}
